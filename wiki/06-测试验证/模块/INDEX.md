objectId: module-index
objectType: guide

# 模块 INDEX

| 模块 | 对象 | 正式入口 |
|------|------|------|
| `analysis` | `module/analysis/basic` | `run test module --id module/analysis/basic` |
| `managed-smoke` | `module/managed-smoke/basic` | `run test module --id module/managed-smoke/basic` |
| `reflection` | `module/reflection/basic` | `run test module --id module/reflection/basic` |
| `interop` | `module/interop/basic` | `run test module --id module/interop/basic` |
| `hosted-runtime` | `module/hosted-runtime/basic` | `run test module --id module/hosted-runtime/basic` |
| `trace-export (windows)` | `module/trace-export/windows` | `run test module --id module/trace-export/windows` |
| `trace-export (macos)` | `module/trace-export/macos` | `run test module --id module/trace-export/macos` |
