...
- `2026-05-29`：新增统一 CLI 入口 `verification/__main__.py`，支持 `--slug`/`--batch`/`--ci`/`--managed-bench` 四种模式。删除 `scripts/test/robust-batch.py` 和 `run-batch-verification.py`（功能由 `--skip` 参数覆盖）。修复 `ci_smoke.py` 路径 bug。更新 [`foundation-dll-verification-chain-authenticity.md`](./foundation-dll-verification-chain-authenticity.md) 关键文件路径表。
