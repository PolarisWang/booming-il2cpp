...
> D3（Dual-Layer Dispatch）是热更新的核心分派机制。Codegen 为每个公开方法生成静态 Dispatch Table，通过模式感知分支在 AOT direct call 和 Interpreter patch 路径之间切换。本文描述 D3 的完整翻译路径：从构建期 emit 到运行时 dispatch，再到验证流程。
...
当前阶段：D3 codegen 已验证 dispatch table + NameIndex + token→slot 映射的正确性。调用站点的模式感知分支 emit 尚未实现（见"实现状态"）。
...
每个 family 的 D3 验证 exe 执行以下 7 步流程：
...
所有 32 个 family 共 459 个方法 D3 验证通过。
...
| D3 验证 runner | `build/toolchains/run/testing/foundation_dll/d3_hotupdate_runner.py` |
...
| D3 决策文档 | `wiki/03-功能模块/06-il2cpp核心架构/04-历史决策/18-HotUpdateDispatchD3-决策.md` |
