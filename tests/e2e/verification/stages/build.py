...
def _compile_custom_subjects(
    ...
    # 防线 4 硬阻断：解析 ATG stderr，检测 [probe-warn] ALL-DEFAULT-SETS 标记。
    # 当已知 stub-蒙过嫌疑方法（Convert.ChangeType / Enum.TryParse / 等）的 value
    # set 全部是 default 输入时，probe 无法暴露 stub-return-0，这是测试覆盖缺口。
    # 对【已知 stub 方法白名单】强制要求 ≥1 个非-default 输入 —— 若仍全 default，
    # 说明 ATG 的多值探针未生效，pipeline 应 fail（防止蒙过静默继续）。
    # 普通方法（不在此白名单）仍只打印警告，不阻断。
    from collections import defaultdict
    all_default_sets = []
    for line in result.stderr.splitlines():
        if "[probe-warn] ALL-DEFAULT-SETS:" in line:
            marker = line.split("ALL-DEFAULT-SETS:", 1)[-1].strip()
            all_default_sets.append(marker)
            # 解析 method name 用于白名单匹配
    # 已知必须非-default 探针的方法族（stub 蒙过高危）
    STUB_BLOCK_FAMILIES = ["ChangeType", "Enum.TryParse"]
    blocked = [m for m in all_default_sets if any(fam in m for fam in STUB_BLOCK_FAMILIES)]
    if blocked:
        # 这些方法的全-default 输入绕过了 stub-return-0 蒙过检测 —— 触发硬阻断
        print("  [build] BLOCK (防线4): 以下 stub 蒙过嫌疑方法只有 default 输入探针:")
        for m in blocked:
            print(f"      {m}")
        if os.environ.get("CHAOS_FACT266_BLOCK_ALL_DEFAULT", "1") != "0":
            print("  [build] ERROR: 高危 stub 方法无非-default 探针 — 蒙过无法检测，拒绝继续。"
                  " 需修复 ATG 多值探针或实现真语义。设 CHAOS_FACT266_BLOCK_ALL_DEFAULT=0 临时放行。")
            return None
    elif all_default_sets:
        print(f"  [build] INFO: {len(all_default_sets)} 个方法只有 default 输入探针（非高危族，仅警告）。")
...
def run_build(ctx: ChunkContext, stages: dict[str, StageResult]) -> StageResult:
    ...
    if not _custom_fallback:
        ...
        combined_csproj.write_text(
            ...
            "    <AllowUnsafeBlocks>true</AllowUnsafeBlocks>\n"
            "    <NoWarn>$(NoWarn);SYSLIB0011;SYSLIB5006;CA2252</NoWarn>\n"
            "    <EnableDefaultCompileItems>false</EnableDefaultCompileItems>\n"
            "  </PropertyGroup>\n"
            "  <ItemGroup>\n"
            f"    <Compile Include=\"{combined_cs_path.name}\" />\n"
