...
def _regenerate_host_arrays(ctx, metadata: dict) -> bool:
    ...
    #
    # The dispatcher symbols this function parses for (kSubjectSlotMap,
    # kHotpatchTypeTable, kHotpatchMethodTable, ...) are NOT all in
    # native-aot.generated.cpp any more: the emitter splits the payload across
    # native-aot.payload.page-XXXX.cpp translation units, and kSubjectSlotMap
    # lands in whichever page the payload sectioner placed it.  Reading only
    # the generated.cpp therefore reports "Could not parse kSubjectSlotMap" on
    # every chunk large enough to be paged, which fails the whole hotupdate
    # stage.  Concatenate the page files behind the main file so every regex
    # below still sees one logical document.
    aot_gen_candidates = [
        ctx.chunk_dir / "native" / "subjects",
        ctx.chunk_dir / "native" / "codegen" / "generated",
    ]
    aot_gen_dir = next((d for d in aot_gen_candidates if (d / "native-aot.generated.cpp").exists()), None)
    if aot_gen_dir is None:
        print(f"  [hotupdate] Cannot read AOT generated code at either location")
        return False

    aot_parts = [aot_gen_dir / "native-aot.generated.cpp"]
    # Deterministic order: the page suffix is zero-padded, so a plain sort is stable.
    aot_parts.extend(sorted(aot_gen_dir.glob("native-aot.payload.page-*.cpp")))
    aot_code = "".join(
        p.read_text(encoding="utf-8", errors="replace") for p in aot_parts if p.exists()
    )
