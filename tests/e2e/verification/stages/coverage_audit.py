...
def run_coverage_audit(ctx: ChunkContext, stages: dict[str, StageResult]) -> StageResult:
    ...
    if not metadata_path.exists():
        ...
        if missing:
            ...
            # The partition (namespace-partition.json) is a static definition of
            # which methods belong to which chunk, generated by the partitioning
            # tool from the source DLL metadata.  The ATG, which generates the
            # actual subjects, probes the same DLL and may produce fewer methods
            # because some methods are not probeable (delegates, events, abstract,
            # interface-only, generic with unsupported constraints, etc.).  A
            # declared≠covered gap is therefore EXPECTED and does NOT indicate a
            # regression — it signals that the partition includes methods that
            # ATG cannot probe.  Report as advisory, not error.
            print(f"  [coverage-audit]   INFO: {msg}")
