...
class Phase8MobileFoundationTests(unittest.TestCase):
    ...
    def test_benchmark_manifests_declare_mobile_native_perf_surface(self) -> None:
        for subject_id in MOBILE_BENCHMARK_SUBJECTS:
            ...
            self.assertIn("mobile-native-benchmark", pipeline_ids, msg=subject_id)
            self.assertIn("android-arm64-native-perf", matrix_ids, msg=subject_id)
            self.assertIn("ios-arm64-native-perf", matrix_ids, msg=subject_id)

            mobile_pipeline = next(
                pipeline
                for pipeline in list(manifest.get("executionPipelines") or [])
                if str(pipeline.get("pipelineId") or "") == "mobile-native-benchmark"
            )
            stage_kinds = {
                str(stage.get("kind") or "")
                for stage in list(dict(mobile_pipeline).get("stages") or [])
            }
            self.assertEqual(
                {
                    "source-resolve",
