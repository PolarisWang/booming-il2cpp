...
SUBJECT_ROOT = REPO_ROOT / "subjects" / "InterfaceDispatchProof"
MANIFEST_PATH = SUBJECT_ROOT / "subject.manifest.json"
SOURCE_PROJECT_PATH = SUBJECT_ROOT / "source" / "InterfaceDispatchProof.csproj"
...
class Phase2AotRuntimeCompletenessTests(unittest.TestCase):
    def test_interface_dispatch_subject_tree_realizes_phase2_batch1_proof_slice(self) -> None:
        ...
        self.assertEqual("InterfaceDispatchProof", manifest["subjectId"])
        self.assertEqual("dotnet-project", manifest["source"]["type"])
        self.assertEqual("subjects/InterfaceDispatchProof/source/InterfaceDispatchProof.csproj", manifest["source"]["path"])
        self.assertEqual("InterfaceDispatchProof/InterfaceDispatchProofEntry::Run()", manifest["source"]["entry"])
