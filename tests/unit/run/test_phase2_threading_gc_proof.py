...
SUBJECT_ROOT = REPO_ROOT / "subjects" / "ThreadingProof"
MANIFEST_PATH = SUBJECT_ROOT / "subject.manifest.json"
SOURCE_PROJECT_PATH = SUBJECT_ROOT / "source" / "ThreadingProof.csproj"
...
class Phase2ThreadingGcProofTests(unittest.TestCase):
    def test_threading_proof_subject_tree_realizes_phase2_batch3_slice(self) -> None:
        ...
        self.assertEqual("ThreadingProof", manifest["subjectId"])
        self.assertEqual("dotnet-project", manifest["source"]["type"])
        self.assertEqual("subjects/ThreadingProof/source/ThreadingProof.csproj", manifest["source"]["path"])
        self.assertEqual("ThreadingProof/ThreadingProofEntry::Run()", manifest["source"]["entry"])
