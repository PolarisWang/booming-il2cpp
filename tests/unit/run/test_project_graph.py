...
GOLDEN_ROOT = REPO_ROOT / "subjects" / "golden"
...
class ProjectGraphTests(unittest.TestCase):
    ...
    def _run_convert(self, subject_id: str) -> Path:
        ...
        subject_root = GOLDEN_ROOT / subject_id
        run_checked(
            [
                "dotnet",
                "run",
    ...
    def test_golden_subjects_exist(self) -> None:
        ...
        actual_subjects = {path.name for path in GOLDEN_ROOT.iterdir() if path.is_dir()}
        self.assertTrue(expected_subjects.issubset(actual_subjects))

    def test_convert_golden_simple_lib_loads_entry_and_project_reference_assemblies(self) -> None:
    ...
    def test_convert_golden_multi_project_tracks_transitive_cross_assembly_references(self) -> None:
        output_root = self._run_convert("GoldenMultiProject")
    ...
    def test_convert_golden_with_package_exposes_package_reference_in_project_graph(self) -> None:
        output_root = self._run_convert("GoldenWithPackage")
