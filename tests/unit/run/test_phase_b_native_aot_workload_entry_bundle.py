...
PROJECT_PATH = REPO_ROOT / "subjects" / SUBJECT_ID / "source" / f"{SUBJECT_ID}.csproj"
DLL_PATH = REPO_ROOT / "subjects" / SUBJECT_ID / "source" / "bin" / "Release" / "net8.0" / f"{SUBJECT_ID}.dll"
...
class NativeAotWorkloadEntryBundleTests(unittest.TestCase):
    ...
    def _ensure_bundle_generated(self) -> None:
        ...
        subject_intermediate_root = tooling_module.allocate_dotnet_intermediate_dir(SUBJECT_ID, host_platform="windows")
        if subject_intermediate_root is None:
            subject_intermediate_root = TEST_OUTPUT_ROOT / "obj" / SUBJECT_ID
        ...
        run_checked(
            [
                ...
                "-c",
                "Release",
                f"-p:BaseIntermediateOutputPath={Path(subject_intermediate_root).as_posix()}/$(MSBuildProjectName)/",
                f"-p:MSBuildProjectExtensionsPath={Path(subject_intermediate_root).as_posix()}/$(MSBuildProjectName)/",
            ],
            cwd=REPO_ROOT,
        )
