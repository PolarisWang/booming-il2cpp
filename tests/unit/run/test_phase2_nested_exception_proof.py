...
SUBJECT_ROOT = REPO_ROOT / "subjects" / "NestedExceptionProof"
MANIFEST_PATH = SUBJECT_ROOT / "subject.manifest.json"
SOURCE_PROJECT_PATH = SUBJECT_ROOT / "source" / "NestedExceptionProof.csproj"
...
class Phase2NestedExceptionProofTests(unittest.TestCase):
    def test_nested_exception_subject_tree_realizes_phase2_batch4_slice(self) -> None:
        ...
        self.assertEqual("NestedExceptionProof", manifest["subjectId"])
        self.assertEqual("dotnet-project", manifest["source"]["type"])
        self.assertEqual("subjects/NestedExceptionProof/source/NestedExceptionProof.csproj", manifest["source"]["path"])
        self.assertEqual("NestedExceptionProof/NestedExceptionProofEntry::Run()", manifest["source"]["entry"])
