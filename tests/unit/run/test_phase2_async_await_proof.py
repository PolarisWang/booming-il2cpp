...
SUBJECT_ROOT = REPO_ROOT / "subjects" / "AsyncAwaitProof"
MANIFEST_PATH = SUBJECT_ROOT / "subject.manifest.json"
SOURCE_PROJECT_PATH = SUBJECT_ROOT / "source" / "AsyncAwaitProof.csproj"
...
class Phase2AsyncAwaitProofTests(unittest.TestCase):
    def test_async_await_subject_tree_realizes_phase2_batch2_proof_slice(self) -> None:
        ...
        self.assertEqual("AsyncAwaitProof", manifest["subjectId"])
        self.assertEqual("dotnet-project", manifest["source"]["type"])
        self.assertEqual("subjects/AsyncAwaitProof/source/AsyncAwaitProof.csproj", manifest["source"]["path"])
        self.assertEqual("AsyncAwaitProof/AsyncAwaitProofEntry::Run()", manifest["source"]["entry"])
