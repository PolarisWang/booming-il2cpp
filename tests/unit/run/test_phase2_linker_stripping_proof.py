...
SUBJECT_ROOT = REPO_ROOT / "subjects" / "LinkerStrippingProof"
MANIFEST_PATH = SUBJECT_ROOT / "subject.manifest.json"
SOURCE_PROJECT_PATH = SUBJECT_ROOT / "source" / "LinkerStrippingProof.csproj"
...
class Phase2LinkerStrippingProofTests(unittest.TestCase):
    ...
    def test_linker_stripping_subject_tree_realizes_phase2_batch5_slice(self) -> None:
        ...
        self.assertEqual("LinkerStrippingProof", manifest["subjectId"])
        self.assertEqual("dotnet-project", manifest["source"]["type"])
        self.assertEqual("subjects/LinkerStrippingProof/source/LinkerStrippingProof.csproj", manifest["source"]["path"])
        self.assertEqual("LinkerStrippingProof/LinkerStrippingProofEntry::Run()", manifest["source"]["entry"])
    ...
    def test_convert_emits_preserve_descriptor_and_keeps_stripped_type_out_of_analysis(self) -> None:
        ...
        self.assertIn("LinkerStrippingProof/PreservedUnusedHarness", preserve_subject_ids)
        self.assertNotIn("LinkerStrippingProof/StrippedUnusedHarness", preserve_subject_ids)
        self.assertIn("LinkerStrippingProof/ReachableHarness", registered_subject_ids)
        self.assertIn("LinkerStrippingProof/PreservedUnusedHarness", registered_subject_ids)
        self.assertNotIn("LinkerStrippingProof/StrippedUnusedHarness", registered_subject_ids)
        self.assertNotIn("LinkerStrippingProof/StrippedUnusedHarness::Marker()", registered_subject_ids)
