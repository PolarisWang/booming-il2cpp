...
ENTRY_SUBJECT_ID = "CoreRuntimeFeatures/FinallyAndFilterProofEntry::Run()"
EXECUTE_SUBJECT_ID = "CoreRuntimeFeatures/FinallyAndFilterProofEntry::Execute()"
TEST_OUTPUT_ROOT = REPO_ROOT / "artifacts" / ".tmp-tests" / "phase3-finally-filter-native-aot"
...
class Phase3FinallyFilterNativeAotTests(unittest.TestCase):
    @classmethod
    ...
    def test_aot_core_ir_keeps_real_filter_and_finally_shape_for_owner_proof(self) -> None:
        self._ensure_bundle_generated()
        ...
        self.assertIn("endfilter", [instruction["op"] for instruction in execute_method["instructions"]])
        self.assertIn("endfinally", [instruction["op"] for instruction in execute_method["instructions"]])
        self.assertIn("leave", [instruction["op"] for instruction in execute_method["instructions"]])

    def test_owner_proof_filter_is_dispatch_free(self) -> None:
        self._ensure_bundle_generated()
        ...
        self.assertNotIn(
            ...
            [instruction.get("callee") for instruction in execute_method["instructions"]],
        )

    def test_emit_native_aot_succeeds_for_finally_filter_owner_proof(self) -> None:
        ...
        self.assertIn("CoreRuntimeFeatures_FinallyAndFilterProofEntry_Execute", generated_cpp)
        self.assertIn("CoreRuntimeFeatures_FinallyAndFilterProofEntry_Run", generated_cpp)
