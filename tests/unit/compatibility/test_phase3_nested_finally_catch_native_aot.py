...
OUTPUT_ROOT = REPO_ROOT / "artifacts" / ".tmp-tests" / "phase3-nested-finally-catch-native-aot"
GENERATED_CPP_RELATIVE_PATH = Path("generated") / "native-aot.generated.cpp"
SYNTHETIC_ASSEMBLY_NAME = "NestedFinallyCatchNativeAotHarness"
SYNTHETIC_ENTRY_SUBJECT_ID = f"{SYNTHETIC_ASSEMBLY_NAME}/Program::RunNestedFinally()"
...
class Phase3NestedFinallyCatchNativeAotTests(unittest.TestCase):
    @classmethod
...
internal static class Program
{
    ...
    private static string NestedFinally(bool shouldThrow)
    {
    ...
    private static int RunNestedFinally()
    {
        return NestedFinally(true) is null ? 1 : 0;
    }
}
""".strip()
            + "\n",
    ...
    def test_emit_native_aot_succeeds_for_nested_finally_catch_shape(self) -> None:
