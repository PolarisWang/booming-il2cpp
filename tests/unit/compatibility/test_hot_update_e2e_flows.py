...
BRIDGE_ROUNDTRIP_PROOF_ROOT = HOT_UPDATE_FIXTURE_ROOT / "BridgeRoundtripProof"
...
class HotUpdateE2ETests(unittest.TestCase):
    def test_runtime_manager_and_bridge_surface_support_generic_hot_update_e2e(self) -> None:
        ...
        for required_fragment in [
            ...
            "IReadOnlyList<object?>",
            "RegisterByRefInt32Target",
            "RegisterPointerInt32Target",
            "RegisterReadOnlySpanInt32Target",
            "TargetAuthorityKey",
    ...
    def test_auto_bridge_and_version_rollback_proofs_cover_dynamic_dispatch_and_history(self) -> None:
        ...
        for required_fragment in [
            ...
            "version-rollback-v1-handle=11",
            "version-rollback-v1-stale=stale-handle",
            "version-rollback-v2=22",
            "version-rollback-v2-handle=22",
            "version-rollback-v2-stale=stale-handle",
            "version-rollback-back-v1=11",
            "version-rollback-back-aot=5",
            "version-rollback-target-aot=rejected",
            "version-rollback-target-aot-reason=hot update package target AOT version '2.0.0' is not compatible with runtime AOT version '1.0.0'. Expected matching major.minor compatibility band.",
            "version-rollback-target-aot-can-rollback=false",
            "version-rollback-kernel-version=rejected",
            "version-rollback-kernel-version-reason=hot update package kernel artifact version 'v2' is not compatible with runtime kernel artifact version 'v1'.",
            "version-rollback-kernel-version-can-rollback=false",
        ]:
            self.assertIn(required_fragment, rollback_output)

    def test_bridge_roundtrip_fixture_covers_aot_hot_update_and_engine_boundaries(self) -> None:
        self.assertTrue(BRIDGE_ROUNDTRIP_PROOF_ROOT.is_dir(), msg=f"missing fixture root: {BRIDGE_ROUNDTRIP_PROOF_ROOT}")
        self.assertTrue(BRIDGE_ROUNDTRIP_PROOF_PROJECT_PATH.is_file(), msg=f"missing project: {BRIDGE_ROUNDTRIP_PROOF_PROJECT_PATH}")
        self.assertTrue(BRIDGE_ROUNDTRIP_PROOF_PROGRAM_PATH.is_file(), msg=f"missing program: {BRIDGE_ROUNDTRIP_PROOF_PROGRAM_PATH}")

        completed = run_checked(
            ["dotnet", "run", "--project", str(BRIDGE_ROUNDTRIP_PROOF_PROJECT_PATH), "-c", "Release", "--"],
            cwd=REPO_ROOT,
        )
    ...
    def test_hot_update_benchmark_subjects_keep_workload_contract_and_build_serially(self) -> None:
        benchmark_sources = [
            BENCH_LOAD_SOURCE_PATH,
            BENCH_DISPATCH_SOURCE_PATH,
            BENCH_ROUNDTRIP_SOURCE_PATH,
        ]

        self.assertTrue(HOT_UPDATE_HOST_PROJECT_PATH.is_file(), msg=f"missing canonical host project: {HOT_UPDATE_HOST_PROJECT_PATH}")

        for source_path in benchmark_sources:
            self.assertTrue(source_path.is_file(), msg=f"missing benchmark source: {source_path}")
            source_text = source_path.read_text(encoding="utf-8")
            self.assertIn("[ChaosBenchmark(", source_text)
            self.assertIn("RunWorkload", source_text)
            self.assertNotIn("Stopwatch", source_text)
            self.assertNotIn("elapsedMilliseconds", source_text)
            self.assertNotIn("opsPerSecond", source_text)
            self.assertNotIn("roundtripsPerSecond", source_text)
            self.assertNotIn("loadDurationMs", source_text)
            self.assertNotIn("Console.WriteLine", source_text)
