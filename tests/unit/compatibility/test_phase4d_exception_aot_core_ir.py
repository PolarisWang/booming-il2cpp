...
ENTRY_SUBJECT_ID = "CoreRuntimeFeatures/ExceptionProofEntry::Run()"
CAPTURE_SUBJECT_ID = "CoreRuntimeFeatures/ExceptionThrower::Capture()"
TEST_OUTPUT_ROOT = REPO_ROOT / "artifacts" / ".tmp-tests" / "phase4d-exception-aot-core-ir"
...
class Phase4DExceptionAotCoreIrTests(unittest.TestCase):
    @classmethod
    ...
    def test_aot_core_ir_carries_typed_exception_regions_for_retained_exception_proof(self) -> None:
        ...
        catch_region = capture_method["exceptionRegions"][0]
        self.assertEqual(1, catch_region["handlingKindCode"])
        self.assertEqual(
            "System.Private.CoreLib/System.InvalidOperationException",
            catch_region["catchTypeSubjectId"],
        )
        self.assertGreater(catch_region["handlerOffset"], catch_region["tryOffset"])
        self.assertGreater(catch_region["handlerLength"], 0)
