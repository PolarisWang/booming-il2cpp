...
OUTPUT_ROOT = REPO_ROOT / "artifacts" / ".tmp-tests" / "monitor-locking-native-aot"
...
class MonitorLockingNativeAotTests(unittest.TestCase):
    @classmethod
    ...
    def test_emit_native_aot_materializes_static_gate_initializer_before_monitor_access(self) -> None:
        ...
        self.assertIn(f"if ({STATIC_GATE_SYMBOL} == static_cast<CHAOS_IL2CPP_INTPTR>(0))", generated_cpp)
        self.assertIn(f"{STATIC_GATE_SYMBOL} = reinterpret_cast<CHAOS_IL2CPP_INTPTR>(", generated_cpp)
