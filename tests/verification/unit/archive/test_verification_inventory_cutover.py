...
def test_write_inventory_outputs_also_materializes_verification_formal_source() -> None:
    ...
    try:
        ...
        assert (repo_root / "verification" / "archive" / "latest" / "result-snapshot.json").is_file()
        assert (
            repo_root / "verification" / "archive" / "latest" / "evidence-claims-snapshot.json"
        ).is_file()
        assert (repo_root / "verification" / "archive" / "master" / "result-master.json").is_file()
        assert (
            repo_root / "verification" / "archive" / "master" / "evidence-claims-master.json"
        ).is_file()
        assert (
            repo_root
            / "verification"
        ...
        assert "verification/archive/latest/result-snapshot.json" in payload["artifacts"]
        assert "verification" in payload
        assert payload["verification"]["outputRoot"] == "verification/archive"
        assert payload["validated"] is True
        assert payload["verificationData"]["latestResultPath"] == "verification/archive/latest/result-snapshot.json"
        assert payload["verificationData"]["masterResultPath"] == "verification/archive/master/result-master.json"
...
def test_write_inventory_outputs_projects_capability_table_from_merged_formal_master() -> None:
    ...
    try:
        ...
        capability_payload = json.loads(
            (repo_root / "verification" / "projections" / "testing-inventory" / "capability-inventory.json").read_text(encoding="utf-8")
        )
...
def test_write_inventory_outputs_renders_fixed_homepage_views_from_formal_master() -> None:
    ...
    try:
        ...
        html_document = (repo_root / "verification" / "projections" / "testing-inventory" / "inventory.html").read_text(encoding="utf-8")
...
def test_validate_inventory_outputs_fails_when_required_formal_artifact_is_missing() -> None:
    ...
    try:
        (repo_root / "verification" / "archive" / "master").mkdir(parents=True, exist_ok=True)
        (repo_root / "verification" / "archive" / "reports" / "completed" / "testing-inventory").mkdir(
            parents=True,
            exist_ok=True,
        )
        (repo_root / "verification" / "archive" / "master" / "result-master.json").write_text(
            "{}",
            encoding="utf-8",
        )
        (repo_root / "verification" / "archive" / "reports" / "completed" / "testing-inventory" / "summary.md").write_text(
            "# summary\n",
            encoding="utf-8",
        )
        ...
        try:
            generator_module.validate_inventory_outputs(
                repo_root,
                {
                    ...
                    "verification": {
                        "outputRoot": "verification/archive",
                    },
                },
            )
            raise AssertionError("validate_inventory_outputs should fail when latest result is missing")
        except RuntimeError as error:
            assert "verification/archive/latest/result-snapshot.json" in str(error)
    finally:
        cleanup_repo_root(repo_root)
...
def test_validate_inventory_outputs_collects_codegen_stub_paths_when_required() -> None:
    ...
    try:
        (repo_root / "verification" / "archive" / "latest").mkdir(parents=True, exist_ok=True)
        (repo_root / "verification" / "archive" / "master").mkdir(parents=True, exist_ok=True)
        (repo_root / "verification" / "archive" / "reports" / "completed" / "testing-inventory").mkdir(
            parents=True,
            exist_ok=True,
        ...
        (repo_root / "verification" / "archive" / "latest" / "result-snapshot.json").write_text(
            "{}",
            encoding="utf-8",
        )
        (repo_root / "verification" / "archive" / "master" / "result-master.json").write_text(
        ...
        (
            repo_root / "verification" / "archive" / "reports" / "completed" / "testing-inventory" / "summary.md"
        ).write_text(
            "# summary\n",
            encoding="utf-8",
        )
