...
int main() {
    std::printf("================================================================\n");
    std::printf("  Native blittable struct marshalling benchmarks\n");
    std::printf("  Platform: Windows x64, Release build\n");
    std::printf("  Struct: 64 bytes (16 x Int32 fields)\n");
    std::printf("  %d samples per benchmark, %d warmup iterations\n", kSamples, kWarmup);
    std::printf("================================================================\n\n");
    ...
    std::printf("\n  Managed→Native:\n");
    std::printf("    Fast path:         %8.1f ns/op  (%10.0f ops/sec)\n", fast_mn_ns, 1e9 / fast_mn_ns);
    std::printf("    Slow path:         %8.1f ns/op  (%10.0f ops/sec)\n", slow_mn_ns, 1e9 / slow_mn_ns);
    std::printf("    Mixed path:        %8.1f ns/op  (%10.0f ops/sec)\n", mixed_mn_ns, 1e9 / mixed_mn_ns);
    std::printf("    Fast/slow speedup: %.1fx\n", slow_mn_ns / fast_mn_ns);
    std::printf("    Fast/mixed speedup: %.1fx\n", mixed_mn_ns / fast_mn_ns);

    std::printf("\n  Native→Managed:\n");
    std::printf("    Fast path:         %8.1f ns/op  (%10.0f ops/sec)\n", fast_nm_ns, 1e9 / fast_nm_ns);
    std::printf("    Slow path:         %8.1f ns/op  (%10.0f ops/sec)\n", slow_nm_ns, 1e9 / slow_nm_ns);
    std::printf("    Mixed path:        %8.1f ns/op  (%10.0f ops/sec)\n", mixed_nm_ns, 1e9 / mixed_nm_ns);
    std::printf("    Fast/slow speedup: %.1fx\n", slow_nm_ns / fast_nm_ns);
    std::printf("    Fast/mixed speedup: %.1fx\n", mixed_nm_ns / fast_nm_ns);

    std::printf("\n\n  Interpretation:\n");
    std::printf("    Fast speedup (all-blittable struct): %.1fx — pure memcpy vs field dispatch overhead\n",
                slow_mn_ns / fast_mn_ns);
    std::printf("    Mixed slowdown (blittable+bool+string): %.1fx — dispatch + conversion overhead\n",
                mixed_mn_ns / fast_mn_ns);
    std::printf("    Blittable fast-path eliminates: branch mispredictions from switch(field.kind)\n");
    std::printf("    + per-field bounds checks + indirect call through field dispatch.\n");
