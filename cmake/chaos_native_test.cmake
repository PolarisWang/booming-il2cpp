...
function(chaos_native_add_test name)
    ...
    if(ARG_LABELS)
        ...
        # Default every test to the "unit" tier so ctest -LE (benchmark|stress|soak)
        # always has a label to filter on — prevents slow untagged tests leaking
        # through -LE and hitting the --timeout as false failures.
...
function(chaos_native_glob_add_tests prefix pattern)
    ...
    foreach(_src IN LISTS _srcs)
        ...
        endforeach()
        if(NOT _skip)
            # Derive a LABEL from the source name so ctest -LE (benchmark|stress|soak)
            # can exclude the heavy tiers even when a call site doesn't set LABELS.
            # "stress"/"benchmark"/"soak" may appear as a middle segment (gc_stress_test)
            # or suffix (tiering_benchmark); unit is the default otherwise.
            set(_labels "unit")
            if(_name_we MATCHES "(benchmark|bench|_bench)")
                set(_labels "benchmark")
            elseif(_name_we MATCHES "(stress|soak)")
                set(_labels "stress")
            endif()
            set(_lock)
            if(A_LOCK_SOURCES)
            ...
            if(_lock)
                chaos_native_add_test(${prefix}_${_name_we} ${_src}
                    RESOURCE_LOCK ${_lock} LABELS ${_labels})
            else()
                chaos_native_add_test(${prefix}_${_name_we} ${_src} LABELS ${_labels})
            endif()
        endif()
    endforeach()
endfunction()
