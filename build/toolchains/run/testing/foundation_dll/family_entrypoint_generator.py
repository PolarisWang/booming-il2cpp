...
def generate_entrypoint_source(
    ...
    *,
    ...
    # For subjects variant, dispatch is handled by ChaosDispatchMethod which calls
...
def compute_entry_point_subject_id(
    ...
    *,
    ...
    if variant == "subjects":
        ...
        # are void(void) and dispatch is handled by ChaosDispatchMethod via s_hotpatch_entries.
...
def generate_runtime_entry(*, is_jit: bool = False) -> str:
    ...
    Uses ChaosDispatchMethod (hotpatch_dispatch.h) for all method dispatch,
    with extern declarations from verification_dispatch.generated.cpp and
    ...
    # Runtime initialization block (AOT mode): initializes ChaosRuntimeHost, registers
    # the codegen module, disables BGC for short-lived verification processes, and
    # redirects diagnostic logs to stderr to keep stdout clean for machine-consumed output.
    runtime_init_decl = (
        '\n'
        '// ── g_log_use_stderr (declared extern in <chaos/log.h>) ──────────\n'
        ...
        '#if defined(_WIN32)\n'
        '#include <windows.h>\n'
        '// VectoredExceptionHandler: captures crash diagnostics for debugging.\n'
        'static LONG CALLBACK JitVehHandler(PEXCEPTION_POINTERS ExceptionInfo) noexcept {\n'
        '    auto* ctx = ExceptionInfo->ContextRecord;\n'
        '    std::fprintf(stderr,\n'
        '        "JIT CRASH: Code=0x%08lX RIP=0x%p RSP=0x%p"\n'
    ...
    runtime_init_call = (
        ...
        '#endif\n'
        '\n'
        '    // Redirect diagnostic log output to stderr\n'
        '    chaos::il2cpp::common::log_internal::g_log_use_stderr = true;\n'
        '\n'
        '    // Disable BGC for this short-lived verification process\n'
        '    chaos::il2cpp::runtime_core::g_bgc_enabled = false;\n'
        '\n'
        '    // Initialize ChaOS runtime and register the AOT module\n'
    ...
    # Use a sentinel marker for injection since f-string with {{}} escaping is
    # impractical for the large C++ template body.
    template = '''// runtime-entry.cpp — Auto-generated by family_entrypoint_generator.py
...
'''
    return template.replace('__JIT_DECL__', jit_decl).replace('__JIT_CALL__', jit_call).replace('__RUNTIME_INIT_DECL__', runtime_init_decl).replace('__RUNTIME_INIT_CALL__', runtime_init_call)
...
def main() -> None:
    ...
    if args.output_dir:
        ...
        family_slug = slug_from_family_id(args.family_id)
        output_dir = repo_root / "testing" / "foundation-dll" / args.assembly_name / family_slug / "il2cpp_dist" / "entrypoint"
