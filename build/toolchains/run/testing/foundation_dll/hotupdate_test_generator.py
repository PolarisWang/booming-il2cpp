...
def nsslug_from_family_id(family_id: str) -> str:
    return slug_from_family_id(family_id).replace("-", "_")


def class_name(family_slug: str, variant: str) -> str:
...
def find_genuine_cpp(family_slug: str) -> Path | None:
...
def has_new_format_typeinfo(content: str) -> bool:
...
def find_genuine_cpp_for_fix(family_slug: str) -> Path | None:
    ...
    if candidate.exists():
        ...
        if 'extern "C" CHAOS_IL2CPP_INT32 RunNativeAot' in content:
            if has_new_format_typeinfo(content):
                return candidate
...
def discover_host_symbols(family_slug: str) -> list[str]:
    """Scan the genuine (host) generated C++ to discover MethodN symbols.

    Returns host_symbols list.
    """
    host_cpp = find_genuine_cpp(family_slug)
    if host_cpp is None:
        return []
    return extract_method_symbols(host_cpp)


def extract_method_symbols(cpp_path: Path) -> list[str]:
...
def load_contract(family_slug: str) -> list[str]:
...
def detect_method_return_type(family_slug: str) -> str:
...
def detect_aborting_methods(family_slug: str, host_symbols: list[str]) -> set[int]:
...
def generate_hotupdate_test(
    ...
    ns_slug = _nsslug_from_family_id(family_id)
    method_count = len(host_symbols)

    # Embed .patchdata for D3 dispatch hotpatch
    host_class_name = class_name(family_slug, "host")
    patch_data_cxx, _ = embed_patch_data(family_slug)

    # Detect whether codegen uses 'void' or 'CHAOS_IL2CPP_INT32' return type
    return_type = detect_method_return_type(family_slug)

    # Detect methods that abort during baseline capture (call null-returning stub → abort)
    aborting_indices = detect_aborting_methods(family_slug, host_symbols)
    ...
    emit_header(lines, family_id, method_count)
    if patch_data_cxx:
        lines.append(patch_data_cxx)
    emit_forward_decls(lines, host_symbols, return_type)
    emit_namespace_block(lines, host_symbols, method_count, return_type)
    emit_main_function(lines, method_count, family_id, bool(patch_data_cxx), host_class_name, aborting_indices, return_type)

    return "\n".join(lines)


def embed_patch_data(family_slug: str) -> tuple[str, int]:
...
def emit_fix_method_names_fn(lines: list[str]) -> None:
...
def emit_main_function(lines: list[str], method_count: int, family_id: str,
                         has_patch_data: bool,
                         host_class_name: str,
    ...
    if has_patch_data:
        emit_fix_method_names_fn(lines)
...
def generate_hotupdate_cmake_full(family_slug: str) -> str:
    ...
    mismatches.
    """
    ns_slug = _nsslug_from_family_id(family_id=f"family/System.Private.CoreLib/{family_slug.replace('-', '/')}")
...
def generate_family(family_slug: str) -> dict[str, Any]:
    ...
    # Discover symbols
    host_symbols = discover_host_symbols(family_slug)
    if not host_symbols:
    ...
    method_subject_ids = load_contract(family_slug)
    print(f"  Contract methods: {len(method_subject_ids)}")

    # Generate test
    test_source = generate_hotupdate_test(
    ...
    ns_slug = _nsslug_from_family_id(f"family/System.Private.CoreLib/{family_slug.replace('-', '/')}")
    remediate_genuine_for_hotupdate_keep_extern(family_slug, ns_slug)

    # Append Hotpatch target to existing CMakeLists.txt
    # (preserves benchmark static library target already in the file).
    cmake_hotupdate_section = generate_hotupdate_cmake_full(family_slug)
...
def remediate_genuine_for_hotupdate_keep_extern(family_slug: str, ns_slug: str) -> Path | None:
    ...
    """
    src = find_genuine_cpp_for_fix(family_slug)
    if src is None:
        # Fallback: try NativeEntry subdirectory
        src = find_genuine_cpp(family_slug)
        if src is None:
            return None

    dst = _VERIFICATION / family_slug / "il2cpp_dist" / "hotupdate" / "genuine-fixed" / "native-aot.generated.cpp"
    rename_and_fix_patch_file(src, dst, ns_slug, suffix="genuine_fixed")
    return dst


def inject_missing_structs_and_type_ids(content: str) -> str:
...
def generate_oldformat_stub_definitions(content: str) -> str:
...
def inject_reflection_bridge_stubs(content: str) -> str:
    ...
    content = generate_oldformat_stub_definitions(content)
...
def inject_interpreter_entry_include(content: str) -> str:
...
def inject_string_materialize_stub(content: str) -> str:
...
def rename_and_fix_patch_file(src: Path, dst: Path, ns_slug: str, suffix: str = "patch") -> None:
    ...
    if is_old_format or is_hybrid_format:
        ...
        content = inject_missing_structs_and_type_ids(content)
    ...
    content = inject_reflection_bridge_stubs(content)
    ...
    content = remediate_inline_param_mismatch(content)
    ...
    if is_new_format:
        ...
        content = move_common_include_outside_namespace(content)
    ...
    if is_hybrid_format:
        ...
        content = remediate_hybrid_common_include(content)
...
def inject_missing_eval_stack_decls(content: str) -> str:
...
def remediate_inline_param_mismatch(content: str) -> str:
    """Fix parameter name mismatch in extern "C" inline function bodies.
...
def inject_host_symbol_forward_decls(content: str) -> str:
...
def move_common_include_outside_namespace(content: str) -> str:
...
def remediate_hybrid_common_include(content: str) -> str:
...
def remediate_newformat_typeinfo_initlist(content: str) -> str:
...
def remediate_closing_brace_in_comment(content: str) -> str:
...
def remediate_raw_int32_normalize_misuse(content: str) -> str:
...
def remediate_todo_struct_fields(content: str) -> str:
