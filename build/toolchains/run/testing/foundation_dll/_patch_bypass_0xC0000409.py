...
3. Generates kAotDirectFnTable for AotDirectDispatch (direct Call fast path)
...
# ── AotDirectDispatch helpers ─────────────────────────────────────────────
# Naming convention: "System.Private.CoreLib/System.Convert::ToChar:System.Char(System.Byte)"
...
def _extract_direct_fn_entries(content: str, aot_core_ir_path: Path) -> list[tuple[str, str]]:
    """Extract (subjectId, function_name) pairs from aot-core-ir.json.

    Only includes Call instructions with dispatchKindCode == 4 (external runtime dispatch)
    whose function names are verified to exist in the generated C++ content.
    """
    if not aot_core_ir_path.exists():
        return []

    with open(aot_core_ir_path, 'r', encoding='utf-8') as f:
        data = json.load(f)

    raw_entries: list[tuple[str, str]] = []
    for method in data.get('methods', []):
        for instr in method.get('instructions', []):
            if instr.get('opCode') != 14:
                continue
            dispatch_kind = instr.get('dispatchKindCode', 0)
            if dispatch_kind not in (4, 5):   # 4=external_runtime, 5=external_runtime_inline
                continue
            # Prefer targetReference.subjectId over callee
            target_ref = instr.get('targetReference', {})
            subject_id = target_ref.get('subjectId', '') or instr.get('callee', '')
            if not subject_id:
                continue
            fn_name = _subject_id_to_fn_name(subject_id)
            # Verify the function is actually declared in this compilation unit
            if f' {fn_name}(' in content:
                raw_entries.append((subject_id, fn_name))

    # Deduplicate by subject_id, preserving order
    seen: set[str] = set()
    unique: list[tuple[str, str]] = []
    for sid, fn in raw_entries:
        if sid not in seen:
            seen.add(sid)
            unique.append((sid, fn))

    if unique:
        print(f"  AotDirectDispatch: {len(unique)} external_runtime entries found")
    else:
        print("  AotDirectDispatch: no external_runtime entries found")
    return unique


def _emit_direct_fn_table(content: str, entries: list[tuple[str, str]], pragma_pos: int) -> str:
    """Inject kAotDirectFnTable/kAotDirectFnSubjectIds/kAotDirectFnCount at pragma_pos."""
    if not entries:
        return content
    ...
    table_lines = [
        ...
        '// Maps subjectId → chaos_external_runtime_* function pointer for direct\n',
        '// invocation from FastExecute Handle_Call (skips ~2200ns method_invoke).\n',
        '// Generated by _patch_bypass_0xC0000409.py\n',
        'extern "C" const char* const kAotDirectFnSubjectIds[] = {\n',
    ]
    for sid, _ in entries:
        table_lines.append(f'    "{sid}",\n')
    table_lines.append('};\n')
    table_lines.append('\n')
    table_lines.append('extern "C" const void* const kAotDirectFnTable[] = {\n')
    for _, fn in entries:
        table_lines.append(f'    reinterpret_cast<void*>(&{fn}),\n')
    table_lines.append('};\n')
    table_lines.append(f'\nextern "C" const int kAotDirectFnCount = {len(entries)};\n')
    table_lines.append('\n')

    table_text = ''.join(table_lines)
    return content[:pragma_pos] + table_text + content[pragma_pos:]
...
extern \"C\" double BenchmarkMethod(int chaos_entry_index, int iterations) {{
    ...
    # ── Always: Generate kAotDirectFnTable (AotDirectDispatch) ───────────
    # Load aot-core-ir.json from the parent of the generated directory.
    # The .cpp is at: <genuine>/<ClassName>/generated/native-aot.generated.cpp
    # The .json is at: <genuine>/aot-core-ir.json
    aot_core_ir_path = filepath.parent.parent.parent / "aot-core-ir.json"
    entries = _extract_direct_fn_entries(content, aot_core_ir_path)
    if entries:
        # Inject INSIDE the namespace (before `}  // namespace`) so that
        # the static chaos_external_runtime_* functions are visible.
        ns_close = content.rfind('\n}  // namespace ')
        if ns_close == -1:
            ns_close = pragma_pop
        content = _emit_direct_fn_table(content, entries, ns_close + 1)
