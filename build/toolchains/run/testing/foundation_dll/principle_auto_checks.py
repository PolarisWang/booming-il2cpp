...
def check_p2_no_bare_new_in_generated(assembly: str, family_slug: str) -> PrincipleCheckResult:
    """P2: Generated C++ uses CHAOS_IL2CPP_NEW_GC instead of bare `new Type{}`.

    Managed object allocations must go through the GC heap via CHAOS_IL2CPP_NEW_GC.
    Bare `new Type{}` uses malloc, which BDWGC cannot track → use-after-free risk.
    """
    cpp_path = _generated_cpp(assembly, family_slug)
    if cpp_path is None:
        return PrincipleCheckResult(
            check_id="p2_no_bare_new", principle="P2",
            status="NOT_APPLICABLE",
            summary="Generated C++ not found",
        )
    ...
    # Match bare `new TypeName{` or `new TypeName(` that allocates managed objects.
    # Exclude known-legal patterns:
    #   - CHAOS_IL2CPP_NEW_GC       → already correct
    #   - new char[                 → Raw domain temp buffer
    #   - new CHAOS_IL2CPP_         → macro already used
    #   - new TypeInfoHot           → Raw domain module metadata
    ...
    for m in bare_new_re.finditer(cpp):
        line = cpp[:m.start()].rfind('\n')
        line_no = cpp[:m.start()].count('\n') + 1
        start_of_line = cpp.rfind('\n', 0, m.start()) + 1
        line_text = cpp[start_of_line:cpp.find('\n', m.start())].strip()

        # Check if this is a legal pattern
        full_match = m.group(0)
        if any(full_match.startswith(p) for p in legal_prefixes):
            continue
        # Also skip known raw-domain patterns
        type_name = m.group(1)
    ...
    if violations:
        return PrincipleCheckResult(
            check_id="p2_no_bare_new", principle="P2",
            status="VIOLATION",
            summary=f"{len(violations)} bare `new` in generated C++ (should use CHAOS_IL2CPP_NEW_GC)",
            details=violations,
...
def run_all_checks(assembly: str, family_slug: str) -> dict[str, Any]:
    ...
    checks["p2_no_bare_new"] = check_p2_no_bare_new_in_generated(assembly, family_slug)
