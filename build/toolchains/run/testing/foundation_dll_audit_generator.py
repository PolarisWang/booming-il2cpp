...
def _slug_to_class_name(slug: str, suffix: str = "") -> str:
    """Convert 'convert-char' -> 'ConvertChar' + optional suffix."""
    return "".join(part.capitalize() for part in slug.split("-")) + suffix


def _build_source_paths(assembly_name: str, slug: str) -> dict[str, str]:
    """Build source file paths for a capability family."""
    cls = _slug_to_class_name(slug)
    base = f"verification/foundation-dll/{assembly_name}/{slug}"
    return {
        "managed_test": f"{base}/test/{cls}Tests.AutoGenerated.cs",
        "native_genuine": f"{base}/native/genuine/generated/native-aot.generated.cpp",
        "managed_benchmark": f"{base}/benchmark/{cls}Benchmarks.AutoGenerated.cs",
        "native_benchmark": f"{base}/native/BenchmarkNativeEntry.cpp",
        "managed_host": f"{base}/host/{cls}HotUpdate.AutoGenerated.cs",
        "managed_patch": f"{base}/patch/{cls}Patch.AutoGenerated.cs",
        "native_hotupdate": f"{base}/native/hotupdate/HotUpdateTest.cpp",
    }
...
.primary-header { background: #c8b89a !important; font-weight: 800; }
.family-table td.primary-cell { background: #f5efe0; font-weight: 600; }
.primary-cell-link { display: block; color: var(--ink); text-decoration: none; }
.primary-cell-link:hover { color: var(--accent); text-decoration: underline; }
/* family detail page */
.family-detail-table { width: 100%; border-collapse: collapse; margin-top: 8px; }
.family-detail-table th,
.family-detail-table td { padding: 6px 10px; border: 1px solid var(--line); font-size: 13px; }
.family-detail-table th { background: var(--bg-strong); font-weight: 700; }
.family-detail-table th:first-child,
.family-detail-table td:first-child { text-align: left; }
...
ALL_GATE_COLUMNS = tuple(ALL_GATE_LABELS.keys())
...
def _render_benchmark_speedup_cell(
    ...
    *,
    ...
    link_to_family_page: bool = False,
) -> str:
    ...
    When link_to_family_page is True, links to the per-family detail page instead.
    """
    ...
    if link_to_family_page:
        detail_href = f"{escape(assembly_name)}/families/{escape(slug)}-benchmark.html"
    else:
...
def _render_dll_detail_page(dll: dict[str, Any], *, repo_root: Path, root_prefix: str) -> str:
    ...
    # Benchmark detail removed — now in per-family benchmark pages
...
def _render_fact_detail_page(family: dict[str, Any], *, assembly_name: str, repo_root: Path, root_prefix: str) -> str:
    """Render a standalone per-family Fact detail page with inline code snippets."""
    ...
    native_proof = dict(family.get("nativeProof") or {})
    method_details = list(native_proof.get("methodDetails") or [])
    np_denominator = int(native_proof.get("denominator") or 0)
    np_numerator = int(native_proof.get("numerator") or 0)
    source_paths = _build_source_paths(assembly_name, slug)

    if method_details:
        fact_rows = ""
        for item in method_details:
            sid = _string(item.get("subjectId") or "")
            covered = item.get("covered", False)
            short_name = _short_method_subject_id(sid)
            status_icon = '&#9989; covered' if covered else '&#10060; uncovered'
            # Extract code snippets
            managed_code = _extract_method_code_snippet(repo_root, source_paths["managed_test"], sid)
            native_code = _extract_method_code_snippet(repo_root, source_paths["native_genuine"], sid)
            managed_block = _render_code_block(managed_code, source_paths["managed_test"].split("/")[-1], "C#")
            native_block = _render_code_block(native_code, source_paths["native_genuine"].split("/")[-1], "C++")
            # Failure reason (if uncovered, check for detail)
            failure_reason = ""
            if not covered:
                detail = _string(item.get("detail") or item.get("failureReason") or "")
                if detail:
    ...
    evidence = list(native_proof.get("evidence") or [])
    evidence_links = "".join(
        f'<a href="{escape(root_prefix + _normalized(_string(item.get("path"))), quote=True)}" target="_blank" rel="noreferrer" class="source-link">{escape(_string(item.get("label")) or _string(item.get("path")))}</a>'
        for item in evidence[:8]
    ) or '<span class="status-muted">n/a</span>'

    back_link = f"../{escape(assembly_name)}.html"
    return f"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>{escape(dname)} - Fact Detail - {escape(assembly_name)} - Foundation DLL Audit</title>
  <link rel="stylesheet" href="{escape(root_prefix)}dashboard.css">
</head>
<body>
  <main>
    <div class="page-header">
      <a class="back-link" href="{escape(back_link, quote=True)}">Back to {escape(assembly_name)}</a>
      <div class="eyebrow">Fact Detail — Native Proof Coverage</div>
      <h1>{escape(dname)}</h1>
      <div class="summary-grid">
        ...
        <div class="summary-card"><strong>Fact Pass Rate</strong>{np_numerator}/{np_denominator}</div>
      </div>
    </div>

    <section class="detail-section">
      <h2>Method Coverage Details</h2>
      <p>Each method must pass its native proof assertions to be marked as covered. Expand code blocks to inspect the managed test and native generated implementation.</p>
      <div class="table-wrap">
        <table class="family-detail-table">
          <thead><tr><th>Method</th><th>Status</th><th>Managed Test Code</th><th>Native Generated Code</th><th>Failure Reason</th></tr></thead>
...
def _render_benchmark_detail_page(family: dict[str, Any], *, assembly_name: str, repo_root: Path, root_prefix: str) -> str:
    """Render a standalone per-family Benchmark detail page with inline code snippets."""
    dname = _string(family.get("displayName") or "")
    family_id = _string(family.get("familyId") or "")
    slug = _family_slug(family_id)
    status = _string(family.get("closureStatus"))
    ...
    if method_results:
        bm_rows = ""
        for r in method_results:
            sid = _string(r.get("methodSubjectId", ""))
            short_sid = _short_method_subject_id(sid)
            status_b = _status_badge(r.get("status"))
            managed_ms = r.get("managedElapsedMs")
            native_ms = r.get("nativeElapsedMs")
            speedup = r.get("speedupPercent")
            managed_str = f"{managed_ms:.4f}" if isinstance(managed_ms, (int, float)) else "n/a"
    ...
    evidence = list(benchmark_proof.get("evidence") or [])
    evidence_links = "".join(
        f'<a href="{escape(root_prefix + _normalized(_string(item.get("path"))), quote=True)}" target="_blank" rel="noreferrer" class="source-link">{escape(_string(item.get("label")) or _string(item.get("path")))}</a>'
        for item in evidence[:8]
    ) or '<span class="status-muted">n/a</span>'
    ...
    back_link = f"../{escape(assembly_name)}.html"
    summary_line = f"Average Speedup: {avg_speedup_fmt} (Native faster: {native_faster}, Managed faster: {managed_faster}, Total methods: {method_count})"
    return f"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>{escape(dname)} - Benchmark Detail - {escape(assembly_name)} - Foundation DLL Audit</title>
  <link rel="stylesheet" href="{escape(root_prefix)}dashboard.css">
</head>
<body>
  <main>
    <div class="page-header">
      <a class="back-link" href="{escape(back_link, quote=True)}">Back to {escape(assembly_name)}</a>
      <div class="eyebrow">Benchmark Detail — Performance Comparison</div>
      <h1>{escape(dname)}</h1>
      <div class="summary-grid">
        <div class="summary-card"><strong>Closure Status</strong>{_status_badge(status)}</div>
        <div class="summary-card"><strong>Methods</strong>{method_count}</div>
        <div class="summary-card"><strong>Avg Speedup</strong>{avg_speedup_fmt}</div>
        <div class="summary-card"><strong>Native Faster</strong>{native_faster}</div>
        <div class="summary-card"><strong>Managed Faster</strong>{managed_faster}</div>
      </div>
    ...
    <section class="detail-section">
      ...
      <p>Managed vs Native execution time comparison per method. Expand code blocks to inspect the benchmark source code.</p>
      <div class="table-wrap">
        <table class="family-detail-table">
          <thead><tr><th>Method</th><th>Managed (ms)</th><th>Native (ms)</th><th>Speedup %</th><th>Status</th><th>Managed Benchmark Code</th><th>Native Benchmark Code</th></tr></thead>
          <tbody>{bm_rows}</tbody>
        </table>
      </div>
    </section>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>{escape(dname)} - HotUpdate Detail - {escape(assembly_name)} - Foundation DLL Audit</title>
  <link rel="stylesheet" href="{escape(root_prefix)}dashboard.css">
</head>
<body>
  <main>
...
"""
    ...
    for gate_code in ALL_GATE_COLUMNS:
        gate_label = ALL_GATE_LABELS.get(gate_code, gate_code)
        gate_progress_data[gate_label] = {"passed": 0, "total": 0, "blocked": 0, "pending": 0}
    for dll in list(payload.get("dlls") or []):
        for family in list(dll.get("capabilityFamilies") or []):
            gates = dict(family.get("verificationGates") or {})
            for gate_code in ALL_GATE_COLUMNS:
                gl = ALL_GATE_LABELS.get(gate_code, gate_code)
...
def _write_projection_bundle(repo_root: Path, payload: dict[str, Any], *, output_root: Path, has_ledger: bool = False) -> list[str]:
    ...
    for dll in list(payload.get("dlls") or []):
        ...
        # Write per-family detail pages (3 pages per family: Fact, Benchmark, HotUpdate)
        assembly_name = _string(dll.get("assemblyName"))
        families = list(dll.get("capabilityFamilies") or [])
        for family in families:
            slug = _family_slug(_string(family.get("familyId") or ""))
            if not slug:
                continue
            family_dir = dll_root / assembly_name / "families"
            family_dir.mkdir(parents=True, exist_ok=True)
            root_pref = _root_relative_prefix(repo_root, family_dir)

            # Fact detail page
            fact_html_path = family_dir / f"{slug}-fact.html"
            fact_html_path.write_text(
                _render_fact_detail_page(
                    family,
                    assembly_name=assembly_name,
                    repo_root=repo_root,
                    root_prefix=root_pref,
                ),
                encoding="utf-8",
            )
            artifacts.append(_relative(repo_root, fact_html_path))

            # Benchmark detail page
            bm_html_path = family_dir / f"{slug}-benchmark.html"
            bm_html_path.write_text(
                _render_benchmark_detail_page(
                    family,
                    assembly_name=assembly_name,
                    repo_root=repo_root,
                    root_prefix=root_pref,
            ...
            artifacts.append(_relative(repo_root, bm_html_path))

            # HotUpdate detail page
            hu_html_path = family_dir / f"{slug}-hotupdate.html"
            hu_html_path.write_text(
                _render_hotupdate_detail_page(
                    family,
                    assembly_name=assembly_name,
                    repo_root=repo_root,
                    root_prefix=root_pref,
            ...
            artifacts.append(_relative(repo_root, hu_html_path))
    return artifacts
