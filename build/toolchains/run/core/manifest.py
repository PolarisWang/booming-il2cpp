...
def _load_manifest_from_shards(repo_root: Path) -> dict[str, Any] | None:
    shard_dir = repo_root / DEFAULT_MANIFEST_SHARD_DIR
    ...
    for shard_path in sorted(shard_dir.glob("commands.*.json")):
        shard = read_json(shard_path)
        commands.extend(shard.get("commands", []))
...
def load_run_manifest(repo_root: Path, manifest_path: str | Path | None = None) -> dict[str, Any]:
    if manifest_path is None:
        sharded = _load_manifest_from_shards(repo_root)
