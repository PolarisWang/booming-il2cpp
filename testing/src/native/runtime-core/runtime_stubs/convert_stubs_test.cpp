...
// ═══════════════════════════════════════════════════════════════════════════
// exception_jmp depth-boundary tests (SETJMP EH mode)
// ═══════════════════════════════════════════════════════════════════════════
//
// These tests verify push_exception_jmp_buf/pop_exception_jmp_buf depth
// boundaries at kMaxNestedTry.  Compile with CHAOS_IL2CPP_EH_SETJMP to
// enable the push/pop functions (WIN32_SEH and CPP_THROW modes don't have
// this mechanism).
//
// WARNING: The test .cpp is compiled WITH and WITHOUT CHAOS_IL2CPP_EH_SETJMP
// (two separate test targets: convert_stubs_test + convert_stubs_eh_test).
// The SETJMP-compiled target runs these tests; the non-SETJMP target
// compiles them as a no-op block.
...
// Provide TLS definitions for the jmp_buf stack (normally in exception_helpers.cpp).
...
/// Helper: push N times then pop N times, expect no crash.
static int push_pop_n(int n) noexcept {
    using namespace chaos::il2cpp::runtime_core;
    for (int i = 0; i < n; i++) push_exception_jmp_buf();
    int d = g_chaos_exception_jmp_depth;
    for (int i = 0; i < n; i++) pop_exception_jmp_buf();
    return d;
}
...
TEST(ExceptionJmpDepth, DepthHalfLimit) {
    using namespace chaos::il2cpp::runtime_core;
    ASSERT_EQ(g_chaos_exception_jmp_depth, 0);
    EXPECT_EQ(push_pop_n(kMaxNestedTry / 2), kMaxNestedTry / 2);
    ASSERT_EQ(g_chaos_exception_jmp_depth, 0);
}
...
TEST(ExceptionJmpDepth, DepthOverflow) {
    using namespace chaos::il2cpp::runtime_core;
    for (int i = 0; i < kMaxNestedTry; i++) push_exception_jmp_buf();
    EXPECT_EQ(g_chaos_exception_jmp_depth, kMaxNestedTry);
    EXPECT_DEATH(push_exception_jmp_buf(), ".*");
}
