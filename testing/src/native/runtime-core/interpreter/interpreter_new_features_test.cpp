/// interpreter_new_features_test.cpp — New InterpreterVM feature tests (gtest)
///
/// Tests: float branch ops (Blt/Bgt/Ble/Bge), LdArgA/LdLocA ref semantics,
///        interface CastClass/IsInst, interface vtable dispatch.
...
TEST(Interpreter_NewFeatures, FloatBlt) {
    IRMethod method;
...
TEST(Interpreter_NewFeatures, FloatBgt) {
    IRMethod method;
...
TEST(Interpreter_NewFeatures, FloatBle) {
    IRMethod method;
...
TEST(Interpreter_NewFeatures, FloatBge) {
    IRMethod method;
...
// ════════════════════════════════════════════════════════════════════════════
// LdArgA/LdLocA ref semantics tests
// ════════════════════════════════════════════════════════════════════════════

TEST(Interpreter_NewFeatures, LdArgA_RefSemantics) {
    IRMethod method;
...
TEST(Interpreter_NewFeatures, LdLocA_RefSemantics) {
    IRMethod method;
    ...
    ExecutionResult result = vm.Execute(method, &frame);
    EXPECT_TRUE(result.has_return_value);
    EXPECT_EQ(result.int32_value, 20);
}
...
TEST(Interpreter_NewFeatures, InterfaceCastClass) {
    ...
    ExecutionResult result = vm.Execute(method, &frame);
    EXPECT_TRUE(result.has_return_value);
    EXPECT_EQ(result.return_value.tag, ValueTag::ObjectRef);
}

TEST(Interpreter_NewFeatures, InterfaceIsInst) {
    ...
    ExecutionResult result = vm.Execute(method, &frame);
    EXPECT_TRUE(result.has_return_value);
    EXPECT_EQ(result.return_value.tag, ValueTag::Null);
}
...
TEST(Interpreter_NewFeatures, InterfaceVtableDispatch) {
    ...
    callvirt3.secondary_index = static_cast<CHAOS_IL2CPP_SIZE>(0u);
    ...
    EXPECT_EQ(result.call_target, reinterpret_cast<void*>(static_cast<CHAOS_IL2CPP_UINTPTR>(0xBABEu)));
}
