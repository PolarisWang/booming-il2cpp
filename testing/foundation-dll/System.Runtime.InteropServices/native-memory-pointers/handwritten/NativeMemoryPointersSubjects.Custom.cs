...
public static partial class NativeMemoryPointersSubjects
{
    ...
    {
        try {
            Marshal.Copy(default, 42, IntPtr.Zero, 42);
        }
        catch { _exitCode = 1; }
    }
    ...
    {
        try {
            Marshal.Copy(IntPtr.Zero, default, 42, 42);
        }
        catch { _exitCode = 1; }
    }
    ...
    {
        try {
            Assert.IsNotNull(Marshal.QueryInterface(IntPtr.Zero, out Guid.Empty, out IntPtr.Zero));
        }
        catch { _exitCode = 1; }
    }
    ...
    {
        try {
            Marshal.ThrowExceptionForHR(42, out Guid.Empty, IntPtr.Zero);
        }
        catch { _exitCode = 1; }
    }
