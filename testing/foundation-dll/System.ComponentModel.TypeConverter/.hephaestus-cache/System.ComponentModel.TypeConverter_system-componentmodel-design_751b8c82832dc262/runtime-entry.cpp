...
// Helper: __try dispatch isolated from C++ objects (avoids C2712)
// MSVC cannot have __try/__except in a function that also has C++ stack objects.
#if defined(_WIN32)
static int64_t FactTryDispatch(int method_index) {
    __try {
        return chaos::il2cpp::runtime_core::ChaosDispatchMethodGetValue(
            GetHotpatchEntries(), kAotMethodCount, method_index, CHAOS_USE_DEFAULT_THUNKS);
...
static int RunFactMode() {
    ...
    for (int si = 0; si < kCount; si++) {
        ...
        if (i < 0 || i >= kAotMethodCount) continue;
#if defined(_WIN32)
        __try {
...
// AOT mode: try/catch is safe because there is no __try/__except in this
// scope (the __try blocks are in the dispatch loop, which has already
// completed by the time CHAOS_FACT_CHECK() is called).  MSVC does not
// allow mixing SEH and C++ EH in the same function, so use real try/catch
// in AOT mode and a no-op stub in JIT mode.
...
#else
// JIT mode: the dispatch loop uses __try/__except (SEH).  MSVC does not
// allow mixing C++ EH (try/catch) and SEH (__try) in the same function.
// Provide a no-op stub — JIT-mode assertion checking relies on the VEH.
