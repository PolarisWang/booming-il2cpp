...
// Weak fallback definition for Assert_Complete. Provides a linkable symbol
// when subjects code has no assertions (no definition from codegen).
// __declspec(selectany) allows the subjects definition to override this at
// link time, eliminating the Python LINK-HACK-3 (chaos_assert_stubs.cpp).
// MSVC: selectany. GCC: __attribute__((weak)).
#if defined(_MSC_VER)
...
static int RunBenchmarkAllMode(int iterations) {
    ...
    for (int si = 0; si < kCount; si++) {
        ...
        __try {
            ...
            printf("{\"methodIndex\":%d,\"elapsedMilliseconds\":-1.0,\"allocatedBytes\":0,\"error\":\"fatal crash\"}\n",
                   i);
            fflush(stdout);
        }
#endif
