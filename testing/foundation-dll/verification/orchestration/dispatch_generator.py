...
def generate_verification_dispatch(
    ...
    write('using chaos::il2cpp::runtime_core::chaos_gc_enter_no_gc_region;')
    write('using chaos::il2cpp::runtime_core::chaos_gc_leave_no_gc_region;')
    write('')
    ...
    if is_jit:
        ...
        write(f'extern "C" CHAOS_IL2CPP_INT32 RunFactAll() {{')
        write('    // Suppress GC during fact dispatch to prevent GC-triggered')
        write('    // object moves from racing with native-AOT generated code.')
        write('    // NO_GC_REGION causes NurseryAllocateSlow to skip collection and')
        write('    // fall back to old-gen (non-moving) allocation when nursery is full.')
        write('    chaos_gc_enter_no_gc_region();')
        write(f'    auto result = ChaosDispatchMethodAllModules({thunks});')
        write('    chaos_gc_leave_no_gc_region();')
        write('    return result;')
        write('}')
    write('')
    ...
    write('    // Suppress GC during timed measurement to prevent GC-triggered')
    write('    // object moves from racing with the tight native-AOT loop.')
    write('    // NO_GC_REGION causes NurseryAllocateSlow to skip collection and')
    write('    // fall back to old-gen (non-moving) allocation when nursery is full.')
    write('    chaos_gc_enter_no_gc_region();')
    ...
    if is_jit:
        ...
        write(f'extern "C" CHAOS_IL2CPP_INT32 RunHotpatchAll() {{')
        write('    // Suppress GC during hotpatch dispatch to prevent GC-triggered')
        write('    // object moves from racing with native-AOT generated code.')
        write('    chaos_gc_enter_no_gc_region();')
        write(f'    auto result = ChaosDispatchMethodAllModules({thunks});')
        write('    chaos_gc_leave_no_gc_region();')
    ...
    write('    result.elapsed_ms = std::chrono::duration<double, std::milli>(end - start).count();')
    write('    result.allocated_bytes = alloc_after - alloc_before;')
    write('#if CHAOS_IL2CPP_PROFILE_ENABLED')
    ...
    write('#endif // CHAOS_IL2CPP_PROFILE_ENABLED')
    write('    return result;')
    write('}')
