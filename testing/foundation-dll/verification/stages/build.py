...
def _custom_subjects_metadata(
    ...
    for idx, (method_name, ret_type, type_name) in enumerate(method_info):
        subject_id = (
            f"CombinedSubjects/{type_name}::{method_name}:{ret_type}()"
        )
        kind = "benchmark"
...
def _compile_custom_subjects(
    ...
    Parses [Fact] and [Benchmark] methods with any return type from the
    .Custom.cs files, generates combined source with proper wrappers,
    and compiles into CombinedSubjects.dll.

    Returns list of (methodName, returnType, typeName) tuples on success,
    or None on failure.
    """
    # Parse methods from .Custom.cs files: extract namespace, class, ret type, method name
    method_info: list[tuple[str, str, str, str]] = []  # (ns, class, methodName, retType)
    for cs_file in sorted(custom_cs_files):
        ...
        # Extract class name
        cls_match = re.search(r'public\s+static\s+(?:partial\s+)?class\s+(\w+)', text)
        class_name = cls_match.group(1) if cls_match else "CustomSubjects"
        # Extract methods: [Fact] [Benchmark] public static TYPE MethodName()
        for match in re.finditer(
            r'\[(?:Fact|Benchmark)\].*?public\s+static\s+(\S+)\s+(\w+)\s*\(\)',
            text, re.DOTALL):
            ...
            metadata_ret = ret_map.get(ret_type, ret_type)
            method_info.append((ns, class_name, method_name, metadata_ret))
    ...
    with open(combined_cs_path, "w", encoding="utf-8") as f:
        ...
        for ns, cls, method_name, metadata_ret in method_info:
            wrapper_ns = f"AutoGenerated.{ns.replace('.', '_')}" if ns else "AutoGenerated"
            wrapper_cls = f"{cls.replace('.', '_')}Tests"
            # Use fully qualified name to call the original method, since the
            # wrapper namespace differs from the original class's namespace.
            qualified_cls = f"global::{ns}.{cls}" if ns else cls
            is_void = metadata_ret == "System.Void"
            wrapper_body = f"{{ {qualified_cls}.{method_name}(); return 0; }}" if is_void else f"{{ return (long){qualified_cls}.{method_name}(); }}"
            f.write(f"namespace {wrapper_ns}\n")
            f.write("{\n")
            f.write(f"    public unsafe partial class {wrapper_cls}\n")
            f.write("    {\n")
            f.write(
                f"        [Fact]\n"
                f"        [Benchmark]\n"
                f"        public long {method_name}() {wrapper_body}\n"
            )
            f.write("    }\n")
            f.write("}\n\n")

    # Return list of (methodName, metadataRet, wrapperTypeName)
    result: list[tuple[str, str, str]] = []
    for ns, cls, method_name, metadata_ret in method_info:
        wrapper_ns = f"AutoGenerated.{ns.replace('.', '_')}" if ns else "AutoGenerated"
        wrapper_cls = f"{cls.replace('.', '_')}Tests"
        result.append((method_name, metadata_ret, f"{wrapper_ns}.{wrapper_cls}"))
    ...
    if proc.returncode != 0 or not subjects_dll.exists():
        print(f"  [build] Custom subjects build FAILED for {tfm}")
        for line in (proc.stderr.splitlines() + proc.stdout.splitlines())[-15:]:
            print(f"      {line}")
        if tfm != "net8.0":
...
def run_build(ctx: ChunkContext, stages: dict[str, StageResult]) -> StageResult:
    ...
    # ── Pre-TPG: generate assert stub definitions ──
    # runtime-entry.cpp references Assert_Reset/Assert_Complete from
    # Chaos.TestFramework.Sdk.  In AOT mode, these aren't generated by
    # the codegen, so provide unconditional stubs before the TPG cmake
    # build runs.  The stub file is found by CMakeLists.txt's GLOB *.cpp.
