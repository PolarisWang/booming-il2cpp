...
def _compile_custom_subjects(
    ...
    with open(combined_cs_path, "w", encoding="utf-8") as f:
        ...
        for idx, (ns, cls, method_name, metadata_ret) in enumerate(method_info):
            ...
            subject_entry_name = f"Subject_{idx}"
            ...
            f.write(
                f"        [Benchmark]\n"
                f"        public long {subject_entry_name}() {wrapper_body}\n"
            )
            f.write("    }\n")
            f.write("}\n\n")

    # Return list of (asmComponent, typePath, methodName, metadataRet)
    # asmComponent = namespace part after AutoGenerated. (e.g. "AutoGenerated_GcStress")
    # typePath = class name WITHOUT "Tests" suffix (TPG adds Tests when reconstructing SubjectId)
    result: list[tuple[str, str, str, str]] = []
    for idx, (ns, cls, method_name, metadata_ret) in enumerate(method_info):
        asm_component = ns.replace('.', '_')  # Dot-separated namespace → underscore-separated identifier
        type_path = cls  # Class name without Tests suffix — TPG adds it
        subject_entry_name = f"Subject_{idx}"
        result.append((asm_component, type_path, subject_entry_name, metadata_ret))
