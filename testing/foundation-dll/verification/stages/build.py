...
def _detect_tfm(dll_path: Path) -> str:
    """Detect target framework moniker from the DLL's runtimeconfig.json or path.

    Priority:
    1. Parse <dll_name>.runtimeconfig.json (same dir as dll_path) for "tfm" field.
    2. Fall back to path heuristics (Microsoft.NETCore.App/X.Y or /netX.Y/runtime/).
    3. Final fallback to "net8.0".
    """
    # Primary: read from .runtimeconfig.json (same directory, same stem)
    rc_path = dll_path.with_suffix(".runtimeconfig.json")
    if rc_path.is_file():
        try:
    ...
    # Fallback: standard runtime layout
