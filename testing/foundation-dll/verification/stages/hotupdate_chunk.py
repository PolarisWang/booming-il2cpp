...
def _generate_patch_host_arrays(native_dir: Path, metadata: dict) -> bool:
    ...
    for idx in hotupdate_indices:
        ...
        host_type_names.append(_sanitize_cpp_identifier(full_type) + 'Tests')
...
def _build_patch_dll(patch_output: Path, patch_dll: Path) -> bool:
    """Combine ATG --patch-mode generated .cs files and build into a DLL."""
    all_patch_files = sorted(patch_output.rglob("*.cs"))
    ...
    for pf in all_patch_files:
        ...
        lines = [l for l in text.splitlines()
                 if not l.startswith("using ") and not l.startswith("//")]
        ns_blocks.append("\n".join(lines))

    combined_cs.write_text(
...
def run_hotupdate_chunk(ctx: ChunkContext, stages: dict[str, StageResult]) -> StageResult:
    ...
    # When PatchDataExtractor is ready, it will produce patch.patchdata at
    # ctx.patch_data_path and the --patch-data flag will activate automatically.
    ...
    if target_dll is None:
        ...
        if atg_result.returncode == 0:
            ...
            if _build_patch_dll(patch_output, patch_dll):
    ...
    # ── Step 3: Run entry.exe --hotupdate [--patch-data ...] ──
    ...
    # Determine status:
    # - With patch data: require allSemantic=true + allRevert=true
    # - Without patch data: just pass if methods executed (no semantic change possible)
    if patch_data_path:
        status = "passed" if (all_semantic and all_revert and failed == 0) else "failed"
        if passed == 0:
            status = "failed"
    else:
    ...
    print(f"  [hotupdate] {status}: {passed} passed, "
          f"semantic_changed={semantic_changed}, revert={all_revert} "
          f"({duration_ms}ms)")

    return StageResult(
        stage="hotupdate", status=status,
        summary=f"{status}: {passed} passed, semantic_changed={semantic_changed}",
        details=result_data,
