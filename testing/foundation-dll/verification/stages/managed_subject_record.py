...
def run_managed_record(ctx: FamilyContext, stages: dict[str, StageResult]) -> StageResult:
    ...
    # ── 4. Generate harness — use reflection to discover [Fact] methods ──
    harness_dir = ctx.family_dir / "managed_test" / "subject_record"
    harness_dir.mkdir(parents=True, exist_ok=True)

    cs_source = (
        "// Auto-generated managed subject record harness\n"
        "// Family: " + ctx.slug + ", Assembly: " + ctx.assembly + "\n"
        "// Uses reflection to discover [Fact]-annotated subject methods.\n"
        "using System;\n"
        ...
        "        // Discover [Fact]-annotated static methods via reflection\n"
        "        var subjectMethods = typeof(" + full_class_ref + ")\n"
        "            .GetMethods(BindingFlags.Public | BindingFlags.Static | BindingFlags.DeclaredOnly)\n"
        "            .Where(m => m.CustomAttributes.Any(a => a.AttributeType.Name == \"FactAttribute\"))\n"
        "            .Select(m =>\n"
        "            {\n"
        "                var name = m.Name;\n"
        "                if (name.StartsWith(\"CustomEntrySubject_\"))\n"
        "                {\n"
        "                    int idx = int.Parse(name.Substring(\"CustomEntrySubject_\".Length));\n"
        "                    return new { Index = idx, Name = name, IsCustom = true, Method = m };\n"
        "                }\n"
        "                if (name.StartsWith(\"Subject_\"))\n"
        "                {\n"
        "                    int idx = int.Parse(name.Substring(\"Subject_\".Length));\n"
        "                    return new { Index = idx, Name = name, IsCustom = false, Method = m };\n"
        "                }\n"
        "                return null;\n"
        "            })\n"
        "            .Where(t => t != null)\n"
        ...
        "            .ToList();\n"
        "\n"
        "        foreach (var sm in subjectMethods)\n"
        "        {\n"
        "            try\n"
        "            {\n"
        "                sm.Method.Invoke(null, null);\n"
        "                results.Add(new MethodResult\n"
        "                {\n"
        "                    MethodIndex = sm.Index,\n"
        "                    SubjectName = sm.Name,\n"
        ...
        "            MethodCount = subjectMethods.Count,\n"
        "            Results = results,\n"
        "        };\n"
