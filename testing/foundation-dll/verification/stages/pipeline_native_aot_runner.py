...
from verification.stages.hotupdate_build import generate_patch_data, write_sentinel_patch_data
...
def ensure_cmake_lists_file(cmakelists: Path, family_slug: str, verification: Path, *, is_jit: bool = False, config_tier: str = "CHECK") -> None:
    ...
    cmake_content = (
        ...
        f'# (set via -D or CMakePresets.json). Expected values: check / profile / ship.\n'
        f'if(NOT DEFINED CHAOS_IL2CPP_CONFIG_TIER)\n'
        f'  set(CHAOS_IL2CPP_CONFIG_TIER "check")\n'
        f'endif()\n'
        ...
        f'elseif(CHAOS_IL2CPP_CONFIG_TIER STREQUAL "check")\n'
        ...
        f'    "${{CHAOS_PROJECT_ROOT}}/src/native/pal"  # chaos/pal/*.h (pal_sync.h, pal_time.h)\n'
        ...
        f'# Linux: runtime stubs compiled from source need pal symbols (e.g. PalGetRealtimeNs)\n'
        f'# The pal library is built as part of the project but not distributed in the SDK yet.\n'
        f'# Fall back to the debug build\'s libchaos_pal.a when the profile/ship SDK doesn\'t have it.\n'
        f'if(NOT MSVC AND NOT EXISTS "${{CHAOS_SDK_DIR}}/ConvertCharSubjects/lib/libchaos_pal.a")\n'
        f'    find_library(CHAOS_PAL_LIB chaos_pal PATHS "${{CHAOS_PROJECT_ROOT}}/build/debug/src/native/pal" NO_DEFAULT_PATH)\n'
        f'    if(CHAOS_PAL_LIB)\n'
        f'        target_link_libraries(entry PRIVATE "${{CHAOS_PAL_LIB}}")\n'
        f'    endif()\n'
        f'endif()\n'
...
def ensure_micro_benchmark_source(native_dir: Path) -> None:
    ...
    content = (
        ...
        '    }\n'
        '\n'
        '    // ── Benchmark 4+: Custom entry benchmarks ────────────────\n'
        '    if (kAotMethodCount > 18) {\n'
        '        constexpr int ce_iter = 100000;\n'
        '        for (int ce_idx = 18; ce_idx < kAotMethodCount; ce_idx++) {\n'
        '            BenchmarkResult br_ce = RunBenchmark(ce_idx, ce_iter);\n'
        '            if (br_ce.elapsed_ms >= 0.0) {\n'
        '                printf("CustomEntry_%d: %.1f ns/op, %ld B/op\\n",\n'
        '                       ce_idx, (br_ce.elapsed_ms * 1e6) / ce_iter, br_ce.allocated_bytes);\n'
        '            } else {\n'
        '                printf("CustomEntry_%d: error (%.1f ms)\\n", ce_idx, br_ce.elapsed_ms);\n'
        '            }\n'
        '        }\n'
        '    }\n'
        '\n'
