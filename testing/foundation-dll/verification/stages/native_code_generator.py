...
def method_slot_name(method_subject_id: str) -> str:
...
def family_namespace_slug(family_id: str) -> str:
    """Derive a C++ namespace fragment from family id."""
    slug = slug_from_family_id(family_id)
    parts = slug.split("-")
    return "_".join(parts)
...
def _generate_hotupdate_cpp(
    *,
    ...
    for idx, subject_id in enumerate(method_subject_ids):
        slot_name = method_slot_name(subject_id)
...
def generate_benchmark_managed_bodies(
    ...
    * ``ManagedBenchmarkHarness.csproj`` — net10.0 executable project.
    ...
    for subject_id in method_subject_ids:
        ...
        is_callable = is_auto_callable(parsed) and not has_unsafe_param(parsed["param_types"])
