// Auto-generated by chaos-il2cpp convert-to-cpp
// Generated Module Source — A1 Typed Dispatch Table wiring
//
// Wires the extern "C" codegen symbols into the typed kFunctions table,
// and implements ChaosGeneratedModuleActivate() for the A2 proxy wrappers.

#include "chaos_runtime_host.h"
#include "chaos_generated_module.h"

// extern "C" symbols from native-aot.generated.cpp
extern "C" const CodeRegistrationV0 chaos_codegen_code_registration;
extern "C" const MetadataRegistrationV0 chaos_codegen_metadata_registration;
extern "C" const CodegenRegistrationOptionsV0 chaos_codegen_options;
extern "C" const int kAotMethodCount;


// extern "C" function declarations for all AOT-compiled methods
extern "C" void GarbageCollectionSubjects_AutoGeneratedProgram__ctor(
            CHAOS_IL2CPP_INTPTR
);
extern "C" void GarbageCollectionSubjects_AutoGeneratedProgram_Main_System_String(
            CHAOS_IL2CPP_INTPTR
);
extern "C" void GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_0(
        void
);
extern "C" void GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_1(
        void
);
extern "C" void GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_2(
        void
);
extern "C" void GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_3(
        void
);
extern "C" void GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_4(
        void
);
extern "C" void GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_5(
        void
);
extern "C" void GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_6(
        void
);

// ═══════════════════════════════════════════════════════════════════════════
// Typed Dispatch Table — wires extern "C" native symbols into typed pointers
// ═══════════════════════════════════════════════════════════════════════════

static const Functions s_functions = {
    {
        &GarbageCollectionSubjects_AutoGeneratedProgram__ctor,
        &GarbageCollectionSubjects_AutoGeneratedProgram_Main_System_String,
    },
    {
        &GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_0,
        &GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_1,
        &GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_2,
        &GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_3,
        &GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_4,
        &GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_5,
        &GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_6,
    },
    9,
};

const Functions kFunctions = s_functions;

// Flat function pointer array for indexed dispatch (benchmarking).
// Avoids fragile reinterpret_cast<void* const*>(&kFunctions) which depends on
// struct layout uniformity across sub-struct members.
extern "C" void* kFunctionsFlat[9];
void* kFunctionsFlat[9] = {
    reinterpret_cast<void*>(&GarbageCollectionSubjects_AutoGeneratedProgram__ctor),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_AutoGeneratedProgram_Main_System_String),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_0),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_1),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_2),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_3),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_4),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_5),
    reinterpret_cast<void*>(&GarbageCollectionSubjects_GarbageCollectionSubjects_Subject_6),
};


bool ChaosGeneratedModuleActivate(ChaosRuntimeHost* host) {
    if (!host) return false;
    if (!host->IsInitialized()) return false;

    return host->RegisterModule(
        &chaos_codegen_code_registration,
        &chaos_codegen_metadata_registration,
        &chaos_codegen_options
    );
}