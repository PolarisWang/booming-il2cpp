...
extern "C" BenchmarkResult RunBenchmark(int entry_index, int iterations) {
    if (entry_index < 0 || entry_index >= kSubjectEntryCount)
        return {-1.0, 0};
    auto* entries = GetHotpatchEntries();
    int actual_index = kSubjectSlotMap[entry_index];
    // Warmup
    ChaosDispatchMethod(entries, kAotMethodCount, actual_index, CHAOS_DISPATCH_THUNKS);
