...
// ── SEH-safe benchmark call ──────────────────────────────────────────────
// Wraps RunBenchmark in __try/__except to catch access violations from JIT
// codegen bugs without crashing the entire process.  This is a standalone
// helper (no C++ exception handling in the same function) to satisfy MSVC's
// C2713 restriction.
extern "C" double RunBenchmark(int, int);
static double RunBenchmarkSafe(int entry_index, int iterations) noexcept {
    __try {
        return RunBenchmark(entry_index, iterations);
    } __except(EXCEPTION_EXECUTE_HANDLER) {
        unsigned long code = GetExceptionCode();
        fprintf(stderr, "[JIT_BENCH_SEH] method=%d code=0x%lx\n", entry_index, code);
        fflush(stderr);
        return -1.0;
    }
}
...
int main(int argc, char** argv) {
    ...
    case RunMode::Benchmark: {
        ...
        chaos::il2cpp::common::g_chaos_fail_hook = []() { throw chaos_managed_exception{}; };
        try {
            elapsed_ms = RunBenchmarkSafe(entry_index, iterations);
        } catch (...) {
            elapsed_ms = -1.0;
        }
        chaos::il2cpp::common::g_chaos_fail_hook = nullptr;
