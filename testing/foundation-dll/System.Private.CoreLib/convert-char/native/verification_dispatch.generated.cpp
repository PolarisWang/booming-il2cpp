...
// Assembly: ConvertCharSubjects, Methods: 18, Mode: AOT
...
// Subject entry count: 18 (from manifest)
...
extern "C" CHAOS_IL2CPP_INT32 RunFactAll() {
    return ChaosDispatchMethodAllModules(kDefaultArgThunks);
}
...
extern "C" double RunBenchmark(int entry_index, int iterations) {
    ...
    for (int i = 0; i < iterations; i++) {
        ChaosDispatchMethod(entries, kAotMethodCount, entry_index, kDefaultArgThunks);
    }
