...
struct ManagedThread {
    ...
    std::atomic<CHAOS_IL2CPP_INTPTR> pending_exception{0}; // QueueExceptionForThread
...
/// Queue an exception for delivery to a target thread.
/// The target thread will throw the exception on its next SafepointPoll.
/// If the target is in cooperative mode, a safepoint kick (APC/SIGUSR1) is
/// attempted to force an immediate poll; otherwise delivery is deferred.
/// @param thread_id     Target thread's managed_id
/// @param exception_obj Opaque exception pointer (stored in pending_exception)
void QueueExceptionForThread(int32_t thread_id, CHAOS_IL2CPP_INTPTR exception_obj) noexcept;
