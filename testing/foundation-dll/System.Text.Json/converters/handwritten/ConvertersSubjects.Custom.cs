...
public static partial class ConvertersSubjects
{
    ...
    {
        try {
            Assert.IsNotNull(default(JsonConverter)!.Read(out default, typeof(byte), null!));
        }
        catch { _exitCode = 1; }
    }
    ...
    {
        try {
            Assert.IsNotNull(default(JsonConverter)!.ReadAsPropertyName(out default, typeof(byte), null!));
        }
        catch { _exitCode = 1; }
    }
