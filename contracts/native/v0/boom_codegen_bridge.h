...
typedef struct CodegenBridgeV0 {
    ...
    BridgeStatus (BOOM_RUNTIME_ABI_CALL* register_codegen)(
        const CodeRegistrationV0* code_registration,
        const MetadataRegistrationV0* metadata_registration,
        const CodegenRegistrationOptionsV0* options);
    BridgeStatus (BOOM_RUNTIME_ABI_CALL* bootstrap_runtime)(void);

    /* Runtime metadata helpers. */
    TypeInfoHandle (BOOM_RUNTIME_ABI_CALL* resolve_type_by_token)(
        ImageHandle image,
        uint32_t type_token);
    MethodInfoHandle (BOOM_RUNTIME_ABI_CALL* resolve_method_by_token)(
        ImageHandle image,
        uint32_t method_token);
    FieldInfoHandle (BOOM_RUNTIME_ABI_CALL* resolve_field_by_token)(
        ImageHandle image,
    ...
    void* (BOOM_RUNTIME_ABI_CALL* box_value)(
        ...
        const void* value,
        size_t value_size);
    BridgeStatus (BOOM_RUNTIME_ABI_CALL* unbox_value)(
        RuntimeState* runtime_state,
        void* boxed_object,
        void* out_value,
    ...
    MethodInfoHandle (BOOM_RUNTIME_ABI_CALL* resolve_virtual_method)(
        TypeInfoHandle instance_type,
        MethodInfoHandle declared_method);
    BridgeStatus (BOOM_RUNTIME_ABI_CALL* invoke_virtual)(
        RuntimeState* runtime_state,
        ThreadState* thread_state,
    ...
    BridgeStatus (BOOM_RUNTIME_ABI_CALL* delegate_invoke)(
        RuntimeState* runtime_state,
        ThreadState* thread_state,
...
BOOM_RUNTIME_ABI_EXPORT const CodegenBridgeV0* BOOM_RUNTIME_ABI_CALL boom_codegen_get_bridge_v0(void);
