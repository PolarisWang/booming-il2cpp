...
public sealed class HelloWorldObjectProofSpecTests
{
    ...
    public void SubjectManifestUsesSubjectRootMetadata()
    {
        ...
        Assert.Equal("proof-dev", root.GetProperty("defaultValidationProfile").GetString());
        ...
        var validationProfiles = root.GetProperty("validationProfiles");
        Assert.True(validationProfiles.TryGetProperty("proof-dev", out var proofDevProfile));
        Assert.Contains(
            proofDevProfile.EnumerateArray().Select(item => item.GetString()).ToArray(),
            validationKind => validationKind == "unit"
        );
        ...
        Assert.Equal("CHECK", unitValidation.GetProperty("defaultVariant").GetString());
        Assert.Equal(
            "subjects/HelloWorldObject/validation/unit/HelloWorldObject.Subject.UnitTests/HelloWorldObject.Subject.UnitTests.csproj",
