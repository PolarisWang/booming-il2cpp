...
| gc-p3-02 | phase-2 | **completed** | **GC 基准回归测试套件（G-15）** | phase-1 | docs/dev/completed/gc-p3-02-benchmark-regression/ | 2 周 |
| gc-p3-03 | phase-2 | **completed** | **SOS 调试扩展（G-16）** | phase-1 | docs/dev/completed/gc-p3-03-sos-debug-extension/ | 2-3 周 |
| gc-p3-04 | phase-2 | **completed** | **Arrhenius 72h 升温测试（G-17）** | phase-1 | docs/dev/completed/gc-p3-04-arrhenius-soak/ | 1 周 |
| gc-p2-vfy | phase-2 | **completed** | **Phase 2 集成验证** | 全部 P2 | 验证目录 | 3 天 |
| **Phase 3 — 高级并发能力** | | | | | | |
| gc-p4-01 | phase-3 | **completed** | **Server GC 模式（G-4）** | phase-2 | gc_old_gen.cpp, gc_bgc.cpp, gc_numa.h | 2-3 月 |
| gc-p4-02 | phase-3 | **completed** | **真正并发根扫描（G-25）** | phase-2 | thread_state.cpp, gc_root_scanner.cpp | 2-3 周 |
| gc-p4-03 | phase-3 | **completed** | **高级堆布局优化（G-23）** | phase-2 | gc_layout.h/cpp, gc_old_gen.cpp | 2 周 |
| gc-p3-vfy | phase-3 | **completed** | **Phase 3 全平台 72h soak 验收** | 全部 P3 | 全平台测试 | 1 周 |
...
串行推进，当前进度：**全部 Phase 1 + Phase 2 已完成。下一步：gc-p4-01（Server GC 模式）**。
