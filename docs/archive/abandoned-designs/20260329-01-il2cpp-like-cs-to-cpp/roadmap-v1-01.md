...
### 缺口 J：编译系统骨架（Requests / Context / Collectors / Results）

当前设计已经有 `Metadata Graph`、`Typed IL IR`、`AOT manifest` 和 runtime ABI，但仍缺少一套正式的编译系统骨架来承载这些产物之间的执行关系。建议补齐：

- `ConversionRequest / CompilationRequest / SharedSettings`
- `AssemblyConversionContext` 对应物与按能力裁剪的 context 视图
- `Services / StatefulServices`
- `Collectors`
- `PhaseResults`
- `Initialize / Setup / PrimaryCollection / PrimaryWrite / SecondaryCollection / SecondaryWrite / Completion`

如果不把这层补出来，后续很容易出现：

- phase 间隐式共享状态蔓延
- linker / codegen / build 边界重新糊成一团
- 并行和增量需求出现时只能返工核心骨架
...
## Shared Skeleton：Requests、Context 与 Phase 骨架

**目标：** 在 `Roadmap 1` 与 `Roadmap 3` 正式铺开实现前，先把共享的编译系统骨架落成正式工程边界。

**说明：**

- 这不是新增独立 Roadmap，而是 `Roadmap 1 / Roadmap 3` 的共同前置子层
- 其作用类似 Unity IL2CPP 中的 `Api + AssemblyConversionContext + Results + Scheduler` 骨架，但我们保留自己的 `Metadata Graph / Typed IL IR / Runtime-Contract IR` 三层显式模型

**关键工作：**

- [ ] 定义 `ConversionRequest / CompilationRequest / SharedSettings`
- [ ] 定义 `AssemblyConversionContext` 对应物
- [ ] 定义 `ReadOnlyContext / CollectionContext / WriteContext / SchedulingContext`
- [ ] 定义 `Services / StatefulServices`
- [ ] 定义 `Collectors`
- [ ] 定义 `PhaseResults`
- [ ] 定义 `Step / Scheduler` 最小骨架，先支持串行执行
- [ ] 定义正式 phase 边界：`Initialize / Setup / PrimaryCollection / PrimaryWrite / SecondaryCollection / SecondaryWrite / Completion`

**验证：**

- [ ] 请求对象可稳定序列化/快照，便于回归和差分
- [ ] phase 间只通过 `PhaseResults` 交接，不依赖隐式全局状态
- [ ] 串行 scheduler 能跑通空转 pipeline 和最小 smoke 输入

**退出标准：**

- `Roadmap 1` 与 `Roadmap 3` 都可以在这套 skeleton 上并行推进
- 后续并行、增量和 per-assembly 模式有明确插入点，而不是再重写主骨架
...
- [ ] 接入 shared skeleton：请求对象、conversion context、services、collectors、phase results
...
1. 先把 `Initialize / Setup / PrimaryCollection` 这三段在 shared skeleton 上跑通
2. 再从无泛型、无异常的简单方法切入 typed IL IR
3. 再补异常区域和复杂控制流
4. 最后处理泛型签名、token 解析和 debug/source 信息
...
- [ ] 复用 shared skeleton 的 phase/context/results，避免 runtime 与前端各自形成平行执行模型
...
1. 先在 `Initialize / Setup` 风格 phase 上打通 bootstrap、类型世界、对象分配和 class init
2. 再补异常、线程、反射缓存
...
- [ ] 在 `PrimaryWrite / SecondaryWrite` 风格 phase 中落地 codegen 与 metadata writer，而不是直接拼接成单阶段后端
...
### 当前执行入口（2026-04-05 冻结）

#### Phase 1 真实关键路径

Phase 1 不应再被理解为“大模块并行铺开”，而应理解为一条由最小闭环驱动的 vertical slice 路线。
...
- `Roadmap 1` 与 `Roadmap 3` 可以在 `Roadmap 0` 完成后部分并行。
- `Roadmap 2` 必须建立在足够稳定的 semantic world 之上。
- `Roadmap 4` 不应理解为“大规模 C++ 发射阶段先行”，而应建立在：
  - 运行时最小落点已经存在
  - `AOT manifest` 已可读可 diff
  - `Metadata Writer` 最小 registration 已稳定

#### 第一条正式 Vertical Slice

Phase 1 第一条正式 vertical slice 统一命名为 `HelloWorld`。

这个 `HelloWorld` 使用 object-based 形态，而不是只打印字面量的微样例。它必须至少证明：
...
- string literal / narrow concat path
- `Console.WriteLine(string)` narrow path
- runtime bootstrap
...
1. `Loader / Typed IL IR`
   - 识别并归一化 `Greeter..ctor`、`Greeter.BuildMessage`、`Program.Main` 的语义 IL 形状
2. `Linker`
   - 产出小而明确的 retained world 与 `AOT manifest`
3. `Metadata Writer`
   - 产出 `code registration`、`metadata registration`、最小 metadata blob/table
4. `Runtime`
   - 完成 bootstrap、对象分配、字段访问、窄路径字符串与控制台输出
5. `CodeGen`
   - 只发射 `Program.Main`、`Greeter..ctor`、`Greeter.BuildMessage`

#### 立即跟进的样例顺序

在 `HelloWorld` 成功后，推荐样例顺序固定为：
...
以下方向不应阻塞 `HelloWorld` 首条闭环：

- `ICU`
- `libffi`
- `MMTk`
- 广义动态调用
- 广义 globalization parity
- 过早的多平台语义完全一致

#### Phase 1 第一证明点

Phase 1 第一证明点不是“生成了很多 C++”，而是同时得到：

- 一个 retained world
- 一套 metadata registration
- 一个 runtime bootstrap
- 一条 generated-code path
- 一个在 reference platform 上输出 `Hello, World!` 且返回 `0` 的 `HelloWorld`
...
| --- | --- | --- | --- | --- | --- |
| `20260331-01-roadmap-0-contract-freeze-and-skeleton` | `Roadmap 0` | `completed` | `codex` | 冻结 contract、工程骨架、verify gate 与 smoke 基线 | `-` |
| `20260404-03-roadmap-1-assembly-and-typed-il-ir` | `Roadmap 1` | `ready` | `unassigned` | 建立程序集加载、元数据世界与 Typed IL IR 正式产物 | `20260331-01-roadmap-0-contract-freeze-and-skeleton` |
| `roadmap-2-aot-closure-and-manifest` | `Roadmap 2` | `planned` | `unassigned` | 建立闭包分析、保留规则与 AOT manifest 体系 | `20260404-03-roadmap-1-assembly-and-typed-il-ir` |
| `20260404-04-roadmap-3-runtime-core-v1` | `Roadmap 3` | `ready` | `unassigned` | 实现 runtime bootstrap、对象模型、线程与异常语义基线 | `20260331-01-roadmap-0-contract-freeze-and-skeleton` |
| `roadmap-4-codegen-bridge-and-e2e` | `Roadmap 4` | `planned` | `unassigned` | 打通 Runtime-Contract IR 到 C++ 发射与单平台端到端链路 | `roadmap-2-aot-closure-and-manifest, 20260404-04-roadmap-3-runtime-core-v1` |
| `roadmap-5-mvp-and-golden-projects` | `Roadmap 5` | `planned` | `unassigned` | 用 golden projects 驱动 MVP 兼容性收敛与缺口补齐 | `roadmap-4-codegen-bridge-and-e2e` |
| `roadmap-6-cross-platform-and-phase-2-3` | `Roadmap 6` | `planned` | `unassigned` | 扩展跨平台、打包、宿主嵌入和 Phase 2/3 演进能力 | `roadmap-5-mvp-and-golden-projects` |

说明：

- `ready` 表示前置依赖已经满足，可以正式创建新的独立任务目录
- `planned` 表示已识别但前置依赖尚未满足，暂不创建任务目录
- 以上 `Roadmap 1` / `Roadmap 3` 的 `ready` 状态来自当前文档中明确写出的依赖关系，而不是表示已经开始执行
