...
## 10. `Chaos.TestFramework` 设计

### 10.1 目标

`Chaos.TestFramework` 是一个极薄的声明层 DLL，只做三件事：
...
/// <summary>
/// 定义源码内 proof 的主分类，覆盖低基数、跨 subject 共享的 correctness 维度。
/// </summary>
public enum ChaosUnitCategory : byte
{
    /// <summary>运行时可观察行为与输出合同。</summary>
    RuntimeContract = 1,
    /// <summary>metadata、registration、supplement 等元数据合同。</summary>
    MetadataContract = 2,
    /// <summary>托管与 native 之间的 interop 合同。</summary>
    InteropContract = 3,
    /// <summary>host lifecycle、embedding、engine-facing 合同。</summary>
    HostContract = 4,
    /// <summary>热更加载、回滚、补丁一致性合同。</summary>
    HotUpdateContract = 5,
}

/// <summary>
/// 定义 runtime benchmark 的主分类，覆盖低基数、跨 subject 共享的性能维度。
/// </summary>
public enum ChaosBenchmarkCategory : byte
{
    /// <summary>调度、分发、调用路径类 benchmark。</summary>
    RuntimeDispatch = 1,
    /// <summary>启动、冷启动、初始化路径类 benchmark。</summary>
    Startup = 2,
    /// <summary>分配、GC 压力、内存 churn 类 benchmark。</summary>
    Allocation = 3,
    /// <summary>热更加载、切换、回合执行类 benchmark。</summary>
    HotUpdate = 4,
}

[Flags]
/// <summary>
/// 定义 benchmark 可产出的标准指标位标志。
/// </summary>
public enum ChaosMetric : ushort
{
    /// <summary>无指标。</summary>
    None = 0,
    /// <summary>墙钟时间，单位微秒。</summary>
    WallClockUs = 1 << 0,
    /// <summary>托管堆分配字节数。</summary>
    ManagedAllocBytes = 1 << 1,
    /// <summary>native 侧分配字节数。</summary>
    NativeAllocBytes = 1 << 2,
    /// <summary>峰值工作集或峰值驻留内存。</summary>
    PeakWorkingSet = 1 << 3,
}

[Flags]
/// <summary>
/// 定义测试入口所依赖的 runtime 能力位标志。
/// </summary>
public enum ChaosRuntimeFeature : uint
{
    /// <summary>无额外能力要求。</summary>
    None = 0,
    /// <summary>泛型共享或泛型实例化相关能力。</summary>
    GenericSharing = 1 << 0,
    /// <summary>反射与 metadata 查询相关能力。</summary>
    Reflection = 1 << 1,
    /// <summary>delegate、function pointer、callback 相关能力。</summary>
    Delegate = 1 << 2,
    /// <summary>异常流、异常传播、异常桥接相关能力。</summary>
    ExceptionFlow = 1 << 3,
    /// <summary>native interop、P/Invoke、ABI 边界相关能力。</summary>
    NativeInterop = 1 << 4,
    /// <summary>热更加载、补丁切换、回滚相关能力。</summary>
    HotUpdate = 1 << 5,
}

[Flags]
/// <summary>
/// 定义 proof 期望产出的证据类型位标志。
/// </summary>
public enum ChaosEvidenceKind : ushort
{
    /// <summary>无额外证据要求。</summary>
    None = 0,
    /// <summary>标准输出文本证据。</summary>
    Stdout = 1 << 0,
    /// <summary>文件、快照、生成物等 artifact 证据。</summary>
    FileArtifact = 1 << 1,
    /// <summary>metadata、manifest、registration 等结构化证据。</summary>
    Metadata = 1 << 2,
    /// <summary>native symbol、导出表、链接结果等证据。</summary>
    NativeSymbol = 1 << 3,
}
...
在新模型里默认都应先收敛成 `Chaos.TestFramework` 提供的 C# API，而不是继续扩成目录协议。

推荐方向是：

- fixture 输入通过 `ChaosFixture` 一类 API 获取
- expected 断言通过 `ChaosExpect` 一类 API 表达
- baseline 比对通过 `ChaosBaseline` 一类 API 表达
- validation helper 通过 `ChaosAssert` / `ChaosProbe` 一类 API 表达

使用层级应与 `ChaosUnitTest` / `ChaosBenchmark` 一致，也就是：

- 由 subject source 直接引用
- 由方法级测试入口直接调用
- 不再要求先落一层 subject 专属目录，再由 runner 反向解释

只有以下内容才继续保留在 `assets/`：

- 二进制样本
- 静态文本模板
- 无法通过 C# API 内联表达的大体积基线文件
- 明确需要按文件保真的外部输入
