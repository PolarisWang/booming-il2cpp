...
updated_at: 2026-04-13 19:47:15 +08:00
...
- latest_progress: `docs/dev/completed/20260413-30-tests-taxonomy-and-subject-project-pruning-roadmap/notes/progress-v1-03.md`
...
- 2026-04-13 19:47:15 +08:00: 继续完成 `tests/` generic 化收尾。为 `tests/support.py` 补齐 temp repo / fixture manifest helper；将 `test_tui.py`、`test_subject_planner.py`、`test_subject_command.py` 改为 fake registry index、synthetic plan、temp repo 驱动；非 integration 测试中的 `select_subject_record(...)` 现已清零，只保留 `tests/support.py` helper 与 `tests/integration/registry/test_registry_scan.py`。
...
- 后续 generic 化回归
  ...
  - `python -m pytest tests/tooling/run tests/unit/planning/test_subject_planner.py tests/unit/reporting/test_events_schema.py tests/unit/performance/test_subject_perf_policy.py tests/unit/compatibility/test_native_reference_bootstrap_support.py tests/integration/run/test_command_manifest.py tests/integration/registry/test_registry_scan.py -q`
    - `220 passed, 34 skipped`

## 残留观察

- `tests/` 中的 `select_subject_record(...)` 现只保留在 `tests/support.py` helper 定义与 `tests/integration/registry/test_registry_scan.py`；unit / tooling 层已不再依赖 live retained subjects。
- 用户之前指出的“`tests/unit/planning/test_subject_planner.py` 以及其他 `tests/` 里仍有 subject 关联逻辑”已进一步收缩到 integration / contract 边界；如果后续还要把 retained smoke 再下沉到更纯的 synthetic contract，可从当前归档任务另起 follow-up。
