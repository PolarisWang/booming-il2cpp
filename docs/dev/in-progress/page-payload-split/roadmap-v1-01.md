...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `pps-1-tests` | 1 | ready | main | 先写失败测试（TDD）：段分类/预算/链接性守卫 | — | b1 | managed 单测 + 负控 | `tests/unit/managed/codegen/Emission/PagePayloadSplitTests.cs` | 测试在旧结构下**红** | `tests/unit/managed/codegen/**` | S |
| `pps-2-extern` | 1 | planned | main | 被拆段内 `static` 表改 `extern` + 共享头补声明 | pps-1 | b1 | 产物符号链接性验证 | `NativeAotEmitter.Shared.cs` | dumpbin 确认符号 external | `src/managed/Chaos.IL2CPP.Generator/NativeAotEmitter.Shared.cs` | M |
