...
### 建议下一入口
- 专职 codegen 域会话修复 `_enrich_metadata_body_availability`（或更根本的 InlineShapeDescriptor IR 发射），使 inline→native 模式正确标注 NativeGenerated。根因：InlineShapeDescriptor 直接内联 C++ 调用不走 IR 发射通路，导致 wrapper `generatedMethodId` 不在 `aot-core-ir.jdata` 中，enrichment Layer 1 匹配失败。
