...
**架构：** D3 双层分派。每个模块生成静态 Dispatch Table，call site 采用模式感知分支——AOT→AOT 零额外开销，patch 时走 InterpreterEntryDirect → Lazy IL→IR 降低 → Interpreter 执行。PatchMetadataCache 本地自洽，不注册全局类型系统（iOS 兼容）。
...
- capabilityItem: HotUpdateDispatchD3
...
**设计文档：** `wiki/03-功能模块/06-il2cpp核心架构/04-历史决策/18-HotUpdateDispatchD3-决策.md`
