...
| [`dev-optimization-campaign`](../../library/skills/dev-optimization-campaign/SKILL.md) | 需要跨设备协调多轮性能优化时；多个 agent 并行优化不同 capability family 时；需要性能优化的完整分析文档和横向方案对比时 | 多 family 性能优化战役编排。跨设备 git 异步协调，每次优化必须产出 docs/optimize/ 完整分析文档，commit 必须含性能对比数据。 | 2.0.0 | `00-skills-maintainers` | large | active |
