---
name: dev-project-wiki-maintenance
...
`wiki` 默认不写。只有当一个任务**确实**产生了长期有效知识，且该知识值得脱离任务上下文复用时，才更新 `wiki/`。

如果一个任务**没有**产生长期知识，不在本技能里补空页面，而是在任务目录的 `STATUS.md` 中记录 `wiki: n/a`；不要为了走流程额外创建 `notes/progress-*.md`。
