# Claude Code 的跨平台 Polyglot Hooks

Claude Code 插件需要在 Windows、macOS 和 Linux 上都能工作的 hooks。本文档解释了使这成为可能的 polyglot 包装器技术。
...
Polyglot 脚本同时是多种语言的有效语法。我们的包装器在 CMD 和 bash 中都是有效的：
...
2. `@echo off` - 抑制命令回显
3. bash.exe 命令以以下方式运行：
   - `-l`（登录 shell）获取带有 Unix 工具的正确 PATH
...
1. `: << 'CMDBLOCK'` - `:` 是无操作，`<< 'CMDBLOCK'` 开始 heredoc
...
对于有多个 hooks 的插件，你可以创建一个通用包装器，将脚本名称作为参数：
...
CMD 找不到 bash。包装器使用完整路径 `C:\Program Files\Git\bin\bash.exe`。如果 Git 安装在其他位置，更新路径。
