...
- 如果用户明确要求“新的验证通过记录 / 新的数据”，或任务命�?`dev:project-test-governance` / `verification-v1` 主线，则 formal objects �?regression 通过后还必须刷新 formal source；默认执�?`run verify verification-v1 --json`
- 只有 `summaryPath` / `eventsPath` / `consolePath`，但没有最�?`verification/archive/{latest,master,reports}` �?`verification/evidence/owners/*/codegen-stubs/*` 更新时，不足以宣称“新测试流程已打通�?- `docs/dev/**` 只记录任务过程，不是长期 authority
