...
对于来自已批准 roadmap 的 child task，默认目标是自动继续执行，而不是在每一步都回头询问用户。执行前只做最小 `preflight self-review`；只要结果是 `pass` 或 `warn`，就继续推进。只有命中 `stop` 条件，才停下来确认。
...
- 不要把 `notes/progress-*.md` 当作每一步都要追加的流水账
...
- 如果当前任务带有 `source_relation: roadmap-child*`，或 `clearance_source: parent-roadmap`：
  ...
  - 若父 roadmap 为 `child_execution_mode = auto`，则不为本 child 再次向用户索要启动确认
- 开始实现前，必须存在一次最小 `preflight self-review`
  - 如果 `preflight_review` 已存在：直接消费
  - 如果缺失：先补做一次最小 `preflight self-review` 并写回 `STATUS.md`
- 最小 `preflight self-review` 检查项：
  ...
  - 是否出现新的执行前问题，无法直接从父 roadmap / plan / design 输入中解出
...
6. 满足后：
   ...
   - 如果父 roadmap 允许自动续跑，且存在 `recommended_next_child`：优先自动切换到下一 child，而不是停下来等待额外确认
...
如果某项不存在，写 `n/a`，不要为了“完整叙述”补长段说明。`summaryPath` / `eventsPath` / `consolePath` 只要仓库提供，就应写入；如本轮要求新的 verification 数据，则 `verification_data.*` 也必须同步写入，保证后续恢复与复查可重新定位正式产物。
...
- 父 roadmap 边界、authority 或执行策略需要重写
