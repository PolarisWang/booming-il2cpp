...
这是本仓库 `subjects/test/Chaos.TestFramework/collection/runner/codegen` 主线的治理入口，不负责重复抄写全部 authority 文档。

本技能负责两件事：

- 识别当前任务是否命中测试治理 / AOT obligation 主线
- 强制任务回到正式 authority、planning intake 与 completion gate
...
- 本技能本身：
  - 只负责把任务路由到这些 authority，并要求计划和执行阶段消费它们
...
## 命中 AOT / IL2CPP / test-flow 任务时，先冻结 intake
...
- canonical proof / benchmark / host correctness 不允许依赖 `Console.WriteLine` 或 `ChaosEvidenceKind.Stdout`
- AOT onboarding 的深规则以 `wiki/06-测试验证/AOT新Feature接入自测规范.md` 为准

### 3. 测试阶段 `dotnet` 编译崩溃是阻断缺陷

在任何测试阶段，只要 `dotnet build` / `dotnet test` / `msbuild` 发生编译崩溃：

- 当前验证立即视为失败
- 必须保留并检查 `stderr`、`binlog`、崩溃堆栈或 dump 信息（如果可用）
- 必须按 `dev:systematic-debugging` 查明根因并修复
- 根因未修复前，不得继续归档、提交或宣称通过

### 4. pipeline / codegen 不变式

- 正式主线固定为 `managed solution -> collector -> collection -> managed/native/hotupdate hosts`
- native / hotupdate 专有绑定必须经 manifest 分层，不污染 collection contract
