...
- 检查项只聚焦是否还能安全自动继续：
  - 当前 child 范围是否仍落在父 roadmap 的 `resolved_decisions`、范围边界与非目标内
  - 是否出现新的边界漂移、authority 变化、职责变化或关键结构改动，导致父 roadmap 需要重写
  - `depends_on`、父阶段依赖、`requiredGates` 与 obligation 前提是否已经满足
  - 是否出现新的执行前问题，无法直接从父 roadmap 或设计输入中解出
  - 是否新增 formal object、proof、benchmark、hotupdate 或其它必须先确认的义务
  - 当前 `ACTIVE.md`、父子任务指针与目录状态是否存在冲突
- 结果只能写成：
...
- 同时在 `STATUS.md` 记录：
  - `preflight_summary`
  - `auto_execution_decision: continue|stop`
- 判定规则：
  - `pass`：没有实质风险，自动继续
  - `warn`：只有实现级风险、结构观察项或可在执行中处理的事项；记录后自动继续
  - `stop`：存在新的执行前问题、边界/authority 漂移、依赖未满足、义务新增或需要修改父 roadmap；必须停下来和用户确认
- roadmap-child 在 `preflight_review = pass|warn` 时，不重新回到完整 `brainstorming`
- roadmap-child 只有在 `preflight_review = stop` 时，才返回 `brainstorming` 或回父 roadmap 修正
