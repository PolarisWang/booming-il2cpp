...
{
    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonArrayAndMemoryMarshalFamilyCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (!buildContext.SubjectId.Contains("/System.Array::", StringComparison.Ordinal) &&
            !buildContext.SubjectId.Contains("/System.Runtime.InteropServices.MemoryMarshal::", StringComparison.Ordinal))
        {
            return RuntimeSkeletonFamilyHandlerResult.NoMatch;
        }

        var result = TryBuildRuntimeSkeletonArrayAndMemoryMarshalHandler(buildContext);
        return result.MatchKind == RuntimeSkeletonFamilyHandlerMatchKind.Match
            ? result
            : RuntimeSkeletonFamilyHandlerResult.CreateUnsupported("array-memorymarshal-family-unsupported-shape");
    }

    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonArrayAndMemoryMarshalCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (TryBuildAssemblyBoundArrayAndMemoryMarshalStub(
    ...
    private static bool TryBuildAssemblyBoundArrayAndMemoryMarshalCore(
        string assemblyName,
        string subjectId,
        ...
        out string stub)
    {
        ...
        if (!methodsBySubjectId.TryGetValue(subjectId, out var method) ||
            !RuntimeSkeletonArrayAndMemoryMarshalAbi.TryCreate(method, out var abi))
        {
            return false;
        }
        ...
        stub = ScribanTemplateRenderer.RenderTemplate(
            ScribanTemplateRenderer.LoadTemplate(NativeReferenceProofCatalog.RuntimeSkeletonArrayAndMemoryMarshalStubTemplateRelativePath),
            model);
        return true;
    }
}
