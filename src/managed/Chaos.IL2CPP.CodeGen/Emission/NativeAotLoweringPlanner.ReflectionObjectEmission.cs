...
public sealed partial class NativeAotLoweringPlanner
{
	private void EmitReflectionObjectHelpers(StringBuilder builder, IReadOnlyList<AotCoreIrMethodArtifact> reachableMethods, IReadOnlySet<string> referenceTypeSubjectIds, IReadOnlySet<string> boxedTypeSubjectIds)
	{
		...
		foreach (CustomAttributeMaterializationPlan item15 in _customAttributeSupport.Materializations.OrderBy<CustomAttributeMaterializationPlan, string>((CustomAttributeMaterializationPlan plan) => plan.TargetSubjectId, StringComparer.Ordinal).ThenBy<CustomAttributeMaterializationPlan, string>((CustomAttributeMaterializationPlan plan) => plan.AttributeTypeSubjectId, StringComparer.Ordinal))
		{
			...
			if (_vtableTypes?.Contains(item15.AttributeTypeSubjectId) == true)
			{
				stringBuilder = builder;
				StringBuilder stringBuilder_vt1 = stringBuilder;
				handler = new StringBuilder.AppendInterpolatedStringHandler(41, 1, stringBuilder);
				handler.AppendLiteral("        chaos_attribute->header.vtable = ");
				handler.AppendFormatted(GetNativeVTableSymbol(item15.AttributeTypeSubjectId));
				handler.AppendLiteral(";");
				stringBuilder_vt1.AppendLine(ref handler);
			}
			...
			builder.AppendLine("        return reinterpret_cast<CHAOS_IL2CPP_INTPTR>(chaos_attribute);");
			builder.AppendLine("    }");
			builder.AppendLine();
		}
		builder.AppendLine("    return static_cast<CHAOS_IL2CPP_INTPTR>(0);");
