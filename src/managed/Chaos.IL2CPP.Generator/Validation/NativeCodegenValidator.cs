...
public sealed class NativeCodegenValidator
{
    ...
    public ValidationResult ValidateContent(string content, string fileName, ValidationResult? result = null)
    {
        ...
        // Skip auto-generated headers (first line starts with // Auto-generated or // AUTO-GENERATED).
        if (fileName.EndsWith(".h", StringComparison.Ordinal) && !fileName.StartsWith("chaos/", StringComparison.Ordinal))
        {
            ...
            var firstLine = headerLines.Length > 0 ? headerLines[0].Trim() : "";
            var isAutoGenerated = firstLine.StartsWith("// Auto-generated", StringComparison.OrdinalIgnoreCase);
            var hasGuard = !isAutoGenerated && IncludeGuardRegex.IsMatch(headerLines[0]);
            if (!hasGuard && !isAutoGenerated)
            {
