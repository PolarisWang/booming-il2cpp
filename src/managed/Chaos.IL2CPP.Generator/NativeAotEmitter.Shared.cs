...
public sealed partial class NativeAotEmitter
{
    ...
    private static string BuildSharedHeader(NativeAotTemplateModel templateModel)
    {
        ...
        {
            sb.Append("// Auto-generated by chaos-il2cpp convert-to-cpp\n#pragma once\n");
        }
        else
        {
            sb.Append(code);
        ...
        // payload TU that defines them and page 0, whose shell calls them. The
        // payload preambles get these declarations via
        // WrapPayloadSectionInTranslationUnit, but page 0 does not take that path
        // — it includes this header. Without them page 0's call site has no
        // declaration and MSVC reports C3861 "identifier not found" for every
        // part, which reads like a missing definition rather than a missing
        // include. Page 0 always includes this header, so emitting here covers
        // both sides.
        if (templateModel.CrossSectionSymbols is { Count: > 0 })
        {
