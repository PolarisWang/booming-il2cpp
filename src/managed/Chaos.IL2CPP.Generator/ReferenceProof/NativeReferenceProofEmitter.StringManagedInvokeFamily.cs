...
{
    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonStringManagedInvokeFamilyCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        ...
        var result = TryBuildRuntimeSkeletonStringManagedInvokeHandler(buildContext);
        return result.MatchKind == RuntimeSkeletonFamilyHandlerMatchKind.Match
            ? result
            : RuntimeSkeletonFamilyHandlerResult.CreateUnsupported("string-managed-invoke-unsupported-shape");
    }

    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonStringManagedInvokeCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (TryBuildAssemblyBoundStringManagedInvokeStub(
    ...
    private static bool TryBuildAssemblyBoundStringManagedInvokeCore(
        string assemblyName,
        string subjectId,
        ...
        out string stub)
    {
        ...
        if (!methodsBySubjectId.TryGetValue(subjectId, out var method) ||
            !RuntimeSkeletonStringManagedInvokeAbi.TryCreate(method, out var abi))
        {
            return false;
        }
        ...
        var model = new ScriptObject
        {
            ...
            ["return_buffer_declaration"] = abi.ReturnShape.BufferDeclaration,
            ["return_value_argument"] = abi.ReturnShape.ReturnValueArgument,
            ["return_value_size"] = abi.ReturnShape.ReturnValueSizeExpression,
            ["return_assignment_statement"] = abi.ReturnShape.AssignmentStatement,
        };
        stub = ScribanTemplateRenderer.RenderTemplate(
            ScribanTemplateRenderer.LoadTemplate(NativeReferenceProofCatalog.RuntimeSkeletonStringManagedInvokeStubTemplateRelativePath),
            model);
        return true;
    }
}
