...
{
    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonUnsafeManagedInvokeFamilyCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (!buildContext.SubjectId.Contains("/System.Runtime.CompilerServices.Unsafe::", StringComparison.Ordinal))
        {
            return RuntimeSkeletonFamilyHandlerResult.NoMatch;
        }

        var result = TryBuildRuntimeSkeletonUnsafeManagedInvokeHandler(buildContext);
        return result.MatchKind == RuntimeSkeletonFamilyHandlerMatchKind.Match
            ? result
            : RuntimeSkeletonFamilyHandlerResult.CreateUnsupported("unsafe-managed-invoke-unsupported-shape");
    }

    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonUnsafeManagedInvokeCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (TryBuildAssemblyBoundUnsafeManagedInvokeStub(
    ...
    private static bool TryBuildAssemblyBoundUnsafeManagedInvokeCore(
        string assemblyName,
        string subjectId,
        ...
        out string stub)
    {
        ...
        if (!methodsBySubjectId.TryGetValue(subjectId, out var method) ||
            !RuntimeSkeletonUnsafeManagedInvokeAbi.TryCreate(method, out var abi))
        {
            return false;
        }
        ...
        var model = new ScriptObject
        {
            ...
            ["return_value_argument"] = abi.ReturnShape.ReturnValueExpression,
