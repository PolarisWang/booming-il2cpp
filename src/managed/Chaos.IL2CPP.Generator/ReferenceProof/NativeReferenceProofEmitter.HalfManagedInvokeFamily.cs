...
{
    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonHalfManagedInvokeFamilyCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (!buildContext.SubjectId.Contains("/System.Half::", StringComparison.Ordinal))
        {
            return RuntimeSkeletonFamilyHandlerResult.NoMatch;
        }

        if (TryBuildRuntimeSkeletonHalfIdentityForwarderCore(buildContext) is { MatchKind: RuntimeSkeletonFamilyHandlerMatchKind.Match } identityResult)
        {
            return identityResult;
        }

        var result = TryBuildRuntimeSkeletonHalfManagedInvokeHandler(buildContext);
        return result.MatchKind == RuntimeSkeletonFamilyHandlerMatchKind.Match
            ? result
            : RuntimeSkeletonFamilyHandlerResult.CreateUnsupported("half-managed-invoke-unsupported-shape");
    }

    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonHalfManagedInvokeCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (TryBuildAssemblyBoundHalfManagedInvokeStub(
    ...
    private static bool TryBuildAssemblyBoundHalfManagedInvokeCore(
        string assemblyName,
        string subjectId,
        ...
        out string stub)
    {
        ...
        if (!methodsBySubjectId.TryGetValue(subjectId, out var method) ||
            !RuntimeSkeletonHalfManagedInvokeAbi.TryCreate(method, out var abi))
        {
            return false;
        }
        ...
        stub = ScribanTemplateRenderer.RenderTemplate(
            ScribanTemplateRenderer.LoadTemplate(NativeReferenceProofCatalog.RuntimeSkeletonHalfManagedInvokeStubTemplateRelativePath),
            model);
        return true;
    }

    private static RuntimeSkeletonFamilyHandlerResult TryBuildRuntimeSkeletonHalfIdentityForwarderCore(
        RuntimeSkeletonStubBuildContext buildContext)
    {
        if (!string.Equals(
                buildContext.SubjectId,
                "System.Private.CoreLib/System.Half::op_UnaryPlus:System.Half(System.Half)",
