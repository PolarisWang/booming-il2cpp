...
public sealed partial class NativeAotLoweringPlanner
{
    ...
    /// <summary>
    /// Emit T4 JIT method entry table for T4 JIT compilation dispatch.
    /// Generated when codegen mode is Jit. Produces:
    /// <code>
    /// extern "C" const JitT4Entry kChaosT4JitEntries[] = { ... };
    /// extern "C" const uint32_t kChaosT4JitEntryCount = N;
    /// </code>
    ...
    /// this builds entries for T4 JIT compilation — first call triggers JitStubDispatchImpl
    /// → Compile() → direct_ptr patched to compiled native code.
    /// </summary>
    internal string BuildT4JitMethodRegistration(
        ...
        MetadataRegistrationArtifact metadataRegistration)
    {
        ...
        var sb = new StringBuilder(4096);
        sb.AppendLine("// --- T4 JIT Method Entry Table ---");
        sb.AppendLine("// Auto-generated by chaos-il2cpp codegen for --mode jit (T4 path).");
        sb.AppendLine("#include <cstdint>");
        sb.AppendLine("#include \"jit_registration.h\"");
        sb.AppendLine();

        // Serialize each method to JSON and emit as C++ string literal + JitT4Entry.
        ...
        for (int i = 0; i < methodsForLowering.Count; i++)
        {
            ...
            sb.Append("static const char kT4Json_")
                .Append(i)
                .Append("[] = ");
        ...
        // Emit JitT4Entry array
        sb.Append("extern \"C\" const JitT4Entry kChaosT4JitEntries[")
        ...
        // Emit count symbol
        sb.Append("extern \"C\" const uint32_t kChaosT4JitEntryCount = ")
            .Append(methodsForLowering.Count)
            .AppendLine("u;");
    ...
    internal string BuildHybridMethodRegistration(
        IReadOnlyList<AotCoreIrMethodArtifact> methodsForLowering,
        MetadataRegistrationArtifact metadataRegistration)
    {
        if (methodsForLowering == null || methodsForLowering.Count == 0)
        ...
        var sb = new StringBuilder(4096);
        sb.AppendLine("// --- Hybrid Method Entry Table ---");
        sb.AppendLine("// Auto-generated by chaos-il2cpp codegen for --mode hybrid.");
        sb.AppendLine("#include <cstdint>");
        sb.AppendLine("#include \"jit_registration.h\"");
        sb.AppendLine();

        // Serialize each method to JSON
        var jsonOptions = new JsonSerializerOptions
        {
            PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
            DefaultIgnoreCondition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull,
        ...
        {
            string escaped = EscapeCppStringLiteral(jsonStrings[i]);
            sb.Append("static const char kHybridJson_")
                .Append(i)
                .Append("[] = ");
            if (escaped.Length <= maxStringLiteralLen)
            {
        ...
        // Emit HybridT4Entry array
        sb.Append("extern \"C\" const HybridT4Entry kChaosHybridEntries[")
            .Append(methodsForLowering.Count)
            .AppendLine("] =");
        sb.AppendLine("{");
        for (int i = 0; i < methodsForLowering.Count; i++)
        ...
        // Emit count symbol
        sb.Append("extern \"C\" const uint32_t kChaosHybridEntryCount = ")
            .Append(methodsForLowering.Count)
            .AppendLine("u;");
