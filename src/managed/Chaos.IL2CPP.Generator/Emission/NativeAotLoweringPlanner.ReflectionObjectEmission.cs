...
public sealed partial class NativeAotLoweringPlanner
{
	private void EmitReflectionObjectHelpers(StringBuilder builder, IReadOnlyList<AotCoreIrMethodArtifact> reachableMethods, IReadOnlySet<string> referenceTypeSubjectIds, IReadOnlySet<string> boxedTypeSubjectIds)
	{
		...
		// ── Receiver normalization for the managed Type object model ──
		//
		// The A2.7 typeof fold pushes a raw TypeInfoHot* (chaos_mt_X.AsTypeInfoHot()),
		// but functions below reinterpret their receivers as managed
		// chaos_type_System_Type* (header + runtime_type_handle).  Reading a raw
		// pointer yields a garbage handle → GetMethod/GetProperty return 0.
		//
		// chaos_reflection_as_managed_type normalizes both representations:
		// a managed object's first qword is its header's TypeInfoHot* (non-null),
		// while a MethodTable's first field is null by definition
		// ({nullptr, nullptr, stable_id, …}).  A raw TypeInfoHot* is mapped through
		// its 64-bit stable_id to the reflection pseudo type handle and a TLS-cached
		// managed Type object is materialized.  Unknown stable_ids fall through to
		// the legacy raw reinterpret — coercion must never turn a previously-clean
		// failure into a null dereference (v1 returned nullptr there and produced
		...
		builder.AppendLine("    }");
		builder.AppendLine("    if (*reinterpret_cast<void* const*>(chaos_value) != nullptr)");
		builder.AppendLine("    {");
		builder.AppendLine("        // Managed object: header.type_info is live — reinterpret directly.");
		builder.AppendLine("        return reinterpret_cast<chaos_type_System_Private_CoreLib_System_Type*>(chaos_value);");
		builder.AppendLine("    }");
		builder.AppendLine("    // Raw TypeInfoHot* (MethodTable, first field null): resolve stable_id →");
		builder.AppendLine("    // reflection type handle → TLS-cached managed Type object.");
		builder.AppendLine("    const auto* chaos_ti = reinterpret_cast<const TypeInfoHot*>(chaos_value);");
		builder.AppendLine("    const auto chaos_handle = chaos_reflection_type_handle_from_stable_id(chaos_ti->stable_id);");
		builder.AppendLine("    if (chaos_handle == 0)");
		builder.AppendLine("    {");
		builder.AppendLine("        // Unknown stable_id: preserve the legacy raw reinterpret — downstream");
		builder.AppendLine("        // reads a garbage handle and fails cleanly (caught), never dereferences null.");
		builder.AppendLine("        return reinterpret_cast<chaos_type_System_Private_CoreLib_System_Type*>(chaos_value);");
		builder.AppendLine("    }");
		builder.AppendLine("    return reinterpret_cast<chaos_type_System_Private_CoreLib_System_Type*>(");
		builder.AppendLine("        chaos_reflection_create_type_value(chaos_handle));");
		builder.AppendLine("}");
		builder.AppendLine();
		...
		if (RequiresManagedStringFactory(reachableMethods))
		{
			...
			builder.AppendLine("        chaos_left_string_value == 0");
			builder.AppendLine("            ? static_cast<CHAOS_IL2CPP_SIZE>(0)");
			builder.AppendLine("            : chaos_runtime_get_abi_v0()->is_string_id(chaos_left_string_value)");
			...
			builder.AppendLine("        chaos_right_string_value == 0");
			builder.AppendLine("            ? static_cast<CHAOS_IL2CPP_SIZE>(0)");
			builder.AppendLine("            : chaos_runtime_get_abi_v0()->is_string_id(chaos_right_string_value)");
			builder.AppendLine("                ? static_cast<CHAOS_IL2CPP_SIZE>(chaos::il2cpp::string_table::Resolve(");
			...
			builder.AppendLine("    if ((chaos_left_string_value != 0 && chaos_left_utf8 == nullptr && chaos_left_length != 0) ||");
			builder.AppendLine("        (chaos_right_string_value != 0 && chaos_right_utf8 == nullptr && chaos_right_length != 0))");
			builder.AppendLine("    {");
			...
			builder.AppendLine("    chaos_str->length = static_cast<CHAOS_IL2CPP_INTPTR>(chaos_combined_length);");
			builder.AppendLine("    chaos_str->string_id = 0;");
			builder.AppendLine("    chaos_str->utf8_data = chaos_raw + sizeof(chaos_type_System_Private_CoreLib_System_String);");
			builder.AppendLine();
		...
		if (flag6)
		{
			...
			builder.AppendLine("{");
			builder.AppendLine("    if (chaos_handler_ref == 0)");
			builder.AppendLine("    {");
			builder.AppendLine("        CHAOS_IL2CPP_FAIL();");
			builder.AppendLine("    }");
			builder.AppendLine();
			...
			builder.AppendLine("    {");
			builder.AppendLine("        if (chaos_runtime_get_abi_v0()->is_string_id(chaos_fragment))");
			builder.AppendLine("        {");
			builder.AppendLine("            const auto chaos_view = chaos::il2cpp::string_table::Resolve(");
		...
		if (flag2 || flag4 || flag5)
		{
			...
			builder.AppendLine("{");
			builder.AppendLine("    const auto chaos_total_size = sizeof(chaos_managed_array) + chaos_length * sizeof(CHAOS_IL2CPP_INTPTR);");
			builder.AppendLine("    auto* chaos_array = static_cast<chaos_managed_array*>(CHAOS_IL2CPP_MALLOC_GC(chaos_total_size));");
			...
			handler.AppendLiteral("    auto* chaos_type = CHAOS_IL2CPP_NEW_GC(");
			...
			{
				builder.Append("    if (chaos_method_handle == ");
				builder.Append(GetMethodHandleLiteral(methodEntry.MethodSubjectId));
				builder.Append(") { return chaos_reflection_create_string_literal(");
				builder.Append(ToCppStringLiteral(methodEntry.MethodName));
				builder.AppendLine("); }");
			}
			builder.AppendLine("    return 0;");
			builder.AppendLine("}");
			...
			{
				builder.Append("    if (chaos_method_handle == ");
				builder.Append(GetMethodHandleLiteral(methodEntry2.MethodSubjectId));
				builder.Append(") { return ");
				builder.Append(methodEntry2.MetadataToken);
				builder.AppendLine("; }");
			}
			builder.AppendLine("    return 0;");
			builder.AppendLine("}");
			...
			if (flag5)
			{
				...
				builder.AppendLine("{");
				builder.AppendLine("    if (chaos_member_value == 0)");
				builder.AppendLine("    {");
				builder.AppendLine("        return 0;");
				builder.AppendLine("    }");
				builder.AppendLine();
				builder.AppendLine("    auto* chaos_header = reinterpret_cast<ThinLockableHeader*>(chaos_member_value);");
				stringBuilder = builder;
				...
				handler.AppendLiteral("        auto* chaos_type = chaos_reflection_as_managed_type(chaos_member_value);");
				stringBuilder20.AppendLine(ref handler);
				builder.AppendLine("        return chaos_type->runtime_name_value == 0");
				...
				builder.AppendLine("        return chaos_method->runtime_name_value == 0");
				...
				builder.AppendLine();
				builder.AppendLine("    auto* chaos_header = reinterpret_cast<ThinLockableHeader*>(chaos_member_value);");
				builder.AppendLine("    CHAOS_IL2CPP_INTPTR chaos_declaring_type_handle = 0;");
				stringBuilder = builder;
				...
				builder.AppendLine("    }");
				builder.AppendLine();
				builder.AppendLine("    return chaos_declaring_type_handle == 0");
				...
				builder.AppendLine("        const auto chaos_lookup_handle = chaos_method->generic_definition_method_handle != 0");
				...
				builder.AppendLine("{");
				builder.AppendLine("    if (chaos_parameter_value == 0)");
				builder.AppendLine("    {");
				builder.AppendLine("        return 0;");
				builder.AppendLine("    }");
				builder.AppendLine();
				...
				foreach (IGrouping<string, ReflectionMemberFieldEntry> item2 in _reflectionMemberSupport.FieldEntries.GroupBy<ReflectionMemberFieldEntry, string>((ReflectionMemberFieldEntry entry) => entry.DeclaringTypeSubjectId, StringComparer.Ordinal).OrderBy<IGrouping<string, ReflectionMemberFieldEntry>, string>((IGrouping<string, ReflectionMemberFieldEntry> group) => group.Key, StringComparer.Ordinal))
				{
					...
					foreach (ReflectionMemberFieldEntry item3 in item2.OrderBy<ReflectionMemberFieldEntry, string>((ReflectionMemberFieldEntry entry) => entry.FieldName, StringComparer.Ordinal))
					{
						...
						handler.AppendLiteral("                auto* chaos_field = CHAOS_IL2CPP_NEW_GC(");
						handler.AppendFormatted(GetNativeTypeSymbol("System.Private.CoreLib/System.Reflection.FieldInfo"));
						handler.AppendLiteral(");");
		...
		if (flag3)
		{
			...
			builder.AppendLine("    if (chaos_type_handle == 0)");
			builder.AppendLine("    {");
			builder.AppendLine("        return 0;");
			builder.AppendLine("    }");
			builder.AppendLine();
			...
			handler = new StringBuilder.AppendInterpolatedStringHandler(39, 1, stringBuilder);
			handler.AppendLiteral("    auto* chaos_assembly_name = CHAOS_IL2CPP_NEW_GC(");
		...
		if (flag2 || flag4 || flag5)
		{
			...
			builder.AppendLine();
			// stable_id → reflection pseudo type handle.  Same table as
			// chaos_reflection_get_object_type above, but keyed by the raw 64-bit
			// TypeInfoHot stable_id — consumed by chaos_reflection_as_managed_type.
			builder.AppendLine("CHAOS_IL2CPP_UINT64 chaos_reflection_type_handle_from_stable_id(CHAOS_IL2CPP_UINT64 chaos_stable_id) noexcept");
			builder.AppendLine("{");
			...
			handler.AppendLiteral("    auto* chaos_method = CHAOS_IL2CPP_NEW_GC(");
			handler.AppendFormatted(GetNativeTypeSymbol("System.Private.CoreLib/System.Reflection.MethodInfo"));
			handler.AppendLiteral(");");
			...
			builder.AppendLine("{");
			builder.AppendLine("    if (chaos_method_value == 0 || chaos_type_array_value == 0)");
			builder.AppendLine("    {");
			builder.AppendLine("        return 0;");
			builder.AppendLine("    }");
			builder.AppendLine();
			...
			builder.AppendLine("    if (chaos_type_array->length <= 0)");
			builder.AppendLine("    {");
			builder.AppendLine("        return 0;");
			builder.AppendLine("    }");
			builder.AppendLine();
			...
			handler = new StringBuilder.AppendInterpolatedStringHandler(39, 1, stringBuilder);
			handler.AppendLiteral("    auto* chaos_closed_method = CHAOS_IL2CPP_NEW_GC(");
			handler.AppendFormatted(GetNativeTypeSymbol("System.Private.CoreLib/System.Reflection.MethodInfo"));
			handler.AppendLiteral(");");
			...
			builder.AppendLine("    chaos_closed_method->generic_definition_method_handle = chaos_method->generic_definition_method_handle != 0");
			builder.AppendLine("        ? chaos_method->generic_definition_method_handle");
			builder.AppendLine("        : chaos_method->runtime_method_handle;");
			...
			builder.AppendLine("    chaos_closed_method->runtime_name_value = chaos_method->runtime_name_value == 0");
			...
			foreach (AotCoreIrMethodArtifact item13 in _genericStaticMethodCandidates)
			{
				...
				builder.AppendLine("    {");
				builder.AppendLine("        if (chaos_target_value != 0 || chaos_args == nullptr || chaos_args->length < static_cast<CHAOS_IL2CPP_INTPTR>(1))");
				builder.AppendLine("        {");
				builder.AppendLine("            return 0;");
				builder.AppendLine("        }");
				builder.AppendLine();
				...
				foreach (string item14 in boxedTypeSubjectIds.Where((string candidate) => !IsStructuredValueTypeSubjectId(candidate)).OrderBy<string, string>((string candidate) => candidate, StringComparer.Ordinal))
				{
					...
					handler.AppendLiteral("            auto* chaos_boxed_result = CHAOS_IL2CPP_NEW_GC(");
					handler.AppendFormatted(GetNativeBoxTypeSymbol(item14));
					handler.AppendLiteral(");");
		...
		builder.AppendLine("{");
		builder.AppendLine("    if (chaos_member_value == 0 || chaos_attribute_type_handle == 0)");
		builder.AppendLine("    {");
		builder.AppendLine("        return 0;");
		builder.AppendLine("    }");
		builder.AppendLine("");
