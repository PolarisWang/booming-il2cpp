...
public sealed record NativeAotTemplateModel
{
    ...
    /// <summary>
    /// JSON content of the shape-registry capability manifest.  Emitted next to
    /// the generated codegen output so the ATG injector and the fact classifier
    /// can distinguish a real implementation failure from a missing one.
    /// Empty string when the registry produced nothing.
    /// </summary>
    public string CapabilityManifestJson { get; init; } = "";
