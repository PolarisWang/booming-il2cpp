...
public sealed partial class NativeAotLoweringPlanner
{
    ...
    /// <summary>
    /// Set of callee SubjectIds that should use direct C++ calls instead of Demeter
    /// Table indirect dispatch. Populated from PGO profile data via <see cref="LoadGoldDirectCallProfile"/>.
    /// When a callee is in this set, <see cref="TryResolveDirectInvocationTarget"/> sets
    /// DirectNativeSymbol to the target's C++ function symbol, bypassing
    /// kChaosExternalRuntimeFnTable indirection.
    /// </summary>
    private HashSet<string> _goldDirectCallCache = new(StringComparer.Ordinal);

    /// <summary>
    /// Load Gold Direct Link profile data from a JSON file.
    /// Format:
    /// {
    ///   "hotMethods": [
    ...
    ///   ]
    /// }
    /// The profile data is typically generated by the runtime's PGO dump mechanism
    /// (see chaos_dump_gold_profile in fast_dispatch.cpp).
    /// </summary>
    public void LoadGoldDirectCallProfile(string profilePath)
    {
        if (!File.Exists(profilePath))
        {
            Console.Error.WriteLine($"[GoldDirectLink] Profile not found: {profilePath}");
            return;
        }
        try
        {
            ...
            if (root.TryGetProperty("hotMethods", out var methods) && methods.ValueKind == JsonValueKind.Array)
            {
                int count = 0;
                foreach (var method in methods.EnumerateArray())
                {
                    ...
                    {
                        _goldDirectCallCache.Add(callee);
                        count++;
                    }
                }
                Console.Error.WriteLine($"[GoldDirectLink] Loaded {count} hot methods from {profilePath}");
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[GoldDirectLink] Failed to load profile: {ex.Message}");
        }
    }

    /// <summary>
    /// Add a single callee SubjectId to the Gold Direct Link cache.
    /// Used for programmatic configuration (e.g., from CLI args or test configuration).
    /// </summary>
