/// native_verify_main.cpp — Fact Static: Self-verifying native AOT host.
///
...
/// fact_verifier.py from the managed entrypoint DLL).
///
/// On success: prints "Fact Static: N/M passed" and exits 0.
...
///      expected checksum from the managed FactStaticHarness reflection run.
///
/// Exception handling: methods that throw (e.g. Convert.ToChar(bool)) call
/// RaiseManagedException which needs the full runtime.  In Fact Static
/// verification mode the runtime is not initialized, so we register a setjmp/longjmp
...
int main() {
    ...
    for (int i = 0; i < kExpectedCount; i++) {
        ...
        if (threw) {
            CHAOS_IL2CPP_LOG_ERROR_M("FactStatic", "FAIL [{0}]: native threw, expected {1}",
                                     i, kExpectedChecksums[i]);
            return_value_failures++;
            continue;
        }
        if (actual != kExpectedChecksums[i]) {
            CHAOS_IL2CPP_LOG_ERROR_M("FactStatic", "FAIL [{0}]: expected {1}, got {2}",
    ...
    CHAOS_IL2CPP_LOG_INFO_M("FactStatic", "{0}/{1} passed (assert_failures={2}, return_failures={3})",
