/// fact_runtime_main.cpp — Fact Runtime: Execution Verification Host
///
...
///   3. Report Fact Runtime: N/M passed
///
...
int main() {
    ...
    init_params.host_name_utf8 = "fact-runtime-verify";
    ...
    if (abi == nullptr) {
        CHAOS_IL2CPP_LOG_ERROR("FactRuntime", "GetRuntimeAbiV0() returned null");
        return 1;
    }
    ...
    if (status != CHAOS_RUNTIME_STATUS_OK || runtime_state == nullptr) {
        CHAOS_IL2CPP_LOG_ERROR_M("FactRuntime", "RuntimeInit failed (status={0})", static_cast<int>(status));
        return 1;
    }
    ...
    if (status != CHAOS_RUNTIME_STATUS_OK || thread_state == nullptr) {
        CHAOS_IL2CPP_LOG_ERROR_M("FactRuntime", "ThreadAttach failed (status={0})", static_cast<int>(status));
    ...
    for (int i = 0; i < kExpectedCount; i++) {
        ...
        if (threw) {
            CHAOS_IL2CPP_LOG_ERROR_M("FactRuntime", "FAIL [{0}]: native threw, expected {1}",
                                     i, kExpectedChecksums[i]);
            return_value_failures++;
            continue;
        }
        if (actual != kExpectedChecksums[i]) {
            CHAOS_IL2CPP_LOG_ERROR_M("FactRuntime", "FAIL [{0}]: expected {1}, got {2}",
    ...
    CHAOS_IL2CPP_LOG_INFO_M("FactRuntime", "{0}/{1} passed (assert_failures={2}, return_failures={3})",
