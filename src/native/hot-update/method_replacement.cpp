...
bool Register(CHAOS_IL2CPP_UINT32 method_token, void* thunk) {
    ...
    // NOTE: original semantics (pre-P1-B-regression revert) — Register installs a
    // replacement for a method_token WITHOUT requiring that token to already exist in a
    // registered vtable.  Real hotpatch registers NEW replacement tokens (the very act
    // of Register is what binds a method to this patch); hard-failing when the token is
    // absent from a vtable breaks that (and broke ConcurrentRegister* / the smoke test).
