...
PatchContext* ApplyPatchFromMemoryEx(
    ...
    for (uint32_t i = 0; i < total; ++i) {
        ...
        const char* type_name = cache->GetTypeName(method_entry);
        const char* type_ns = cache->GetTypeNamespace(method_entry);
        const char* subject_method_name = cache->GetString(method_entry->name_offset);
        if (type_name == nullptr || subject_method_name == nullptr) continue;

        // Keep the patch DLL's own identity for the secondary dispatch below.
        // `subject_type_name`/`subject_method_name` describe the Subject_N entry
        // as declared in the patch assembly; the host overrides replace them with
        // the *original* AOT method that the subject wraps.
        const char* subject_type_name = type_name;
        const char* subject_ns = type_ns;

        const char* method_name = subject_method_name;
        ...
        // ── Secondary patch: the Subject_N dispatch entry itself ─────────
        // The overrides above retarget this entry onto the *original* AOT method
        // that Subject_N wraps, so the verification harness — which dispatches
        // the SUBJECT slot, not the original — would otherwise bypass the patch
        // entirely and observe an unchanged value (semanticChangedCount=0).
        //
        // Register a second PatchMethod for the subject's own dispatch entry so
        // the harness call lands on kHotpatchActive and executes the patched IL
        // (the sentinel body / synthetic LdcI4+Ret IR) through the interpreter.
        //
        // The Subject_N method lives under the HOST type (AutoGenerated.*Tests)
        // in the AOT dispatch table, NOT under the patch DLL's type (PatchSubjects).
        // So the lookup uses lookup_ns + lookup_type (the host overrides) but
        // subject_method_name (the original Subject_N name, not the override).
        //
        // Skipped when there was no override (no host arrays — Subject_N and
        // original method would be the same dispatch entry).
        if (has_per_method_overrides) {
            if (subject_type_name != nullptr && subject_method_name != nullptr) {
                uint64_t subj_lookup = registry.LookupMethod(
                    lookup_ns, lookup_type, subject_method_name);
                if (subj_lookup != 0) {
                    uint32_t subj_module = ExtractModuleId(subj_lookup);
                    uint32_t subj_token = ExtractToken(subj_lookup);
                    uint32_t subj_slot = registry.TokenToSlot(subj_module, subj_token);
                    if (subj_slot != ~0u && (subj_module != module_id || subj_slot != slot)) {
                        registry.SetPatchedBySlot(subj_module, subj_slot, true,
                                                  &patch_method, patch_domain_id_ex);
                        HOTPATCH_DIAG("DIAG[APFM-Ex]: secondary patch subject='%s' module=%u slot=%u\n",
                            subject_method_name, subj_module, subj_slot);
                    }
                } else {
                    HOTPATCH_DIAG("DIAG[APFM-Ex]: subject lookup miss for '%s' ns='%s' type='%s'\n",
                        subject_method_name,
                        lookup_ns ? lookup_ns : "",
