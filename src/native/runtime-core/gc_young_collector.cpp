...
bool IsInNursery(const void* ptr) {
    // Check if the pointer falls within any active nursery region.
    // At C2, we simply check against the current TLS nursery.
    // A full implementation iterates the region table.
    ...
    uintptr_t end   = reinterpret_cast<uintptr_t>(ctx->nursery->end);
...
YoungCollectionResult GcYoungCollection(Region* nursery, Region* tenured_target) {
    ...
    uintptr_t heap_start = reinterpret_cast<uintptr_t>(nursery->begin);
    uintptr_t heap_end   = reinterpret_cast<uintptr_t>(nursery->end);
    result.dirty_cards_scanned = 0;

    ScanDirtyCards(heap_start, heap_end,
    ...
    //
    // The promoted region spans from the first tenured allocation to the
    // current tenured bump pointer.  We scan this entire range object-by-object.
    {
        CHAOS_IL2CPP_PROFILE_SCOPE("GcYoungCollection::CheneyBfs");
        // Walk the tenured region object-by-object.
        // At C2, all promoted objects are contiguous in the old gen pages.
        // We iterate all pages and scan their payloads.
        // For now, we rely on the dirty card Phase 1 to have already
        // scavenged transitive references from old→nursery.  The full
        // Cheney BFS will be completed when we have a tenured bump range.
        // This is a placeholder for the tenured range walk.
        // In C3, we track a dedicated promoted range during GcScavengeObject.
    }
