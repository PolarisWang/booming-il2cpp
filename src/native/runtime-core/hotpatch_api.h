...
#define CHAOS_PATCH_ERR_NO_METHODS          -3   // No methods matched the runtime
                                                // method registry (the patch blob
                                                // may target methods not available
...
typedef struct {
    ...
    uint8_t  status;           ///< 0=active, 1=reverted, 2=partial_fail (some methods
                               ///  in this entry failed to revert -- does NOT represent
                               ///  a partial apply state, which transactional guarantees
                               ///  prevent).
...
/// @param data   Pointer to the .patchdata binary.  The API copies the data
///               internally; the caller may release or reuse data immediately
///               after ChaosApplyPatch returns.
...
///               Note: ChaosApplyPatch never returns token 0 (the internal
///               counter starts at 1), so token 0 is always the "revert all"
///               sentinel and never conflicts with a real patch token.
...
/// only the first max_patches entries are written.
/// @param max_patches  Capacity of out_info array (0 = query count only).
/// @param out_info     Output buffer, or NULL if max_patches==0.
/// @return Total number of active patches.  If > max_patches, only the first
///         max_patches entries are written to out_info (truncated).  Callers
///         should call with max_patches=0 to get the count, then allocate
///         and call again.
