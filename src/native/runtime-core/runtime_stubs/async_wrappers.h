// ── C++20 Awaiter Wrapper Templates (F8) ──
// Adapts .NET TaskAwaiter/ValueTaskAwaiter/YieldAwaitable to C++20 awaitable.
#pragma once
...
template<typename P> struct ValueTaskAwaiterWrapper {
    CHAOS_IL2CPP_INTPTR _t;
    ValueTaskAwaiterWrapper(CHAOS_IL2CPP_INTPTR t) noexcept : _t(t) {}
    bool await_ready() noexcept { return chaos_async_valuetask_is_completed(_t) != 0; }
    void await_suspend(std::coroutine_handle<P> h) noexcept { chaos_async_valuetask_on_completed(_t, h.address()); }
    void await_resume() { chaos_async_valuetask_get_result(_t); }
};
