...
// Each entry is [lo, hi] inclusive.  Tables are sorted by lo.
// Generated from Unicode categories for BMP (0x0000-0xFFFF).
// Supplementary planes (>0xFFFF) return false.

struct CharRange { CHAOS_IL2CPP_INT32 lo; CHAOS_IL2CPP_INT32 hi; };

// Nd (Decimal Digit Number) — non-ASCII ranges
static constexpr CharRange kDigitRanges[] = {
    ...
    {0x19D0, 0x19D9}, {0x1B50, 0x1B59}, {0x1BB0, 0x1BB9},
    {0x1C40, 0x1C49}, {0x1C50, 0x1C59}, {0xA620, 0xA629},
    {0xA8D0, 0xA8D9}, {0xA900, 0xA909}, {0xAA50, 0xAA59},
    {0xFF10, 0xFF19},
};

// Letter categories (Lu, Ll, Lt, Lm, Lo) — non-ASCII ranges
// Covers Latin Extended, Greek, Cyrillic, Arabic, CJK, etc.
static constexpr CharRange kLetterRanges[] = {
    ...
    {0x02EE, 0x02EE}, {0x0370, 0x0377}, {0x037A, 0x037D},
    {0x0386, 0x0386}, {0x0388, 0x038A}, {0x038C, 0x038C},
    {0x038E, 0x03A1}, {0x03A3, 0x03F5}, {0x03F7, 0x0481},
    {0x048A, 0x052F}, {0x0531, 0x0556}, {0x0559, 0x0559},
    {0x0561, 0x0587}, {0x05D0, 0x05EA}, {0x05F0, 0x05F2},
    {0x0620, 0x064A}, {0x066E, 0x066F}, {0x0671, 0x06D3},
    {0x06D5, 0x06D5}, {0x06E5, 0x06E6}, {0x06EE, 0x06EF},
    {0x06FA, 0x06FC}, {0x06FF, 0x06FF}, {0x0710, 0x0710},
    {0x0712, 0x072F}, {0x074D, 0x07A5}, {0x07B1, 0x07B1},
    {0x07CA, 0x07EA}, {0x07F4, 0x07F5}, {0x07FA, 0x07FA},
    {0x0800, 0x0815}, {0x081A, 0x081A}, {0x0824, 0x0824},
    {0x0828, 0x0828}, {0x0840, 0x0858}, {0x08A0, 0x08B4},
    ...
    {0x0CBD, 0x0CBD}, {0x0CDE, 0x0CDE}, {0x0CE0, 0x0CE1},
    {0x0CF1, 0x0CF2}, {0x0D05, 0x0D0C}, {0x0D0E, 0x0D10},
    {0x0D12, 0x0D3A}, {0x0D3D, 0x0D3D}, {0x0D4E, 0x0D4E},
    {0x0D5F, 0x0D61}, {0x0D7A, 0x0D7F}, {0x0D85, 0x0D96},
    {0x0D9A, 0x0DB1}, {0x0DB3, 0x0DBB}, {0x0DBD, 0x0DBD},
    {0x0DC0, 0x0DC6}, {0x0E01, 0x0E30}, {0x0E32, 0x0E33},
    ...
    {0x0EAD, 0x0EB0}, {0x0EB2, 0x0EB3}, {0x0EBD, 0x0EBD},
    ...
    {0x17D7, 0x17D7}, {0x17DC, 0x17DC}, {0x1820, 0x1877},
    ...
    {0x1C4D, 0x1C4F}, {0x1C5A, 0x1C7D}, {0x1CE9, 0x1CEC},
    {0x1CEE, 0x1CF1}, {0x1CF5, 0x1CF6}, {0x1D00, 0x1DBF},
    ...
    {0x24B6, 0x24E9}, {0x2C00, 0x2C2E}, {0x2C30, 0x2C5E},
    ...
    {0x30A1, 0x30FA}, {0x30FC, 0x30FF}, {0x3105, 0x312D},
    {0x3131, 0x318E}, {0x31A0, 0x31BA}, {0x31F0, 0x31FF},
    {0x3400, 0x4DB5}, {0x4E00, 0x9FD5}, {0xA000, 0xA48C},
    ...
    {0xA78B, 0xA7AD}, {0xA7B0, 0xA7B7}, {0xA7F7, 0xA801},
    {0xA803, 0xA805}, {0xA807, 0xA80A}, {0xA80C, 0xA822},
    {0xA840, 0xA873}, {0xA882, 0xA8B3}, {0xA8F2, 0xA8F7},
    {0xA8FB, 0xA8FB}, {0xA8FD, 0xA8FD}, {0xA90A, 0xA925},
    ...
    {0xAB5C, 0xAB65}, {0xAB70, 0xABE2}, {0xAC00, 0xD7A3},
    ...
    {0xFE70, 0xFE74}, {0xFE76, 0xFEFC}, {0xFF21, 0xFF3A},
    {0xFF41, 0xFF5A}, {0xFF66, 0xFFBE}, {0xFFC2, 0xFFC7},
    {0xFFCA, 0xFFCF}, {0xFFD2, 0xFFD7}, {0xFFDA, 0xFFDC},
};

// Space Separator (Zs) + special whitespace codepoints
static constexpr CharRange kWhiteSpaceRanges[] = {
    {0x0085, 0x0085},  // NEL
    {0x00A0, 0x00A0},  // NBSP
...
inline bool InRanges(CHAOS_IL2CPP_INT32 c, const CharRange* ranges, CHAOS_IL2CPP_INT32 count) {
    CHAOS_IL2CPP_INT32 lo = 0, hi = count;
    while (lo < hi) {
        CHAOS_IL2CPP_INT32 mid = (lo + hi) >> 1;
        if (c < ranges[mid].lo) { hi = mid; continue; }
        if (c > ranges[mid].hi) { lo = mid + 1; continue; }
        return true;
    }
    return false;
}
...
CHAOS_IL2CPP_INT32 ChaosCharIsDigit(CHAOS_IL2CPP_INT32 c) noexcept
{
    ...
    return InRanges(c, kDigitRanges, sizeof(kDigitRanges) / sizeof(kDigitRanges[0])) ? 1 : 0;
}
...
CHAOS_IL2CPP_INT32 ChaosCharIsLetter(CHAOS_IL2CPP_INT32 c) noexcept
{
    ...
    if (c < 0x00AA || c > 0xFFDC) return 0;  // beyond BMP letter ranges
    if (c >= 0x3400 && c <= 0x9FD5) return 1;  // CJK — single large range fast path
    if (c >= 0xAC00 && c <= 0xD7A3) return 1;  // Hangul — single large range fast path
    return InRanges(c, kLetterRanges, sizeof(kLetterRanges) / sizeof(kLetterRanges[0])) ? 1 : 0;
}

CHAOS_IL2CPP_INT32 ChaosCharIsWhiteSpace(CHAOS_IL2CPP_INT32 c) noexcept
{
    // ASCII fast path
    if (c == 0x20 || c == 0x09 || c == 0x0A || c == 0x0D || c == 0x0C) return 1;
    if (c < 0x0085 || c > 0x3000) return 0;  // beyond BMP whitespace ranges
    return InRanges(c, kWhiteSpaceRanges, sizeof(kWhiteSpaceRanges) / sizeof(kWhiteSpaceRanges[0])) ? 1 : 0;
}
