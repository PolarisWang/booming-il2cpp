...
//
// Simple wrapper around std::malloc.  In later stages this may be
...
// ======================================================================
// RegionAllocate — unified allocation entry point
//
// Routes to the correct allocator based on RegionKind.
// ======================================================================

inline void* RegionAllocate(CHAOS_IL2CPP_SIZE size, RegionKind kind,
                            CHAOS_IL2CPP_UINT32 domain_id = 0) {
    switch (kind) {
    case RegionKind::REGION_NURSERY:
        return NurseryAllocate(size);
    case RegionKind::REGION_DOMAIN:
        return DomainAllocate(domain_id, size);
    case RegionKind::REGION_RAW:
        return RawAllocate(size);
    default:
        // TENURED / FOH allocations go through the GC path directly.
        return NurseryAllocate(size);
    }
}
