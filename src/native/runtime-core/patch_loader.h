...
struct PatchMethod {
    ...
    // code instead of going through the interpreter. Set by ApplyPatchFromMemory
...
// Extended ApplyPatchFromMemory with namespace and per-method type name overrides.
// Maps Subject_N methods (patch DLL MethodDef index) to their AOT
// HotpatchNameRegistry entries.  Parameters:
//   data, size          — .patchdata binary blob
//   host_type_ns        — override namespace for ALL methods (e.g. "AutoGenerated.System_Private_CoreLib")
//   host_type_names     — per-method type name overrides (indexed by MethodDef index).
//                         null entries fall through to host_type_name or patch DLL metadata.
//   host_method_names   — per-method method name overrides (indexed by MethodDef index).
//                         null entries fall through to patch DLL metadata.
//   method_count        — length of host_type_names and host_method_names arrays.
//                         Must match the patch DLL's MethodDef count.
// Returns a PatchContext (must be freed via Unpatch or delete), or nullptr on failure.
