...
void TierManager::ThreadProc(void* arg) noexcept {
    ...
    // BackgroundLoop uses C++ EH (unique_lock, etc.), so __try wrapping
    // at this level triggers MSVC C2712.  SEH protection for compilation
    // call sites is in the separate tier_manager_seh.cpp TU.
    self->BackgroundLoop();
}
