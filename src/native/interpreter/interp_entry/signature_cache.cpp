...
static void CacheSignature(PatchMethod* patch_method) noexcept {
    ...
    if (patch_method->signature_blob != nullptr && patch_method->signature_len > 1) {
        ...
        // Parse return type.
        patch_method->cached_ret_tag = static_cast<uint8_t>(ElementTypeToValueTag(ptr[0]));
        ptr += 1;
        remaining -= 1;
    ...
    if (patch_method->aot_core_ir_json == nullptr)
        return;
    ...
    uint32_t total_args = is_static ? static_cast<uint32_t>(count) : static_cast<uint32_t>(count) + 1;
