...
// Uses hash maps for O(1) lookup instead of O(n) linear strcmp scans.
// Hash maps are populated at registration time.

#include <chaos/unordered_dense.h>

static CHAOS_IL2CPP_UNORDERED_DENSE_MAP(std::string, const void*) s_direct_fn_map;
...
extern "C" void ChaosRegisterDirectFnTable(const char* const* subjects, const void* const* table, int count) noexcept {
    ...
    for (int i = 0; i < count; ++i) {
        if (subjects[i] != nullptr && table[i] != nullptr) {
            s_direct_fn_map[std::string(subjects[i])] = table[i];
        }
    }
}
...
static void* ResolveDirectFn(const char* subject_id, void* /*user_data*/) noexcept {
    ...
    {
        auto it = s_direct_fn_map.find(std::string(subject_id));
        if (it != s_direct_fn_map.end()) {
            CHAOS_IL2CPP_LOG_DEBUG("ResolveDirectFn: hash map hit for '%s'", subject_id);
    ...
    // to the native AOT direct_ptr registered during BootstrapRuntime.
    {
        ...
        // Step 3: ExternalRuntimeFnTable hash map lookup (O(1)) — populated
        // at startup by InitExternalRuntimeFnTable.
        if (!s_external_runtime_map_initialized) {
            ...
            s_external_runtime_map_initialized = true;
        }
        {
            auto it = s_external_runtime_map.find(std::string(subject_id));
            if (it != s_external_runtime_map.end()) {
                void* fn = it->second;
                if (fn != nullptr) {
