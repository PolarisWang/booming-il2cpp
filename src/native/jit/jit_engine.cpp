...
private:
    ...
    bool EmitSimd(const interpreter::RegisterInstruction& instr,
                  uint8_t simd_op, uint8_t elem_type, uint16_t simd_imm) noexcept;
...
void NativeCodeGenerator::LoadGpr(uint8_t x64_reg, uint32_t vreg) noexcept {
    ...
    if (vreg >= interpreter::kGPRegisters) { enc_.EmitMovRM(x64_reg, AT::kStackReg, static_cast<int32_t>(GprOff(vreg))); return; }
...
void NativeCodeGenerator::StoreGpr(uint8_t x64_reg, uint32_t vreg) noexcept {
    ...
    if (vreg >= interpreter::kGPRegisters) { enc_.EmitMovMR(AT::kStackReg, static_cast<int32_t>(GprOff(vreg)), x64_reg); return; }
        ...
        if (cached != kNotCached && num_cache_regs_ > 0) {
            if (cached == x64_reg) {
                ...
                for (; slot < kMaxCacheRegs; ++slot) {
                    if (kCacheableRegs[slot] == cached) break;
                }
...
void NativeCodeGenerator::EmitGprArithmetic(
    ...
    if (opc == IROpCode::Add) {
        ...
        ::chaos::il2cpp::jit::EmitSdiv32(buf_, AT::kScratchA, AT::kScratchA, AT::kScratchB);  // quotient = dividend / divisor
        if (opc == IROpCode::Rem) {
            ::chaos::il2cpp::jit::EmitMul32(buf_, AT::kScratchB, AT::kScratchA, AT::kScratchB);  // WscratchB = quotient * divisor
        ...
        EmitREX(buf_, false, 7, AT::kScratchB); buf_.EmitByte(0xF7); buf_.EmitByte(ModRM(3, 7, AT::kScratchB));  // idiv ecx
        if (opc == IROpCode::Rem) enc_.EmitMovRR(op_reg, AT::kScratchC);
#endif
    } else if (opc == IROpCode::DivUn || opc == IROpCode::RemUn) {
#if defined(__aarch64__)
        // ARM64: UDIV 32-bit unsigned
        enc_.EmitMovRR(AT::kScratchC, AT::kScratchA);  // save dividend
        ::chaos::il2cpp::jit::EmitUdiv32(buf_, AT::kScratchA, AT::kScratchA, AT::kScratchB);  // quotient = dividend / divisor
        ...
        EmitREX(buf_, false, 6, AT::kScratchB); buf_.EmitByte(0xF7); buf_.EmitByte(ModRM(3, 6, AT::kScratchB));  // div ecx
        if (opc == IROpCode::RemUn) enc_.EmitMovRR(op_reg, AT::kScratchC);
#endif
    }
    StoreGpr(op_reg, dst);
}
...
bool NativeCodeGenerator::EmitInstruction(const interpreter::RegisterInstruction& instr) noexcept {
    ...
    case IROpCode::LdcI8: {
        ...
        if (val >= INT32_MIN && val <= INT32_MAX) enc_.EmitMovRI32(AT::kScratchA, static_cast<int32_t>(val));
        else enc_.EmitMovImm64(AT::kScratchA, static_cast<uint64_t>(val));
        StoreGpr(AT::kScratchA, instr.dst_reg());
        return true;
    }
    ...
    case IROpCode::LdcR4: {
        ...
        enc_.EmitMovRIImm32(AT::kScratchA, bits); enc_.EmitMovdXrm(0, AT::kScratchA);
    ...
    case IROpCode::LdcR8: {
        ...
        enc_.EmitMovImm64(AT::kScratchA, bits); enc_.EmitMovqXrm(0, AT::kScratchA);
        StoreFpr(0, instr.dst_reg());
        return true;
    }
    ...
    case IROpCode::LdLoc: {
        ...
        if (vreg < kGprCount) { LoadGpr(AT::kScratchA, vreg); StoreGpr(AT::kScratchA, instr.dst_reg()); }
        return true;
    }
    ...
    case IROpCode::StLoc: {
        ...
        uint32_t vreg = kLocalRegBase + local_idx;
        if (vreg < kGprCount) {
            LoadGpr(AT::kScratchA, instr.src1_reg()); StoreGpr(AT::kScratchA, vreg);
    ...
    case IROpCode::Ret: {
        ...
        enc_.EmitPop(kRDI); enc_.EmitPop(AT::kRetBuf); enc_.EmitPop(AT::kArgsBuf); enc_.EmitPop(AT::kFrameReg); enc_.EmitRet();
#endif
        return true;
    }
    ...
    case IROpCode::Beq: case IROpCode::BneUn: {
        ...
        if (has_graph_coloring_) {
            ...
            if (cmp_a == cmp_b) cmp_b = (cmp_a == AT::kScratchA) ? AT::kScratchB : AT::kScratchA;
        }
    ...
    case IROpCode::Switch: {
        ...
        if (target_count >= 4) {
            ...
            buf_.Emit32(0x8B0C000Au | (AT::kScratchA << 16) | (AT::kTmpReg << 5) | AT::kTmpReg);  // ADD X10, X10, Xidx
    ...
    case IROpCode::StFldBarrier: {
        ...
        enc_.EmitMovImm64(AT::kScratchA, reinterpret_cast<uint64_t>(&chaos::il2cpp::runtime_core::g_bgc_is_marking));
#if defined(__aarch64__)
    ...
    case IROpCode::StElem: {
        ...
        LoadGpr(AT::kScratchB, instr.src1_reg());
        LoadGpr(AT::kScratchC, instr.src2_reg());
        if (instr.flags() & ::chaos::il2cpp::interpreter::kRegHasSrc3) LoadGpr(AT::kExtraScratch0, instr.src3_reg());
    ...
    case IROpCode::Call: case IROpCode::CallBridge: {
        ...
        static constexpr uint8_t kArgRegs[] = {AT::kScratchB, AT::kScratchC, AT::kExtraScratch0, AT::kExtraScratch1};
        uint32_t max_scratch = arg_count < 4 ? arg_count : 4;
        for (uint32_t i = 0; i < max_scratch; ++i) LoadGpr(kArgRegs[i], first_arg_reg + i);
        for (uint32_t i = 4; i < arg_count; ++i) { LoadGpr(AT::kScratchA, first_arg_reg + i); enc_.EmitMovMR(AT::kStackReg, static_cast<int32_t>((i - 4) * 8), AT::kScratchA); }
    ...
    case IROpCode::CallVirtConstrained: {
        ...
        if (pic_slot_count > 0) {
            ...
            for (uint32_t si = 0; si < pic_slot_count; ++si) {
                ...
                if (arg_count >= 1) LoadGpr(AT::kScratchB, first_arg_reg);
                if (arg_count >= 2) LoadGpr(AT::kScratchC, first_arg_reg + 1);
                if (arg_count >= 3) LoadGpr(AT::kExtraScratch0,  first_arg_reg + 2);
                if (arg_count >= 4) LoadGpr(AT::kExtraScratch1,  first_arg_reg + 3);
...
static void OptimizeInstructions(
    CHAOS_IL2CPP_VECTOR(::chaos::il2cpp::interpreter::RegisterInstruction)& instrs,
    ...
    bool has_seh) noexcept
{
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        if ((ri.flags() & ::chaos::il2cpp::interpreter::kRegHasSrc3) && ri.src3_reg() < 64) ++use_count[ri.src3_reg()];
    }

    // ── Header builder for folded instructions ───────────────────────
    auto MakeHdr = [](IROpCode opc, uint8_t dst, uint8_t src1,
    ...
    auto FoldBinary = [](IROpCode opc, int32_t a, int32_t b) -> int32_t {
        switch (opc) {
            ...
            case IROpCode::Xor: return a ^ b;
            case IROpCode::Shl: return static_cast<int32_t>(
                static_cast<uint32_t>(a) << (static_cast<uint32_t>(b) & 0x1F));
            case IROpCode::Shr: return a >> (static_cast<uint32_t>(b) & 0x1F);
            default: return 0;
        }
    };
            ...
            if (prev.has_dst() && prev.dst_reg() == reg) {
                if (prev.op_code() == IROpCode::LdcI4) {
                    ...
                    for (uint32_t k = current_idx; k < instrs.size(); ++k) {
                        ...
                        if (bi.has_imm() &&
                            (bopc == IROpCode::Br ||
                             bopc == IROpCode::BrTrue ||
                             bopc == IROpCode::BrFalse ||
                             bopc == IROpCode::Beq ||
                             bopc == IROpCode::BneUn ||
                             bopc == IROpCode::Blt ||
                             bopc == IROpCode::BltUn ||
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        if (!has_seh && IsFoldable(opc) && ri.has_dst() && ri.has_src1() && ri.has_src2()) {
            ...
            if (v1 && v2) {
                ...
                ri.header = MakeHdr(IROpCode::LdcI4, ri.dst_reg(), 0, 0,
        ...
        if (opc == IROpCode::BrFalse && ri.has_src1()) {
            ...
            if (v) {
                ...
                if (*v == 0) {
                    ...
                    ri.header = MakeHdr(IROpCode::Br, 0, 0, 0,
        ...
        if (opc == IROpCode::StLoc && ri.has_src1() && i > 0) {
            ...
            if (prev.op_code() == IROpCode::StLoc &&
        ...
        if (opc == IROpCode::StLoc && ri.has_src1() && i > 0) {
            ...
            for (uint32_t j = i; j > 0 && (i - j) < kMaxScan; --j) {
                ...
                if (prev_opc == IROpCode::Br ||
                    prev_opc == IROpCode::BrTrue ||
                    prev_opc == IROpCode::BrFalse ||
                    prev_opc == IROpCode::Beq ||
                    prev_opc == IROpCode::Blt ||
                    prev_opc == IROpCode::Bgt ||
                    prev_opc == IROpCode::Ble ||
                    prev_opc == IROpCode::Bge ||
                    prev_opc == IROpCode::BneUn ||
                    prev_opc == IROpCode::BgeUn ||
                    prev_opc == IROpCode::BgtUn ||
                    prev_opc == IROpCode::BleUn ||
                    prev_opc == IROpCode::BltUn ||
                    prev_opc == IROpCode::Switch ||
                    prev_opc == IROpCode::Call ||
                    prev_opc == IROpCode::CallVirt)
                    break;
                if (prev_opc == IROpCode::StLoc &&
                    prev.imm.operand_index == local_idx) {
                    removed_mask[idx] = 1;  // previous StLoc is dead
                    did_opt = true;
                    break;
                }
                if (prev_opc == IROpCode::LdLoc &&
        ...
        if ((opc == IROpCode::Br ||
             opc == IROpCode::BrFalse ||
             opc == IROpCode::BrTrue) && ri.has_imm())
        {
            uint32_t target = ri.imm.branch_target;
            ...
            while (resolved < n && max_hop > 0) {
                ...
                if (target_instr.op_code() == IROpCode::Br &&
        ...
        if (ri.has_dst() && ri.has_src1()) {
            ...
            if (opc == IROpCode::Add || opc == IROpCode::Sub ||
                opc == IROpCode::Or  || opc == IROpCode::Xor ||
                opc == IROpCode::Shl || opc == IROpCode::Shr ||
                opc == IROpCode::ShrUn)
            {
                ...
                if (ri.has_src2()) {
                    const int32_t* v2 = FindDefLdcI4(i, ri.src2_reg());
                    if (v2 && *v2 == 0) {
                        ri.header = MakeHdr(IROpCode::Dup, ri.dst_reg(), ri.src1_reg(), 0,
                                            ::chaos::il2cpp::interpreter::kRegHasDst | ::chaos::il2cpp::interpreter::kRegHasSrc1);
                        ri.imm.i4 = 0;
                        identity_opt = true;
                    }
        ...
        if (opc == IROpCode::Beq && ri.has_src1() && ri.has_src2() && !removed_mask[i]) {
            ...
            for (uint32_t j = i; j < n && case_count < max_cases; ) {
                if (j > i) {
                    ...
                    if (instrs[j].op_code() == IROpCode::LdcI4 &&
            ...
            if (valid_chain && case_count >= 3) {
                ...
                uint64_t switch_header =
                    static_cast<uint64_t>(IROpCode::Switch) |
                    ...
                    (static_cast<uint64_t>(::chaos::il2cpp::interpreter::kRegHasSrc1 |
                                           ...
                                           ::chaos::il2cpp::interpreter::kRegIsBranch) << 40) |
...
JitMethod* NativeCodeGenerator::Generate() noexcept {
    ...
    for (const auto& instr : rm_.instructions) {
        auto opc = instr.op_code();
        if (opc == IROpCode::Call ||
            opc == IROpCode::CallBridge) {
            slot_count_++;
        }
    }
    ...
    if (localloc_extra_ > 0) {
        enc_.EmitMovMR(AT::kStackReg, static_cast<int32_t>(kFrameSize + frame_align_adj_ + xmm_save_size_), AT::kScratchA);
    }
    ...
    if (!is_tier0_ && config_.enable_liveness && n_instrs > 0)
    {
        ...
        for (uint32_t i = 0; i < n_instrs; ++i) {
            ...
            if (inst.op_code() == IROpCode::Calli &&
        ...
        while (changed) {
            ...
            for (int32_t i = static_cast<int32_t>(n_instrs) - 1; i >= 0; --i) {
                ...
                } else if (opc == IROpCode::Ret ||
                           opc == IROpCode::Throw ||
                           opc == IROpCode::Rethrow ||
                           opc == IROpCode::EndFinally ||
                           opc == IROpCode::EndFilter) {
    ...
    for (uint32_t slot = num_cache_regs_; slot > 0; --slot)
        enc_.EmitPop(callee_saved_regs_[slot - 1]);
    enc_.EmitPop(kRDI); enc_.EmitPop(AT::kRetBuf); enc_.EmitPop(AT::kArgsBuf); enc_.EmitPop(AT::kFrameReg);
    enc_.EmitRet();
#endif
    ...
    uint32_t osr_entry = 0;
    {
        ...
        if (has_loop) {
            ...
            for (uint32_t slot = 0; slot < num_cache_regs_; ++slot)
                enc_.EmitPush(callee_saved_regs_[slot]);
#endif
            enc_.EmitSubRI(AT::kStackReg, static_cast<int32_t>(kFrameSize + frame_align_adj_ + xmm_save_size_ + localloc_extra_));
            ...
            for (uint32_t i = 0; i < kGprCount + kFprCount; ++i) {
                ...
                ::chaos::il2cpp::jit::EmitStr64(buf_, AT::kScratchA, AT::kStackReg, static_cast<uint16_t>((kGprFileOff + i * 8) / 8));
            }
#else
            enc_.EmitLeaRM(kRDI, AT::kStackReg, static_cast<int32_t>(kGprFileOff));
...
bool NativeCodeGenerator::EmitSimd(
    const interpreter::RegisterInstruction& instr,
    uint8_t simd_op, uint8_t elem_type, uint16_t simd_imm) noexcept
{
    ...
    case 1:  // kSimdAdd
        if (kUseVexEncoding) {
            switch (elem_type) {
            case 0:  EmitVPaddbRR(buf_, xmm_dst, xmm_src1, xmm_src2); break;
            ...
            switch (elem_type) {
            case 0:  enc_.EmitPaddbRR(xmm_src1, xmm_src2); break;
            ...
            case 4:  EmitFadd4S(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            case 5:  EmitFadd2D(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            default: return false;
            }
        }
        break;
    ...
    case 2:  // kSimdSub
        if (kUseVexEncoding) {
            switch (elem_type) {
            case 0:  EmitVPsubbRR(buf_, xmm_dst, xmm_src1, xmm_src2); break;
            ...
            switch (elem_type) {
            case 0:  enc_.EmitPsubbRR(xmm_src1, xmm_src2); break;
            ...
            case 4:  EmitFsub4S(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            case 5:  EmitFsub2D(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            default: return false;
            }
        }
        break;
    ...
    case 3:  // kSimdMul
        if (kUseVexEncoding) {
            ...
            case 4:  EmitFmul4S(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            case 5:  EmitFmul2D(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            default: return false;
            }
        }
        break;
    ...
    // ── Bitwise ────────────────────────────────────────────────────
    case 4:  // kSimdAnd
        if (kUseVexEncoding) EmitVPandRR(buf_, xmm_dst, xmm_src1, xmm_src2);
        else enc_.EmitPandRR(xmm_src1, xmm_src2);
        break;
    case 5:  // kSimdOr
        if (kUseVexEncoding) EmitVPorRR(buf_, xmm_dst, xmm_src1, xmm_src2);
        else enc_.EmitPorRR(xmm_src1, xmm_src2);
        break;
    case 6:  // kSimdXor
        if (kUseVexEncoding) EmitVPxorRR(buf_, xmm_dst, xmm_src1, xmm_src2);
        else enc_.EmitPxorRR(xmm_src1, xmm_src2);
        break;
    case 7:  // kSimdAndNot
    ...
    case 8:  // kSimdEq
        if (kUseVexEncoding) {
            switch (elem_type) {
            case 0:  EmitVPcmpeqbRR(buf_, xmm_dst, xmm_src1, xmm_src2); break;
            ...
            switch (elem_type) {
            case 0:  enc_.EmitPcmpeqbRR(xmm_src1, xmm_src2); break;
            ...
            case 4:  EmitFcmeq4S(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            case 5:  EmitFcmeq2D(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            default: return false;
            }
        }
        break;
    ...
    case 9:  // kSimdGt
        if (kUseVexEncoding) {
            switch (elem_type) {
            case 0:  EmitVPcmpgtbRR(buf_, xmm_dst, xmm_src1, xmm_src2); break;
            ...
            switch (elem_type) {
            case 0:  enc_.EmitPcmpgtbRR(xmm_src1, xmm_src2); break;
            ...
            case 4:  EmitFcmgt4S(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            case 5:  EmitFcmgt2D(buf_, xmm_src1, xmm_src1, xmm_src2); break;
            default: return false;
            }
        }
        break;
    ...
    case 15:  // kSimdAbs
        if (kUseVexEncoding) {
            switch (elem_type) {
            case 0:  EmitVPabsbRR(buf_, xmm_dst, xmm_src1); break;
            case 1:  EmitVPabswRR(buf_, xmm_dst, xmm_src1); break;
            case 2:  EmitVPabsdRR(buf_, xmm_dst, xmm_src1); break;
            default: return false;
            }
        } else {
            switch (elem_type) {
