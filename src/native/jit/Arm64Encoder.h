...
private:
    ...
    void EmitAddRIDecomposed(uint8_t rd, uint8_t rn, int32_t imm) noexcept {
        ...
        } else if ((static_cast<uint32_t>(imm) & 0xFFF) == 0 && (static_cast<uint32_t>(imm) >> 12) <= 4095) {
    ...
    void EmitLeaRMDecomposed(uint8_t dst, uint8_t base, int32_t disp) noexcept {
        ...
        } else if ((static_cast<uint32_t>(disp) & 0xFFF) == 0 && (static_cast<uint32_t>(disp) >> 12) <= 4095) {
...
public:
    ...
    void EmitMovImm64(uint8_t dst, uint64_t imm) override { EmitLoadImm64(buf_, dst, imm); }
    ...
    void EmitLeaRM(uint8_t dst, uint8_t base, int32_t disp) override { EmitLeaRMDecomposed(dst, base, disp); }
    void EmitLeaRipRel(uint8_t dst, int32_t disp) override { chaos::il2cpp::jit::EmitAdr(buf_, dst, disp); }

    // ── ARM64-specific overrides (stub on x64) ─────────────────────────
    void EmitAdr(uint8_t reg, int32_t imm) noexcept override { chaos::il2cpp::jit::EmitAdr(buf_, reg, imm); }
    void EmitBr(uint8_t reg) noexcept override { chaos::il2cpp::jit::EmitBr(buf_, reg); }
    void EmitAddRI(uint8_t dst, uint8_t src, int32_t imm) noexcept override { EmitAddRIDecomposed(dst, src, imm); }
    ...
    // ── Addition ─────────────────────────────────────────────────────────
    void EmitAddRR(uint8_t dst, uint8_t src) override { EmitAdd64(buf_, dst, dst, src); }
    void EmitAddRI(uint8_t dst, int32_t imm) override { EmitAddRIDecomposed(dst, dst, imm); }
    ...
    void EmitAdd32RR(uint8_t dst, uint8_t src) override { EmitAdd32(buf_, dst, dst, src); }

    // ── Subtraction ──────────────────────────────────────────────────────
    void EmitSubRI(uint8_t dst, int32_t imm) override { EmitSubRIDecomposed(dst, dst, imm); }
    void EmitSubRR(uint8_t dst, uint8_t src) override { EmitSub64(buf_, dst, dst, src); }
    void EmitSub32RR(uint8_t dst, uint8_t src) override { EmitSub32(buf_, dst, dst, src); }

    // ── Multiplication ───────────────────────────────────────────────────
    void EmitImulRR(uint8_t dst, uint8_t src) override { EmitMul64(buf_, dst, dst, src); }
    void EmitImul32RR(uint8_t dst, uint8_t src) override { EmitMul32(buf_, dst, dst, src); }

    // ── Bitwise AND ──────────────────────────────────────────────────────
    void EmitAndRR(uint8_t dst, uint8_t src) override { EmitAnd64(buf_, dst, dst, src); }
    void EmitAnd32RR(uint8_t dst, uint8_t src) override { EmitAnd32(buf_, dst, dst, src); }

    // ── Bitwise OR ───────────────────────────────────────────────────────
    void EmitOrRR(uint8_t dst, uint8_t src) override { EmitOrr64(buf_, dst, dst, src); }
    void EmitOr32RR(uint8_t dst, uint8_t src) override { EmitOrr32(buf_, dst, dst, src); }

    // ── Bitwise XOR ──────────────────────────────────────────────────────
    void EmitXorRR(uint8_t dst, uint8_t src) override { EmitEor64(buf_, dst, dst, src); }
    void EmitXor32RR(uint8_t dst, uint8_t src) override { EmitEor32(buf_, dst, dst, src); }
    ...
    void EmitXorpsRR(uint8_t dst, uint8_t src) override { EmitXor16B(buf_, dst, dst, src); }

    // ── Negation / Not ──────────────────────────────────────────────────────
    void EmitNeg32(uint8_t reg) override { ::chaos::il2cpp::jit::EmitNeg32(buf_, reg, reg); }
    void EmitNeg(uint8_t reg) override { EmitNeg64(buf_, reg, reg); }
    void EmitNot32(uint8_t reg) override { EmitMvn32(buf_, reg, reg); }
    void EmitNot(uint8_t reg) override { EmitMvn64(buf_, reg, reg); }

    // ── Shift ────────────────────────────────────────────────────────────
    void EmitShlRI(uint8_t reg, uint8_t imm) override { EmitLsl64Imm(buf_, reg, reg, imm); }
    void EmitShrRI(uint8_t reg, uint8_t imm) override { EmitLsr64Imm(buf_, reg, reg, imm); }
    void EmitSarRI(uint8_t reg, uint8_t imm) override { EmitAsr64Imm(buf_, reg, reg, imm); }
    ...
    void EmitShrRCL(uint8_t reg) override { EmitLsr64Var(buf_, reg, reg, 1); }
    void EmitSarRCL(uint8_t reg) override { EmitAsr64Var(buf_, reg, reg, 1); }
    ...
    void EmitCmpRI(uint8_t reg, int32_t imm) override { EmitCmpRIDecomposed(reg, imm); }
    ...
    void EmitJmpRel32(int32_t offset) override { EmitB(buf_, offset); }
    void EmitJmpRel8(int8_t offset) override { EmitB(buf_, offset); }
    ...
    void EmitJmpReg(uint8_t reg) override { chaos::il2cpp::jit::EmitBr(buf_, reg); }
    void EmitCallRel32(int32_t offset) override { EmitBl(buf_, offset); }
    ...
    void EmitCallReg(uint8_t reg) override { EmitBlr(buf_, reg); }
    ...
    // ── Sign extension ───────────────────────────────────────────────────
    void EmitMovsxd(uint8_t dst, uint8_t src) override { EmitSxtw(buf_, dst, src); }

    // ── Float / SSE (scalar double) ──────────────────────────────────────
    void EmitMovSDRR(uint8_t dst, uint8_t src) override { EmitFmov64(buf_, dst, src); }
    void EmitMovSSRR(uint8_t dst, uint8_t src) override { EmitFmov32(buf_, dst, src); }
    ...
    void EmitAddSDRR(uint8_t dst, uint8_t src) override { EmitFadd64(buf_, dst, dst, src); }
    void EmitSubSDRR(uint8_t dst, uint8_t src) override { EmitFsub64(buf_, dst, dst, src); }
    void EmitMulSDRR(uint8_t dst, uint8_t src) override { EmitFmul64(buf_, dst, dst, src); }
    void EmitDivSDRR(uint8_t dst, uint8_t src) override { EmitFdiv64(buf_, dst, dst, src); }
    void EmitAddSSRR(uint8_t dst, uint8_t src) override { EmitFadd32(buf_, dst, dst, src); }
    void EmitSubSSRR(uint8_t dst, uint8_t src) override { EmitFsub32(buf_, dst, dst, src); }
    void EmitMulSSRR(uint8_t dst, uint8_t src) override { EmitFmul32(buf_, dst, dst, src); }
    void EmitDivSSRR(uint8_t dst, uint8_t src) override { EmitFdiv32(buf_, dst, dst, src); }
    void EmitCvtsi2sd(uint8_t xmm, uint8_t reg) override { EmitScvtfD64(buf_, xmm, reg); }
    void EmitCvtsi2ss(uint8_t xmm, uint8_t reg) override { EmitScvtfS32(buf_, xmm, reg); }
    void EmitCvttsd2si(uint8_t reg, uint8_t xmm) override { EmitFcvtzsD64(buf_, reg, xmm); }
    void EmitCvttss2si(uint8_t reg, uint8_t xmm) override { EmitFcvtzsS32(buf_, reg, xmm); }
    void EmitCvtsd2ss(uint8_t dst, uint8_t src) override { EmitFcvtD2S(buf_, dst, src); }
    void EmitCvtss2sd(uint8_t dst, uint8_t src) override { EmitFcvtS2D(buf_, dst, src); }
    void EmitUcomisd(uint8_t lhs, uint8_t rhs) override { EmitFcmp64(buf_, lhs, rhs); }
    void EmitUcomiss(uint8_t lhs, uint8_t rhs) override { EmitFcmp32(buf_, lhs, rhs); }
    void EmitComisd(uint8_t lhs, uint8_t rhs) override { EmitFcmp64(buf_, lhs, rhs); }
    ...
    void EmitMovqXrm(uint8_t xmm, uint8_t reg) override { EmitFmov64Gpr(buf_, xmm, reg); }
    void EmitMovqMrx(uint8_t reg, uint8_t xmm) override { EmitFmov64Fpr(buf_, reg, xmm); }
    void EmitPxorRR(uint8_t dst, uint8_t src) override { EmitXor16B(buf_, dst, dst, src); }
    ...
    // ── SSE2 integer ALU ───────────────────────────────────────────────────────
    void EmitPaddbRR(uint8_t dst, uint8_t src) override { EmitAdd16B(buf_, dst, dst, src); }
    void EmitPaddwRR(uint8_t dst, uint8_t src) override { EmitAdd8H(buf_, dst, dst, src); }
    void EmitPadddRR(uint8_t dst, uint8_t src) override { EmitAdd4S(buf_, dst, dst, src); }
    void EmitPaddqRR(uint8_t dst, uint8_t src) override { EmitAdd2D(buf_, dst, dst, src); }
    void EmitPsubbRR(uint8_t dst, uint8_t src) override { EmitSub16B(buf_, dst, dst, src); }
    void EmitPsubwRR(uint8_t dst, uint8_t src) override { EmitSub8H(buf_, dst, dst, src); }
    void EmitPsubdRR(uint8_t dst, uint8_t src) override { EmitSub4S(buf_, dst, dst, src); }
    void EmitPsubqRR(uint8_t dst, uint8_t src) override { EmitSub2D(buf_, dst, dst, src); }
    void EmitPmullwRR(uint8_t dst, uint8_t src) override { EmitMul8H(buf_, dst, dst, src); }
    void EmitPmuludqRR(uint8_t dst, uint8_t src) override { EmitUmull2D(buf_, dst, dst, src); }
    void EmitPandRR(uint8_t dst, uint8_t src) override { EmitAnd16B(buf_, dst, dst, src); }
    void EmitPorRR(uint8_t dst, uint8_t src) override { EmitOrr16B(buf_, dst, dst, src); }
    void EmitPandnRR(uint8_t dst, uint8_t src) override { EmitBic16B(buf_, dst, dst, src); }
    void EmitPcmpeqbRR(uint8_t dst, uint8_t src) override { EmitCmeq16B(buf_, dst, dst, src); }
    void EmitPcmpeqwRR(uint8_t dst, uint8_t src) override { EmitCmeq8H(buf_, dst, dst, src); }
    void EmitPcmpeqdRR(uint8_t dst, uint8_t src) override { EmitCmeq4S(buf_, dst, dst, src); }
    void EmitPcmpeqqRR(uint8_t dst, uint8_t src) override { EmitCmeq2D(buf_, dst, dst, src); }
    void EmitPcmpgtbRR(uint8_t dst, uint8_t src) override { EmitCmgt16B(buf_, dst, dst, src); }
    void EmitPcmpgtwRR(uint8_t dst, uint8_t src) override { EmitCmgt8H(buf_, dst, dst, src); }
    void EmitPcmpgtdRR(uint8_t dst, uint8_t src) override { EmitCmgt4S(buf_, dst, dst, src); }
    void EmitPcmpgtqRR(uint8_t dst, uint8_t src) override { EmitCmgt2D(buf_, dst, dst, src); }
    void EmitPshufdRR(uint8_t dst, uint8_t src, uint8_t imm) override {
        ...
        for (int i = 0; i < 4; i++) {
            uint32_t w = (static_cast<uint32_t>(mask[i * 4 + 3]) << 24) |
                         ...
                         (static_cast<uint32_t>(mask[i * 4 + 1]) << 8) | (static_cast<uint32_t>(mask[i * 4 + 0]));
            buf_.Emit32(w);
        }
    }
    void EmitPabsbRR(uint8_t dst, uint8_t src) override { EmitAbs16B(buf_, dst, src); }
    void EmitPabswRR(uint8_t dst, uint8_t src) override { EmitAbs8H(buf_, dst, src); }
    void EmitPabsdRR(uint8_t dst, uint8_t src) override { EmitAbs4S(buf_, dst, src); }
    ...
    void EmitVfnmadd231pdRR(uint8_t acc, uint8_t src1, uint8_t src2) override { EmitFmls2D(buf_, acc, src1, src2); }
...
private:
    ...
    static uint8_t X64ToArm64Cond(uint8_t xcc) noexcept {
        ...
        switch (xcc) {
            case 0:
                return kARM64_VS; // O  → VS (overflow set)
            case 1:
                return kARM64_VC; // NO → VC
            case 2:
                return kARM64_CC; // B  → CC (unsigned below, carry clear)
            case 3:
                return kARM64_CS; // AE → CS (unsigned >=, carry set)
            case 4:
                return kARM64_EQ; // E  → EQ
            case 5:
                return kARM64_NE; // NE → NE
            case 6:
                return kARM64_LS; // BE → LS (unsigned <=)
            case 7:
                return kARM64_HI; // A  → HI (unsigned >)
            case 8:
                return kARM64_MI; // S  → MI (minus/negative)
            case 9:
                return kARM64_PL; // NS → PL (plus)
            case 10:
                return kARM64_AL; // P  → AL (always, parity not mappable)
            case 11:
                return kARM64_AL; // NP → AL
            case 12:
                return kARM64_LT; // L  → LT (signed <)
            case 13:
                return kARM64_GE; // GE → GE (signed >=)
            case 14:
                return kARM64_LE; // LE → LE (signed <=)
            case 15:
                return kARM64_GT; // G  → GT (signed >)
            default:
                return kARM64_AL;
        }
    }
