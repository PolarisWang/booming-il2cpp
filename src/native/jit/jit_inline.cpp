...
InlineDecision EvaluateInline(uint32_t callee_token, uint32_t /*caller_depth*/, bool return_value_used,
                              uint32_t loop_depth, const InlineConfig& cfg) noexcept {
...
Inliner::Inliner(const InlineConfig& cfg, uint32_t inline_depth, uint32_t caller_max_vreg) noexcept
    : cfg_(cfg), inline_depth_(inline_depth), new_max_vreg_(caller_max_vreg) {}
...
static void RemapCalleeVRegs(tree::ExprNode* node, uint32_t callee_max_vreg, uint32_t first_arg_vreg,
...
bool Inliner::TryInline(InlineCandidate& candidate, tree::ExprNode** roots, uint32_t& root_count,
                        uint32_t max_roots) noexcept {
    const InlineDecision& decision = candidate.decision;
    if (!decision.should_inline)
        return false;
    if (!decision.callee_rm)
        return false;
    ...
    if (callee_max_vreg == 0)
        return false;
    uint32_t vreg_shift = new_max_vreg_;
    if (vreg_shift + callee_max_vreg > 64)
        return false;

    // Step 3: Remap callee vregs. Walk all callee roots recursively.
    for (uint32_t ri = 0; ri < arena.result.root_count; ++ri) {
        RemapCalleeVRegs(arena.result.roots[ri], callee_max_vreg, candidate.first_arg_vreg, vreg_shift);
    }
...
uint32_t Inliner::InlineRoots(tree::ExprNode** roots, uint32_t& root_count, uint32_t max_roots) noexcept {
    if (!roots || root_count == 0 || max_roots == 0)
        return 0;
    ...
    for (uint32_t ri = 0; ri < root_count; ++ri) {
        ...
        candidate.decision =
            EvaluateInline(candidate.callee_token, inline_depth_, candidate.arg_count > 0, bb_loop_depth_, cfg_);
...
InlineReverseMap::InlineReverseMap() noexcept : map_(new Map()) {}
...
uint32_t InlineReverseMap::InvalidateCallers(uint32_t callee_token, HotpatchEntryV0* /*callee_entry*/) noexcept {
    std::lock_guard<std::mutex> lock(mtx_);
    uint32_t count = 0;
    auto& entries = map_->entries;
    for (size_t i = entries.size(); i > 0; --i) {
