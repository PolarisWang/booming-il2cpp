...
const JitMethod* FindNativeCodeByAddress(const void* address) noexcept {
    ...
    if (g_t4_lookup_cache.nm != nullptr && g_t4_lookup_cache.page_base == page && g_t4_lookup_cache.generation == gen) {
...
static void BuildUnwindListForCatch(const JitMethod* nm, uint32_t catch_clause_idx, uint32_t throw_offset) noexcept {
    if (nm->seh_table_offset == 0)
        return;
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (cflags != 2 && cflags != 4)
            continue;
        if (ctry_start < catch_try_start || ctry_start >= catch_try_end)
            continue;
...
static void CollectFinallyFaultAtOffset(const JitMethod* nm, uint32_t code_offset) noexcept {
    if (nm->seh_table_offset == 0)
        return;
...
static uint32_t GetClauseHandlerOffset(const JitMethod* nm, uint32_t clause_idx) noexcept {
    const uint8_t* entry = static_cast<const uint8_t*>(nm->code) + nm->seh_table_offset + sizeof(uint32_t) +
...
#else
            const JitMethod* nm = FindNativeCodeByAddress(__builtin_return_address(0));
#endif
            if (nm != nullptr) {
