...
public:
    ...
    static const DeoptEntry* FindEntry(const JitMethod* nm, uint32_t native_offset) noexcept;
    ...
    static void ReconstructRegisterFile(uint64_t* gpr_file, // output[64]
    ...
    static void DeoptTrap(JitMethod* nm, uint32_t return_address, NativeContext ctx, uint64_t codegen_rsp = 0,
                          uint64_t* out_gpr_file = nullptr, double* out_fpr_file = nullptr,
                          uint8_t* out_gpr_tags = nullptr, uint8_t* out_fpr_tags = nullptr) noexcept;
};
