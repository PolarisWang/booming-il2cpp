...
#pragma pack(pop)
static_assert(sizeof(BinaryCatchHandlerEntry) == 12, "BinaryCatchHandlerEntry must be 12 bytes");
...
#pragma pack(pop)
static_assert(sizeof(BinaryStackMapEntry) == 25, "BinaryStackMapEntry must be 25 bytes");
...
#pragma pack(pop)
static_assert(sizeof(BinaryIrHeader) == 32, "BinaryIrHeader must be 32 bytes");
...
// ── Accessor helpers ───────────────────────────────────────────────────
inline uint32_t BinaryIrInstrCount(const BinaryIrHeader& hdr) noexcept {
    return hdr.instr_count;
}
inline uint32_t BinaryIrSehCount(const BinaryIrHeader& hdr) noexcept {
    return hdr.seh_count;
}
inline uint32_t BinaryIrCatchHandlerCount(const BinaryIrHeader& hdr) noexcept {
    return hdr.catch_handler_count;
}
inline uint32_t BinaryIrIlOffsetCount(const BinaryIrHeader& hdr) noexcept {
    return hdr.il_offset_count;
}
inline uint32_t BinaryIrStackMapCount(const BinaryIrHeader& hdr) noexcept {
    return hdr.stack_map_count;
}
