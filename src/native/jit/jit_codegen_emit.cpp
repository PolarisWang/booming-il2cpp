// jit_codegen_emit.cpp — per-instruction T4 machine-code dispatch
// (NativeCodeGenerator::EmitInstruction) — the core hot-path emitter
// (T2.4 module split of jit_engine.cpp).
//
// Pure move: EmitInstruction was relocated verbatim from jit_engine.cpp; no
// logic changed.  It is the ~2100-line switch that dispatches each IR op to
// the per-opcode emitter, reading the NativeCodeGenerator member state
// (buf_, enc_, gcr_, vreg_types_, slot_patches_, …) declared in jit_engine.h
// and calling the co-located codegen TUs (arith/gc/memory/deopt/simd).
...
#include <gc/gc_bgc.h> // g_bgc_is_marking
