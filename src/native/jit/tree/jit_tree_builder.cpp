...
static NodeKind MapOpcode(interpreter::IROpCode opc) noexcept {
    switch (opc) {
        ...
        case interpreter::IROpCode::LdcR8:
            return kLdcR8;
        case interpreter::IROpCode::LdNull:
            return kLdNull;
        ...
        case interpreter::IROpCode::Mul:
            return kMul;
        case interpreter::IROpCode::Div:
            return kDiv;
        case interpreter::IROpCode::Rem:
            return kRem;
...
bool TreeBuilder::EnsureArena(uint32_t bytes) noexcept {
    ...
    while (needed < bytes)
        needed *= 2;
    if (needed < arena_size_ * 2)
        needed = arena_size_ * 2;
...
ExprNode* TreeBuilder::AllocNode(NodeKind kind, TypeTag tag, ExprNode* c0, ExprNode* c1) noexcept {
    if (!EnsureArena(sizeof(ExprNode)))
        return nullptr;
...
TreeBuildResult TreeBuilder::Build(const interpreter::RegisterMethod& rm, uint32_t lo, uint32_t hi) noexcept {
    if (rm.instructions.empty())
        return {};
    return Build(rm.instructions.data(), lo, hi);
}

TreeBuildResult TreeBuilder::Build(const interpreter::RegisterInstruction* instrs, uint32_t lo, uint32_t hi) noexcept {
    // Reset state
    arena_pos_ = arena_;
    root_count_ = 0;
    ...
    for (uint32_t i = lo; i < hi; ++i) {
        ...
        if (opc == interpreter::IROpCode::StLoc) {
            ...
            if (ri.has_src1()) {
                if (uint8_t s1r = ri.src1_reg(); ri.has_dst() && s1r < 64) {
                    ...
                    if (src_node)
                        vn_.SetComputed(src_node->vn_id());
                }
            }
            is_root = false;
            node = nullptr;
            ...
            if (ri.has_src1() && ri.has_dst() && ri.src1_reg() < 64) {
                ...
                if (val_node)
                    vn_.SetComputed(val_node->vn_id());
            }
            ...
            if (call_node) {
                call_node->call_method_token = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(ri.imm.ptr));
            ...
            if (opc == interpreter::IROpCode::LdcI4) {
                ...
                if (node)
                    node->r4 = ri.imm.r8; // stored as double in imm, cast to float
                ...
                node = AllocNode(nk, kInt32);
                if (node)
                    node->operand_index = ri.imm.operand_index;
                ...
                if (ri.has_imm() && opc != interpreter::IROpCode::LdcI4 && opc != interpreter::IROpCode::LdcI8) {
                    if (node)
                        node->i4 = ri.imm.i4;
                }
            }
        ...
        if (node && dst < 64) {
            ...
            if (nk == kCall || nk == kCallVirt) {
                node->set_vn_id(vn_.GetOrCreate(VNKey::Leaf(nk, reinterpret_cast<int64_t>(ri.imm.ptr))));
            } else if (nk == kLdcI4) {
...
std::vector<BBRange> FindBasicBlocks(const interpreter::RegisterInstruction* instrs, uint32_t count) noexcept {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        bool is_terminator = (opc == interpreter::IROpCode::Br || opc == interpreter::IROpCode::BrTrue ||
                              opc == interpreter::IROpCode::BrFalse || opc == interpreter::IROpCode::Beq ||
                              opc == interpreter::IROpCode::BneUn || opc == interpreter::IROpCode::Blt ||
                              opc == interpreter::IROpCode::BltUn || opc == interpreter::IROpCode::Bgt ||
                              opc == interpreter::IROpCode::BgtUn || opc == interpreter::IROpCode::Ble ||
                              opc == interpreter::IROpCode::BleUn || opc == interpreter::IROpCode::Bge ||
                              opc == interpreter::IROpCode::BgeUn || opc == interpreter::IROpCode::Ret ||
                              opc == interpreter::IROpCode::Throw || opc == interpreter::IROpCode::Rethrow ||
                              opc == interpreter::IROpCode::Switch || opc == interpreter::IROpCode::Leave ||
                              opc == interpreter::IROpCode::EndFinally || opc == interpreter::IROpCode::EndFilter);
