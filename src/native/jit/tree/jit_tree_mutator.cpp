...
static int64_t FoldBinary(NodeKind kind, int64_t a, int64_t b) noexcept {
    switch (kind) {
        ...
        case kXor:
            return a ^ b;
        case kShl:
            return static_cast<int64_t>(static_cast<uint64_t>(a) << (static_cast<uint64_t>(b) & 0x3F));
        case kShr:
            return a >> (static_cast<uint64_t>(b) & 0x3F);
        case kShrUn:
            return static_cast<int64_t>(static_cast<uint64_t>(a) >> (static_cast<uint64_t>(b) & 0x3F));
        case kCeq:
            return (a == b) ? 1 : 0;
...
ExprNode* ConstFoldMutator::PostVisit(ExprNode* node) noexcept {
    ...
    if (is_binary) {
        ...
        if (IsConstInt(node->child0, v1) && IsConstInt(node->child1, v2)) {
            ...
            (void)old_vn;
            node->set_kind((k == kCeq || k == kClt || k == kCgt || k == kCltUn || k == kCgtUn) ? kLdcI4 : kLdcI8);
            node->type_tag = (k == kCeq || k == kClt || k == kCgt || k == kCltUn || k == kCgtUn) ? kInt32 : kInt64;
            node->child_count = 0;
            node->child0 = nullptr;
            node->child1 = nullptr;
            if (node->kind() == kLdcI4)
    ...
    if (is_unary) {
        ...
        if (IsConstInt(node->child0, v)) {
            ...
            bool is_32bit_not = (k == kNot && node->child0 && node->child0->kind() == kLdcI4);
