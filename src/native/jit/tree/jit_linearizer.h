...
public:
    ...
    uint32_t Linearize(const ExprNode* root, std::vector<interpreter::RegisterInstruction>& out_instrs) noexcept;
...
private:
    ...
    uint32_t LinearizeNode(const ExprNode* node, std::vector<interpreter::RegisterInstruction>& out) noexcept;

    /// Emit a single RegisterInstruction helper.
    uint32_t EmitInstr(uint64_t header, int64_t imm, std::vector<interpreter::RegisterInstruction>& out) noexcept;
    uint32_t EmitInstr(uint64_t header, int32_t imm, std::vector<interpreter::RegisterInstruction>& out) noexcept;
