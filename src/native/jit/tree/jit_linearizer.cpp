...
static uint64_t MakeHdr(interpreter::IROpCode opc, uint8_t dst = 0, uint8_t src1 = 0, uint8_t src2 = 0,
...
static interpreter::IROpCode MapToIROpCode(NodeKind kind) noexcept {
    switch (kind) {
        case kLdcI4:
            return interpreter::IROpCode::LdcI4;
        case kLdcI8:
            return interpreter::IROpCode::LdcI8;
        case kLdcR4:
            return interpreter::IROpCode::LdcR4;
        case kLdcR8:
            return interpreter::IROpCode::LdcR8;
        case kLdNull:
            return interpreter::IROpCode::LdNull;
        case kLdStr:
            return interpreter::IROpCode::LdStr;
        case kLdArg:
            return interpreter::IROpCode::LdArg;
        case kLdLoc:
            return interpreter::IROpCode::LdLoc;
        case kNeg:
            return interpreter::IROpCode::Neg;
        case kNot:
            return interpreter::IROpCode::Not;
        case kConvI4:
            return interpreter::IROpCode::Conv_I4;
        case kConvI8:
            return interpreter::IROpCode::Conv_I8;
        case kConvR4:
            return interpreter::IROpCode::Conv_R4;
        case kConvR8:
            return interpreter::IROpCode::Conv_R8;
        case kAdd:
            return interpreter::IROpCode::Add;
        case kSub:
            return interpreter::IROpCode::Sub;
        case kMul:
            return interpreter::IROpCode::Mul;
        case kDiv:
            return interpreter::IROpCode::Div;
        case kRem:
            return interpreter::IROpCode::Rem;
        case kAnd:
            return interpreter::IROpCode::And;
        case kOr:
            return interpreter::IROpCode::Or;
        case kXor:
            return interpreter::IROpCode::Xor;
        case kShl:
            return interpreter::IROpCode::Shl;
        case kShr:
            return interpreter::IROpCode::Shr;
        case kShrUn:
            return interpreter::IROpCode::ShrUn;
        case kCeq:
            return interpreter::IROpCode::Ceq;
        case kClt:
            return interpreter::IROpCode::Clt;
        case kCgt:
            return interpreter::IROpCode::Cgt;
        case kLdLen:
            return interpreter::IROpCode::LdLen;
        case kLdElem:
            return interpreter::IROpCode::LdElem;
        case kLdElemA:
            return interpreter::IROpCode::LdElemA;
        case kAbs:
            return interpreter::IROpCode::Abs;
        case kMin:
            return interpreter::IROpCode::Min;
        case kMax:
            return interpreter::IROpCode::Max;
        case kLdFld:
            return interpreter::IROpCode::LdFld;
        case kStLoc:
            return interpreter::IROpCode::StLoc;
        case kReturn:
            return interpreter::IROpCode::Ret;
        case kBox:
            return interpreter::IROpCode::Box;
        case kUnbox:
            return interpreter::IROpCode::Unbox;
        case kPopcnt:
            return interpreter::IROpCode::Popcnt;
        case kLzcnt:
            return interpreter::IROpCode::Lzcnt;
        default:
            return static_cast<interpreter::IROpCode>(0); // unreachable for valid nodes
    }
}
...
uint32_t Linearizer::LinearizeNode(const ExprNode* node, std::vector<interpreter::RegisterInstruction>& out) noexcept {
    if (!node)
        return 0;
    ...
    switch (k) {
        ...
        case kLdNull:
            return EmitInstr(MakeHdr(interpreter::IROpCode::LdNull, NextVReg(), 0, 0, interpreter::kRegHasDst), 0, out);
        ...
        case kLdArg: {
            ...
            interpreter::RegisterInstruction ri;
            ri.header =
                MakeHdr(interpreter::IROpCode::LdArg, dst, 0, 0, interpreter::kRegHasDst | interpreter::kRegHasImm);
        ...
        case kLdLoc: {
            ...
            interpreter::RegisterInstruction ri;
            ri.header =
                MakeHdr(interpreter::IROpCode::LdLoc, dst, 0, 0, interpreter::kRegHasDst | interpreter::kRegHasImm);
            ri.imm.operand_index = node->operand_index;
            out.push_back(ri);
            return dst;
        }
    ...
    if (IsCseDuplicate(node) && ((k >= kNeg && k <= kCgtUn) || k == kAbs || k == kMin || k == kMax)) {
    ...
    if ((k >= kAdd && k <= kCgtUn) || k == kMin || k == kMax) {
        ...
        if (src1_vreg == 0 && node->child0)
            return 0;
        if (src2_vreg == 0 && node->child1)
            return 0;
        ...
        interpreter::RegisterInstruction ri;
        ri.header = MakeHdr(opc, dst, static_cast<uint8_t>(src1_vreg), static_cast<uint8_t>(src2_vreg),
    ...
    if (k == kLdElem || k == kLdElemA) {
        ...
        if (arr_vreg == 0 || idx_vreg == 0)
            return 0;

        interpreter::IROpCode opc =
            (node->flags & kFlagNoBoundsCheck)
                ? ((k == kLdElem) ? interpreter::IROpCode::LdElemNoChk : interpreter::IROpCode::LdElemANoChk)
                : MapToIROpCode(k);

        uint32_t dst = NextVReg();
        interpreter::RegisterInstruction ri;
        ri.header = MakeHdr(opc, dst, static_cast<uint8_t>(arr_vreg), static_cast<uint8_t>(idx_vreg),
                            interpreter::kRegHasDst | interpreter::kRegHasSrc1 | interpreter::kRegHasSrc2);
        ri.imm.i4 = 0;
        out.push_back(ri);
    ...
    if (k == kSimd) {
        ...
        interpreter::RegisterInstruction ri;
        ri.header =
            MakeHdr(interpreter::IROpCode::Simd, dst, static_cast<uint8_t>(src1_vreg), static_cast<uint8_t>(src2_vreg),
                    interpreter::kRegHasDst | interpreter::kRegHasSrc1 | (src2_vreg ? interpreter::kRegHasSrc2 : 0));
        ri.imm.i8 = simd_meta;
        out.push_back(ri);
    ...
    if (k == kStLoc) {
        ...
        if (val_vreg == 0 && node->child0)
            return 0;

        interpreter::RegisterInstruction ri;
        ri.header = MakeHdr(interpreter::IROpCode::StLoc, 0, static_cast<uint8_t>(val_vreg), 0,
                            interpreter::kRegHasSrc1 | interpreter::kRegHasImm | interpreter::kRegIsStore);
    ...
    if (k == kStElem) {
        ...
        interpreter::IROpCode opc =
            (node->flags & kFlagNoBoundsCheck) ? interpreter::IROpCode::StElemNoChk : interpreter::IROpCode::StElem;

        interpreter::RegisterInstruction ri;
        ri.header = MakeHdr(
            opc, 0, static_cast<uint8_t>(arr_vreg), static_cast<uint8_t>(idx_vreg),
            interpreter::kRegHasSrc1 | interpreter::kRegHasSrc2 | interpreter::kRegHasSrc3 | interpreter::kRegIsStore);
    ...
    if (k == kReturn) {
        if (node->child0) {
            uint32_t val_vreg = LinearizeNode(node->child0, out);
            interpreter::RegisterInstruction ri;
            ri.header = MakeHdr(interpreter::IROpCode::Ret, 0, static_cast<uint8_t>(val_vreg), 0,
                                interpreter::kRegHasSrc1 | interpreter::kRegIsBranch);
            ri.imm.i4 = 0;
            out.push_back(ri);
        } else {
            interpreter::RegisterInstruction ri;
            ri.header = MakeHdr(interpreter::IROpCode::Ret, 0, 0, 0, interpreter::kRegIsBranch);
            ri.imm.i4 = 0;
            out.push_back(ri);
        }
        return 0;
...
uint32_t Linearizer::Linearize(const ExprNode* root,
                               std::vector<interpreter::RegisterInstruction>& out_instrs) noexcept {
    return LinearizeNode(root, out_instrs);
}

void Linearizer::LinearizeRoots(const ExprNode* const* roots, uint32_t root_count,
                                std::vector<interpreter::RegisterInstruction>& out_instrs) noexcept {
