...
internal static class Program
{
    ...
    private static int ExecuteDependencyLayer(IReadOnlyDictionary<string, string> options)
    {
        var catalogPath = GetRequiredOption(options, "--catalog");
        var phase1Directory = GetRequiredOption(options, "--phase1-dir");
        var phase2Directory = GetRequiredOption(options, "--phase2-dir");
        ...
        var outputRoot = Path.GetFullPath(outputDirectory);
        Directory.CreateDirectory(outputRoot);
        var assemblyPlanDirectory = Path.Combine(outputRoot, "assembly-nativeization-plan");
        Directory.CreateDirectory(assemblyPlanDirectory);

        var artifacts = DependencyLayerArtifactsBuilder.Build(
            catalogPath,
            phase1Directory,
            phase2Directory,
            phase3Directory,
        ...
        foreach (var assemblyPlan in artifacts.AssemblyPlans)
        {
    ...
    internal static Dictionary<string, AssemblyAnalysis> MergeAssemblies(
    ...
    internal static IReadOnlyDictionary<string, LoadedAssembly> LoadAssemblies(string rootPath, MetadataPreference metadataPreference)
    {
    ...
    internal static void ClassifyAssemblies(IReadOnlyDictionary<string, AssemblyAnalysis> analyses)
    {
    ...
    internal static IEnumerable<ClosureTarget> GetClosureTargets(AssemblyAnalysis analysis)
    {
        var metadata = analysis.PreferredMetadata;
        if (metadata is null)
        {
