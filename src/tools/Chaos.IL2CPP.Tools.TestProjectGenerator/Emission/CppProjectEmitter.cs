...
public sealed class CppProjectEmitter
{
        ...
        bool verificationEnabled = true)
    {
        ...
        // codegen runs (with different page counts) from leaking into the build.
        {
            ...
            // Remove stale bridge redirect stubs (generated by chaos-il2cpp's
            // BridgeAOT compiler, which may be out of sync).  These files reference
            // chaos_bridge_* symbols that are never defined, causing LNK2019.
            // The bridge redirect table will be populated when BridgeAOT is stable.
            var staleBridgeFiles = new[] { "bridge-redirect.generated.cpp", "chaos_register_bridge_redirects.generated.cpp" };
            foreach (var sbf in staleBridgeFiles)
            {
                var path = Path.Combine(subjectsDir, sbf);
