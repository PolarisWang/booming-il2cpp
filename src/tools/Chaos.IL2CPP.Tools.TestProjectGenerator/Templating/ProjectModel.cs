...
internal sealed class ProjectModelBuilder
{
        ...
        foreach (var s in subjects)
        {
            ...
            var entry = new ScriptObject
            {
                ...
                ["function_symbol"] = "CombinedSubjects_AutoGenerated_"
                    + SanitizeForCpp(s.AssemblyName) + "_"
                    + SanitizeFullTypeName(s.FullTypeName)
                    + "Tests_" + (s.GeneratedMethodId ?? s.MethodName),
            };
    ...
    /// Sanitize a C# identifier (assembly name, type name) for use in a C++ symbol.
    /// Replaces dots with underscores, removes other non-alphanumeric chars.
    private static string SanitizeForCpp(string name)
    {
    ...
    /// <summary>Sanitize a fully-qualified type name for use as a C++ identifier.
    /// Replaces special chars (. + ` &lt; &gt; ,) with '_' to match codegen's
    /// CombinedSubjects naming convention. Uses StringBuilder to avoid the
    /// N intermediate string allocations from chain-of-Replace.</summary>
    private static string SanitizeFullTypeName(string name)
    {
        var sb = new System.Text.StringBuilder(name.Length);
        foreach (var c in name)
        {
