...
static int RunPatchMode(string dllPath, string? namespaceFilter, string? outputDir, string? capabilitiesPath, string? capabilityTablePath)
{
    ...
    // Reflection-subject metadata donor.  ValueGenerator's reflection-instance
    // factories emit `typeof(ReflectionSubjectSample)` and friends verbatim
    // (see ReflectionInstanceFactories), so whatever file carries those
    // expressions must also define the type.  The normal (TestEmitter) path
    // emits it once per combined file; patch mode builds its own file with its
    // own namespace and used to omit it, so any subject taking a
    // System.Type / MemberInfo / MethodBase argument produced an uncompilable
    // CombinedPatchSubjects.cs (CS0246) and the whole hotupdate stage failed.
