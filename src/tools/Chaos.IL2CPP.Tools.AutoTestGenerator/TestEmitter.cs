...
public sealed class TestEmitter
{
        ...
        IReadOnlySet<string>? net10OnlyMethods = null)
    {
        ...
        for (int mi = 0; mi < methods.Count; mi++)
        {
            ...
            foreach (var set in sets)
            {
                ...
                // NOTE: when hasException is true we must NOT evaluate the call
                // here — it throws.  AppendAssert wraps it in the assertion lambda
                // (Assert.ThrowsAny for external stubs whose AOT body reproduces the
                // managed throw, Assert.Throws<T> otherwise).
        ...
        bool isExternalAssembly, bool isPlainTask, bool isGenericTask)
    {
        ...
        {
            // External-assembly methods whose AOT catch-all reproduces the managed
            // exception (F, json-xml-production-readiness): the call IS executed and
            // the throw is asserted, so the subject yields a real result instead of
            // an [UNVERIFIED] stub gap.  The AOT side raises a bare
            // chaos_managed_exception without a managed type attached, so the exact
            // exception class cannot be matched — ThrowsAny<Exception> verifies the
