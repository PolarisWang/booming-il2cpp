...
public sealed class TestEmitter
{
        ...
        IReadOnlySet<string>? net10OnlyMethods = null)
    {
        ...
        for (int mi = 0; mi < methods.Count; mi++)
        {
            ...
            foreach (var set in sets)
            {
                ...
                // EXCEPTION: For external AOT assemblies, the stub NEVER throws — so
                // execute the call anyway (the stub returns default, doesn't crash),
                // then assert the stub's return value matches default(T).  This turns
                // the 42-sentinel into a real AOT stub behavior test.
                var hasException = setResult?.HasException == true;
                var factCallStatement = (hasException && !isExternalAssembly)
                    ? $"{preludeStr}"
                    : callStatement;
                ...
                if (!skipFact)
                {
                    ...
                    // EXCEPTION: For external AOT assemblies, the stub NEVER throws,
                    // so hasException methods still execute the call and produce a real
                    // return value — return that instead of 42L.
                    if (method.IsVoid || isPlainTask || (hasException && !isExternalAssembly))
                    {
        ...
        bool isExternalAssembly, bool isPlainTask, bool isGenericTask)
    {
        ...
        if (result.HasException && result.ExceptionType is not null)
        {
            ...
            // The call was already executed above (factCallStatement) as a smoke test
            // (stub won't crash).  Do NOT emit Assert.AreEqual(default) — the stub's
            // default return is not a validated semantic, and a future native-compiled
            // method would return a real value, causing a false-negative assert failure.
            //
            // Instead, emit a [UNVERIFIED] marker so the fact layer can scan for it
            // and exclude these subjects from the authoritative "passed" count.
