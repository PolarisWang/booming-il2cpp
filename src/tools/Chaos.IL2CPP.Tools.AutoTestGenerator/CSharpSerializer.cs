...
public sealed class CSharpSerializer
{
    ...
    public string DeserializeToExpression(string json, string typeName)
    {
        ...
        // Probe-side namespace → subject-side.  A probe value whose runtime type is
        // the metadata donor reports its FullName with the PROBE project's namespace,
        // e.g. "AutoGeneratedProbe.ReflectionSubjectSample[]" for
        // Array.CreateInstance(typeof(ReflectionSubjectSample), ...).  The subject
        // compilation unit declares the same donor under AutoGenerated.<Assembly>,
        // so an emitted "AutoGeneratedProbe.X" reference fails as CS0246.
        //
        // The subject namespace is per-assembly and not known here, but every donor
        // reference resolves by its SHORT name inside the subject namespace (the
        // donor is declared there directly).  Strip the probe namespace rather than
        // rewriting it, which is both correct for the donor and harmless for any
        // other type (no BCL type lives in AutoGeneratedProbe).
        cleanType = StripProbeNamespace(cleanType);
        ...
        if (root.ValueKind == JsonValueKind.Array)
        {
            ...
            // CLR arrays construct with `new T[] { ... }` regardless of whether the
            // element type has an Add method, so handle them before the
            // collection-type check. Previously every array fell through to
            ...
            // Multidimensional arrays (T[,], T[,,]).  These do NOT end with "[]",
            // so they previously fell past the branch above into the collection
            // check, failed IsCollectionType, and emitted default(T[,])! — i.e.
            // NULL.  Array.CreateInstance(type, 0, 0) returns an empty rank-2
            // array, never null, so that expectation rejected a correct
            // implementation (measured on .NET 8: all ranks return Length=0).
            //
            // The probe JSON is flat, so the populated case cannot be
            // reconstructed here; emit the rank-correct EMPTY array, which is
            // exactly what the probed inputs (all-zero dimensions) produce.
            if (TryGetArrayRank(cleanType, out var rank))
            {
                var arrayElementType = ExtractElementType(cleanType)
                    ?? cleanType[..cleanType.IndexOf('[', StringComparison.Ordinal)];
                var elementName = MapToCSharpType(arrayElementType);
                var dims = string.Join(",", Enumerable.Repeat("0", rank));
                return $"new {elementName}[{dims}]";
            }
    ...
    /// <summary>
    /// True when <paramref name="typeName"/> is a multidimensional array
    /// (<c>T[,]</c>, <c>T[,,]</c>, ...); yields its rank (2 for <c>[,]</c>).
    /// A single-dimensional <c>T[]</c> reports false — it is handled by the
    /// element-initializer path instead.
    /// </summary>
    private static bool TryGetArrayRank(string typeName, out int rank)
    {
        rank = 0;
        ...
        // Between the brackets: "" for T[], "," for T[,], ",," for T[,,], ...
        // Also accept the CLR rank form "T[*]" only when it really is multidim.
        var inner = typeName[(open + 1)..close];
        if (inner.Length == 0) return false;  // T[] — 1-D, handled elsewhere
        if (inner.Any(c => c != ',')) return false;
    ...
    /// <summary>
    /// The namespace ProbeEmitter compiles its metadata donor into.  A probe value
    /// whose runtime type is that donor reports a FullName carrying this prefix,
    /// which is meaningless in the subject (test) compilation unit.
    /// </summary>
    internal const string ProbeNamespace = "AutoGeneratedProbe";

    /// <summary>
    /// Remove the probe project's namespace from a type reference so the emitted
    /// expression resolves inside the subject compilation unit.
    ///
    /// The subject declares the metadata donor (<c>ReflectionSubjectSample</c> and
    /// friends) directly in <c>AutoGenerated.&lt;Assembly&gt;</c>, so the short name
    /// always binds there — no prefix is needed.  The subject namespace is
    /// per-assembly, so stripping is both simpler and more robust than rewriting
    /// the prefix to a guessed namespace.
    ///
    /// Applies to every occurrence, including array-rank suffixes and generic
    /// arguments, so "AutoGeneratedProbe.ReflectionSubjectSample[,]" becomes
    /// "ReflectionSubjectSample[,]".
    /// </summary>
    internal static string StripProbeNamespace(string typeName)
    {
        if (string.IsNullOrEmpty(typeName))
            return typeName;
        return typeName.Replace(ProbeNamespace + ".", "", StringComparison.Ordinal);
    }
