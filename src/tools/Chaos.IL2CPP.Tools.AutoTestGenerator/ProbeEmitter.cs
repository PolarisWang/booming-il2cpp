...
public sealed class ProbeEmitter
{
        ...
        IReadOnlyList<IReadOnlyList<ValueSet>> allValueSets)
    {
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // ── Task-like subjects must be COMPLETED, not merely constructed ──");
        sb.AppendLine("        //");
        sb.AppendLine("        // `Activator.CreateInstance<Task>()` yields an UNSTARTED task");
        sb.AppendLine("        // (Status == Created).  A probe that immediately consumes it —");
        sb.AppendLine("        // `Create<Task>().Wait()`, `.GetAwaiter().GetResult()`, `.Result` —");
        sb.AppendLine("        // then blocks forever, because nothing will ever start that task.");
        sb.AppendLine("        // That is CORRECT .NET semantics for an unstarted task, so the");
        sb.AppendLine("        // runtime is not at fault: the probe is unsatisfiable by");
        sb.AppendLine("        // construction.  Measured: the whole threading-tasks fact run hung");
        sb.AppendLine("        // at the first such subject (TaskTests::Wait_7__0) and never reached");
        sb.AppendLine("        // the remaining 448 subjects.");
        sb.AppendLine("        //");
        sb.AppendLine("        // Handing back a COMPLETED task keeps every probe satisfiable while");
        sb.AppendLine("        // leaving the observed contract intact: a completed task's Wait()/");
        sb.AppendLine("        // await/Result all return immediately, which is what these probes");
        sb.AppendLine("        // assert.  Types whose construction is itself the subject (e.g.");
        sb.AppendLine("        // TaskCompletionSource, whose `.Task` a probe may still exercise) are");
        sb.AppendLine("        // deliberately NOT short-circuited here — only the task types whose");
        sb.AppendLine("        // value is already finished.");
        sb.AppendLine("        if (typeof(T) == typeof(global::System.Threading.Tasks.Task))");
        sb.AppendLine("            return (T)(object)global::System.Threading.Tasks.Task.CompletedTask;");
