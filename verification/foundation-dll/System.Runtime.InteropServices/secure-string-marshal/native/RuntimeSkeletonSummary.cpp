...
// native-reference runtime skeleton for family/System.Runtime.InteropServices/secure-string-marshal
#include "bootstrap.h"
...
{
namespace runtime_skeleton_secure_string_marshal
{
...
constexpr CHAOS_IL2CPP_SIZE translation_unit_method_count = 4;
constexpr CHAOS_IL2CPP_SIZE translation_unit_page_size = 4;
...
{
    { 0, "System.Runtime.InteropServices/System.Security.SecureStringMarshal::SecureStringToCoTaskMemAnsi:System.Void(P0)", DispatchRuntimeSkeletonPage0001, 0 },
    { 1, "System.Runtime.InteropServices/System.Security.SecureStringMarshal::SecureStringToCoTaskMemUnicode:System.Void(P0)", DispatchRuntimeSkeletonPage0001, 1 },
    { 2, "System.Runtime.InteropServices/System.Security.SecureStringMarshal::SecureStringToGlobalAllocAnsi:System.Void(P0)", DispatchRuntimeSkeletonPage0001, 2 },
    { 3, "System.Runtime.InteropServices/System.Security.SecureStringMarshal::SecureStringToGlobalAllocUnicode:System.Void(P0)", DispatchRuntimeSkeletonPage0001, 3 },
};
...
int32_t CHAOS_RUNTIME_ABI_CALL RunNativeReferenceAssembly_secure_string_marshal(
    const CodegenBridgeV0* bridge,
    const CodeRegistrationV0* code_registration,
    const MetadataRegistrationV0* metadata_registration,
    const CodegenRegistrationOptionsV0* options,
    ...
    auto* domain = md::FindDomainByName(runtime_skeleton_secure_string_marshal::kAssemblyName);
    ...
    const auto* request = static_cast<const runtime_skeleton_secure_string_marshal::NativeReferenceAssemblyDispatchRequest*>(managed_args);
    if (request->method_id != 0) {
        return runtime_skeleton_secure_string_marshal::DispatchAssemblyMethodId(bridge, &local_code_registration, metadata_registration, options, runtime, thread, request->method_id, request->managed_args);
    }
    return runtime_skeleton_secure_string_marshal::DispatchAssemblySubject(bridge, &local_code_registration, metadata_registration, options, runtime, thread, request->subject_id, request->managed_args);
}

int32_t CHAOS_RUNTIME_ABI_CALL RunNativeReference_secure_string_marshal(
    const CodegenBridgeV0* bridge,
    const CodeRegistrationV0* code_registration,
    const MetadataRegistrationV0* metadata_registration,
    const CodegenRegistrationOptionsV0* options,
    ...
    ThreadState* thread,
    void* managed_args) {
    return RunNativeReferenceAssembly_secure_string_marshal(bridge, code_registration, metadata_registration, options, runtime, thread, managed_args);
}
}  // namespace chaos::il2cpp::generated
