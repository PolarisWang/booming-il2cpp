...
// native-reference runtime skeleton for family/System.Runtime.InteropServices/function-pointers
#include "bootstrap.h"
...
{
namespace runtime_skeleton_function_pointers
{
...
constexpr CHAOS_IL2CPP_SIZE translation_unit_method_count = 1;
constexpr CHAOS_IL2CPP_SIZE translation_unit_page_size = 1;
...
{
    { 0, "System.Runtime.InteropServices/System.Runtime.InteropServices.UnmanagedCallersOnlyAttribute::UnmanagedCallersOnlyAttribute::Void()", DispatchRuntimeSkeletonPage0001, 0 },
};
...
int32_t CHAOS_RUNTIME_ABI_CALL RunNativeReferenceAssembly_function_pointers(
    const CodegenBridgeV0* bridge,
    const CodeRegistrationV0* code_registration,
    const MetadataRegistrationV0* metadata_registration,
    const CodegenRegistrationOptionsV0* options,
    ...
    auto* domain = md::FindDomainByName(runtime_skeleton_function_pointers::kAssemblyName);
    ...
    const auto* request = static_cast<const runtime_skeleton_function_pointers::NativeReferenceAssemblyDispatchRequest*>(managed_args);
    if (request->method_id != 0) {
        return runtime_skeleton_function_pointers::DispatchAssemblyMethodId(bridge, &local_code_registration, metadata_registration, options, runtime, thread, request->method_id, request->managed_args);
    }
    return runtime_skeleton_function_pointers::DispatchAssemblySubject(bridge, &local_code_registration, metadata_registration, options, runtime, thread, request->subject_id, request->managed_args);
}

int32_t CHAOS_RUNTIME_ABI_CALL RunNativeReference_function_pointers(
    const CodegenBridgeV0* bridge,
    const CodeRegistrationV0* code_registration,
    const MetadataRegistrationV0* metadata_registration,
    const CodegenRegistrationOptionsV0* options,
    ...
    ThreadState* thread,
    void* managed_args) {
    return RunNativeReferenceAssembly_function_pointers(bridge, code_registration, metadata_registration, options, runtime, thread, managed_args);
}
}  // namespace chaos::il2cpp::generated
