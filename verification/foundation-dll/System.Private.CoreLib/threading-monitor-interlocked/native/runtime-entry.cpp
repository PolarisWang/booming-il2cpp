...
enum class RunMode { Fact, Benchmark, HotUpdate, HotUpdateAndBenchmark, PatchAndBenchmark, Stress };
...
int main(int argc, char** argv) {
    ...
    } else if (argc >= 2 && std::strcmp(argv[1], "--stress") == 0) {
        mode = RunMode::Stress;
        if (argc >= 3) entry_index = std::atoi(argv[2]);  // thread count
        if (argc >= 4) iterations = std::atoi(argv[3]);    // duration ms
    ...
    case RunMode::Stress: {
        int thread_count = entry_index > 0 ? entry_index : 8;
        int duration = iterations > 0 ? iterations : 5000;
        int result = 1;
        if (setjmp(s_verify_buf) == 0) {
            try {
                result = RunStressTests(thread_count, duration);
            } catch (...) {
                longjmp(s_verify_buf, 1);
            }
        }
