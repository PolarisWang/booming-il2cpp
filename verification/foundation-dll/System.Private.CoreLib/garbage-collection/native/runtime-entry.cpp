...
// External runtime function table — populated by bootstrap or stubs below
...
int main(int argc, char** argv) {
    ...
    // Provide stubs for external runtime methods not available in standalone
    // entry.exe build.  These let the GC verification subjects execute without
    // crashing on null function pointers in kChaosExternalRuntimeFnTable.
    // The stub semantics match what the managed subjects expect: void methods
    // do nothing, query methods return zero/empty.
    // Table has 8 entries, alphabetically sorted by the codegen:
    //   [0]=Array.CreateInstance, [1]=GC.Collect, [2]=GC.GetGeneration,
    //   [3]=GC.GetTotalMemory,    [4]=GC.KeepAlive, [5]=GC.WaitForPendingFinalizers,
    //   [6]=Object.GetHashCode,   [7]=Type.GetTypeFromHandle
    // Entries with existing codegen wrappers ([0],[4],[7]) are already populated.
    kChaosExternalRuntimeFnTable[1] = reinterpret_cast<void*>(+[](){});  // GC.Collect
    kChaosExternalRuntimeFnTable[2] = reinterpret_cast<void*>(+[](CHAOS_IL2CPP_INTPTR) -> CHAOS_IL2CPP_INT32 { return 0; });  // GC.GetGeneration
    kChaosExternalRuntimeFnTable[3] = reinterpret_cast<void*>(+[](CHAOS_IL2CPP_INTPTR) -> CHAOS_IL2CPP_INT64 { return 0; });  // GC.GetTotalMemory
    kChaosExternalRuntimeFnTable[5] = reinterpret_cast<void*>(+[](){});  // GC.WaitForPendingFinalizers
    kChaosExternalRuntimeFnTable[6] = reinterpret_cast<void*>(+[]() -> CHAOS_IL2CPP_INT32 { return 0; });  // Object.GetHashCode
