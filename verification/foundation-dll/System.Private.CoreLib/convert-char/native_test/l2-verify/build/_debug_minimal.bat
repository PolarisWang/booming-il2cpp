...
cl /nologo /std:c++20 /c /EHsc /W3 /utf-8 /Od /MD ^
  "-I%REPO%\src\native\common" ^
  "-I%REPO%\src\native\common\chaos" ^
  "-I%REPO%\contracts\native\v0" ^
  "-I%REPO%\src\native\runtime-core" ^
  "-I%REPO%\src\native\interpreter" ^
  "-I%REPO%\src\native\bootstrap" ^
  "-I%REPO%\third_party\bdwgc\include" ^
  "-I%REPO%\third_party\fmt\include" ^
  "-I%BUILD%" ^
  -DCHAOS_IL2CPP_CHECK -DCHAOS_IL2CPP_TRACE_ENABLED -DCHAOS_IL2CPP_VERIFY_MODE -DCHAOS_RUNTIME_ABI_STATIC -DGC_NOT_DLL ^
  -Fo"%BUILD%\debug_minimal_main.obj" ^
  "%BUILD%\debug_minimal_main.cpp"
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%
...
link /nologo /out:"%BUILD%\debug_minimal_main.exe" ^
