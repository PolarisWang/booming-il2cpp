...
echo === Compile debug_include_gen.cpp ===
cl %FLAGS% %INCS% %DEFS% -Fo"%BUILD%\debug_include_gen.obj" "%BUILD%\debug_include_gen.cpp"
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%
...
link /nologo /out:"%BUILD%\debug_include_gen.exe" ^
  "%BUILD%\debug_include_gen.obj" ^
