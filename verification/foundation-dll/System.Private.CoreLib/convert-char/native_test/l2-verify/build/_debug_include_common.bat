...
echo === Compile gen.obj ===
cl %FLAGS% %INCS% %DEFS% -Fo"%BUILD%\gen_for_bisect.obj" "%GEN%" || exit /b 1

rem === Test with minimal_main + chaos/common.h include ===
>"%BUILD%\test_include_common.cpp" echo #include ^<cstdio^>
>>"%BUILD%\test_include_common.cpp" echo #include ^<chaos/common.h^>
>>"%BUILD%\test_include_common.cpp" echo int main() { printf("main() reached\n"^); fflush(stdout^); return 0; }

cl %FLAGS% %INCS% %DEFS% -Fo"%BUILD%\test_include_common.obj" "%BUILD%\test_include_common.cpp" || exit /b 1

set NATIVE_LIB=%REPO%\build\native\src\native
set BDWGC=%REPO%\build\native\bdwgc_build

link /nologo /out:"%BUILD%\test_include_common.exe" ^
  "%BUILD%\test_include_common.obj" ^
  "%BUILD%\gen_for_bisect.obj" ^
