...
echo === TEST 1: debug_minimal_main.obj + gen.obj (should work) ===
cl %FLAGS% %INCS% %DEFS% -Fo"%BUILD%\debug_minimal_main.obj" "%BUILD%\debug_minimal_main.cpp" || exit /b 1
cl %FLAGS% %INCS% %DEFS% -Fo"%BUILD%\gen_for_test.obj" "%GEN%" || exit /b 1

set NATIVE_LIB=%REPO%\build\native\src\native
set BDWGC=%REPO%\build\native\bdwgc_build

link /nologo /out:"%BUILD%\test_minimal_plus_gen.exe" ^
  "%BUILD%\debug_minimal_main.obj" ^
  "%BUILD%\gen_for_test.obj" ^
...
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%
"%BUILD%\test_minimal_plus_gen.exe"
echo RC1=%ERRORLEVEL%
echo.

echo === TEST 2: native_verify_main.obj + gen.obj ===
cl %FLAGS% %INCS% %DEFS% -Fo"%BUILD%\native_verify_main.obj" "%VERIFY_HOST%" || exit /b 1

link /nologo /out:"%BUILD%\test_verify_plus_gen.exe" ^
  ...
  ole32.lib user32.lib
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%
"%BUILD%\test_verify_plus_gen.exe"
echo RC2=%ERRORLEVEL%
echo.

echo === TEST 3: native_verify_main.obj alone (no gen code) ===
link /nologo /out:"%BUILD%\test_verify_alone.exe" ^
  "%BUILD%\native_verify_main.obj" ^
  "%NATIVE_LIB%\runtime-core\Release\chaos_runtime_core.lib" ^
  "%NATIVE_LIB%\bootstrap\Release\chaos_bootstrap.lib" ^
  "%NATIVE_LIB%\interpreter\Release\chaos_interpreter.lib" ^
