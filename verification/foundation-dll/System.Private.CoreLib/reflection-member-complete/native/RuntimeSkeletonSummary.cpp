...
// native-reference runtime skeleton for family/System.Private.CoreLib/reflection/member-complete
#include "bootstrap.h"
...
{
namespace runtime_skeleton_reflection_member_complete
{
...
int32_t CHAOS_RUNTIME_ABI_CALL RunNativeReferenceAssembly_reflection_member_complete(
    const CodegenBridgeV0* bridge,
    const CodeRegistrationV0* code_registration,
    const MetadataRegistrationV0* metadata_registration,
    const CodegenRegistrationOptionsV0* options,
    ...
    auto* domain = md::FindDomainByName(runtime_skeleton_reflection_member_complete::kAssemblyName);
    ...
    const auto* request = static_cast<const runtime_skeleton_reflection_member_complete::NativeReferenceAssemblyDispatchRequest*>(managed_args);
    if (request->method_id != 0) {
        return runtime_skeleton_reflection_member_complete::DispatchAssemblyMethodId(bridge, &local_code_registration, metadata_registration, options, runtime, thread, request->method_id, request->managed_args);
    }
    return runtime_skeleton_reflection_member_complete::DispatchAssemblySubject(bridge, &local_code_registration, metadata_registration, options, runtime, thread, request->subject_id, request->managed_args);
}

int32_t CHAOS_RUNTIME_ABI_CALL RunNativeReference_reflection_member_complete(
    const CodegenBridgeV0* bridge,
    const CodeRegistrationV0* code_registration,
    const MetadataRegistrationV0* metadata_registration,
    const CodegenRegistrationOptionsV0* options,
    ...
    ThreadState* thread,
    void* managed_args) {
    return RunNativeReferenceAssembly_reflection_member_complete(bridge, code_registration, metadata_registration, options, runtime, thread, managed_args);
}
}  // namespace chaos::il2cpp::generated
