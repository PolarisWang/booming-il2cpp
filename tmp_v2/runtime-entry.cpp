...
// kSubjectSubjectIds maps subject index → full SubjectId string.  Exported
// by the codegen-generated dispatch table (NativeAot.DispatchEntryCode.cpp).
// The fact runner outputs this as "methodSubjectId" so Python consumers can
// cross-reference against subjects.metadata.json by identifier rather than
// by integer index (whose space differs between kMethodTable and the metadata
// file — see NativeAot.DispatchEntryCode.cpp.scriban for the full rationale).
extern "C" const char* const kSubjectSubjectIds[];
...
// Assert lifecycle functions. These are managed methods
// (Chaos.TestFramework.Assert.Complete/Reset) that codegen emits as real AOT
// function bodies into the generated pages.
//
// There used to be a `__declspec(selectany) ... { return 0; }` definition right
// here, on the theory that the linker would let the codegen definition override
// it.  That theory is wrong for functions:
//
//   * __declspec(selectany) is specified for DATA (globals), not functions.
//     Applied to a function definition MSVC does not give it weak-override
//     semantics, so this was an ordinary strong definition competing with the
//     codegen one.
//   * It sits in the same translation unit as RunFactJsonMode, the only caller.
//     The compiler resolves the direct call within the TU and never emits a
//     relocation the linker could redirect at.
//
// Net effect: the `return 0` body always won, so `Assert_Complete()` reported
// "no assertion failure" for every subject ever run — the exit code set by
// Assert.Fail (see Assert.cs) was written and never read.  assertFailed was
// structurally incapable of being true.
//
// The weak fallback is NOT needed for chunks whose codegen closure has no
// assertions: Assert_Complete is only called from RunFactJsonMode below, and
// every chunk's generated pages define it (the SDK's assertion methods are part
// of the codegen closure whenever the subjects can assert).  If a future chunk
// genuinely lacks the symbol, the link will fail loudly here — which is the
// correct outcome, not a silent return-0.
//
// entry_stubs.cpp still provides a plain (non-weak) definition for the SDK
// runtime_stubs collection; that one is only compiled into the SDK lib, and
// codegen's definition takes precedence when both are present.
...
// Benchmark protocol output stream. Defaults to stdout (back-compat); when the
// harness passes --benchmark-out <file>, benchmark JSON is written to that
// file instead of stdout so the protocol data is decoupled from the process's
// console stream (logs go to stderr; machine data can go to a file).
static FILE* g_benchmark_out = stdout;
...
static LONG CALLBACK JitVehHandler(PEXCEPTION_POINTERS ExceptionInfo) noexcept {
    ...
    // Do NOT use RIP+=3 — that causes cascading crash loops.
...
static int RunFactJsonMode() {
    ...
    // s_exitCode from previous runs is cleared.
    //
    // CHAOS_FACT_CHECK is a no-op guard, NOT a detector: the per-subject loop
    // below reads-and-resets Complete() on every iteration, so by the time
    // this runs the exit code is provably 0.  It cannot observe anything.
    // The real attribution happens per-subject (see the assertionFailed /
    // caught pair in the loop).  Kept only so the existing call site at the
    // end of RunFactJsonMode stays valid.
    //
    // Note: Assert_Complete/Assert_Reset are the managed
    // Chaos.TestFramework.Assert.Complete/Reset methods, emitted as real AOT
    // bodies by codegen into the generated pages.  The `return 0` definition
    // in entry_stubs.cpp / this file's weak fallback only applies to chunks
    // whose codegen closure has no assertions — it is overridden by the strong
    // definition at link time (MSVC __declspec(selectany) / GCC __attribute__((weak))).
    // These are deliberately NOT gated on CHAOS_IL2CPP_JIT_MODE: the JIT build
    // compiles the same generated pages, so the symbols resolve there too.
...
#define CHAOS_FACT_CHECK()   do { \
    ...
    // Direct dispatch on main thread (no worker thread — worker threads
    // cannot access TLS runtime state set up by ChaosRuntimeHost on main thread).
    printf("{\"factResults\":[");
    bool first = true;
    for (int si = 0; si < kCount; si++) {
        int i = kSubjectSlotMap[si];
        ...
        if (_setjmp(t_abort_jmp) == 0) {
            __try {
                ...
                caught = true;
                // ── Diagnostic: identify the subject that faulted ──
                // A caught SEH exception (STATUS_ACCESS_VIOLATION, etc.) means the
                // generated AOT body dereferenced an invalid pointer BEFORE any
                // assertion ran.  Without this line the only observable is
                // caught=true, which is indistinguishable from a genuine assertion
                // failure inside the same loop.  Emitting the subject id plus the
                // SEH code turns an opaque failure into a locatable one.
                // See docs/dev/in-progress/debug-20-real-defects.
                {
                    const char* _sid =
                        (si < kSubjectEntryCount && kCount > 0 &&
                         kSubjectSubjectIds != nullptr && kSubjectSubjectIds[si] != nullptr)
                            ? kSubjectSubjectIds[si] : "";
            ...
            caught = true;
            // Subject id in the log (mirrors the SEH-FAULT line): SIGABRT triage
            // otherwise requires manually mapping si → methodSubjectId.
            const char* _abort_sid =
                (si < kSubjectEntryCount && si < kCount && kSubjectSubjectIds != nullptr && kSubjectSubjectIds[si] != nullptr)
                    ? kSubjectSubjectIds[si] : "";
            std::fprintf(stderr, "[ABORT-FAULT] si=%d %s\n", si, _abort_sid);
        }

        // Attribute the outcome by EVIDENCE SOURCE, not by "did it throw".
        //
        // Two independent facts are observable here and they must not be
        // conflated:
        //   assertionFailed — the managed assertion ran and recorded a failure
        //                     (the exit code was set before Complete() zeroed it)
        //   caught          — the dispatch raised, for ANY reason
        //
        // `assertFailed` used to be `!caught && (Complete() != 0)`.  That
        // coupling is wrong now that the method body rethrows managed
        // exceptions (EmitManagedMethod `catch(...) { throw; }`): a failed
        // assertion throws AssertionException, which now propagates and sets
        // caught=true — so the `!caught` guard suppressed the very signal it
        // was meant to capture, and every assertion failure was reported as a
        // plain non-assertion failure.
        //
        // Reading Complete() unconditionally is safe and side-effect-free: it
        // is the managed `Assert.Complete()`, which returns the thread-static
        // exit code and resets it.
        //
        // A subject is a failure if EITHER signal fires.  `assertionFailed`
        // with caught=true is the expected shape for a genuine assertion
        // failure; the two counters let the pipeline tell that apart from a
        // crash that happened before any assertion ran.
        ...
        bool passed = !caught && !assertionFailed;

        if (!first) printf(",");
        printf("{\"si\":%d,\"methodIndex\":%d,\"methodSubjectId\":\"%s\",\"contractIndex\":-1,\"passed\":%s,\"value\":%" PRId64 ",\"assertFailed\":%s,\"caught\":%s}",
               si, i,
               (si < kSubjectEntryCount && si < kCount && kSubjectSubjectIds != nullptr && kSubjectSubjectIds[si] != nullptr)
                   ? kSubjectSubjectIds[si] : "",
               passed ? "true" : "false", result,
               assertionFailed ? "true" : "false",
               caught ? "true" : "false");
        first = false;
    }
    printf("]}\n");
    ...
    for (int si = 0; si < kCount; si++) {
        ...
        if (setjmp(t_abort_jmp) == 0) {
            try {
                                ...
                                // JIT mode: PalDispatchWrapper is not defined (AOT-only).
                                // Call ChaosDispatchMethodGetValue directly — JIT precode
                                // stubs handle dispatch and exceptions separately.
                                result = chaos::il2cpp::runtime_core::ChaosDispatchMethodGetValue(
        ...
        if (!first) printf(",");
        printf("{\"si\":%d,\"methodIndex\":%d,\"methodSubjectId\":\"%s\",\"contractIndex\":-1,\"passed\":%s,\"value\":%" PRId64 "}",
               si, v_i,
               (si < kSubjectEntryCount && si < kCount && kSubjectSubjectIds != nullptr && kSubjectSubjectIds[si] != nullptr)
                   ? kSubjectSubjectIds[si] : "",
               caught ? "false" : "true", caught ? result : result);
        first = false;
    }
    printf("]}\n");
    std::fflush(stdout);
...
#if defined(_WIN32)
        // Wrap the ENTIRE per-method body in __try so process-level crashes
        // (stack overflow, AV in no_gc_region, heap corruption) are caught
        // and the benchmark can continue with the next method.
        __try {
#endif
            // Warmup: per-call crash protection
            for (int w = 0; w < 100; w++) {
            ...
            if (crashed) {
                fprintf(g_benchmark_out, "{\"methodIndex\":%d,\"elapsedMilliseconds\":-1.0,\"allocatedBytes\":0,\"error\":\"managed exception\"}\n",
                       i);
                fflush(g_benchmark_out);
#if defined(_WIN32)
                __leave;
#else
                continue;
            ...
            if (crashed) {
                fprintf(g_benchmark_out, "{\"methodIndex\":%d,\"elapsedMilliseconds\":-1.0,\"allocatedBytes\":%" PRId64 ",\"error\":\"managed exception\"}\n",
                       i, alloc_after - alloc_before);
            } else {
                double elapsed_ms = std::chrono::duration<double, std::milli>(end - start).count();
                double ops_per_sec = elapsed_ms > 0.0 ? (iterations / elapsed_ms) * 1000.0 : 0.0;
                fprintf(g_benchmark_out, "{\"methodIndex\":%d,\"elapsedMilliseconds\":%.3f,\"opsPerSecond\":%.0f,\"iterations\":%d,\"allocatedBytes\":%lld}\n",
                       i, elapsed_ms, ops_per_sec, iterations, (long long)(alloc_after - alloc_before));
            }
            fflush(g_benchmark_out);
#if defined(_WIN32)
        } __except(EXCEPTION_EXECUTE_HANDLER) {
            // Process-level crash caught (e.g. AV in no_gc_region, stack overflow).
            // Record the failure and continue with the next method.
            fprintf(g_benchmark_out, "{\"methodIndex\":%d,\"elapsedMilliseconds\":-1.0,\"allocatedBytes\":0,\"error\":\"fatal crash\"}\n",
                   i);
            fflush(g_benchmark_out);
        }
#endif
    }
    fprintf(g_benchmark_out, "{\"summary\":{\"methodCount\":%d}}\n", kCount);
    fflush(g_benchmark_out);
    return 0;
}
...
static int RunHotupdateMode(const char* patchDataPath = nullptr) {
    ...
    for (int si = 0; si < kCount; si++) {
        ...
        bool caught = false;
#if defined(_WIN32)
        // Per-method SEH + setjmp guard: catch AV/C++ exceptions individually so a
        // single method failure doesn't abort the entire hotupdate run.
        // The setjmp recovery catches deferred crashes (e.g. heap corruption from
        // the dispatched function that manifests on printf/fflush).
        ...
        // Guard printf against deferred heap corruption from dispatch
#if defined(_WIN32)
        __try {
            if (si > 0) printf(",");
            ...
            if (si > 0) printf(",");
            printf("{\"si\":%d,\"passed\":false,\"value\":0,\"error\":\"output_failed\"}", si);
        }
#else
        if (si > 0) printf(",");
        printf("{\"si\":%d,\"passed\":%s,\"value\":%" PRId64 "}",
    ...
    // ApplyPatchFromMemoryEx resolves each patch method by NAME through
    // HotpatchNameRegistry, and that lookup lands on whatever slot the
    // registry's (module_id, token) pair maps to — not necessarily the
    // kSubjectSlotMap flat index this harness dispatches.  Diagnostics
    // confirmed every subject slot still reads flags=0x00000000 and
    // method_key=0x0 after a successful patch, so the harness was calling
    // the unpatched AOT body and always observing the baseline value
    // (semanticChangedCount=0).
    //
    // Fix: after the patch is loaded, mark every subject slot active
    // directly.  HotpatchIsActive() then routes ChaosDispatchMethodGetValue
    // through InterpreterEntryDirect, which executes the patched IR.
    // kHotpatchKeepNative is cleared in the same store so the interpreter is
    // not bypassed; the two-step version had a window where neither bit was
    // set (see SetPatchedBySlot's atomic-flags note).
    if (patch_ctx != nullptr) {
        auto* subject_entries = const_cast<HotpatchEntryV0*>(GetHotpatchEntries());
        for (int si = 0; si < kCount; si++) {
            int slot = kSubjectSlotMap[si];
            if (slot < 0 || slot >= kAotMethodCount) continue;
            auto& atomic_flags =
                *reinterpret_cast<std::atomic<uint32_t>*>(&subject_entries[slot].flags);
    ...
    for (int si = 0; si < kCount; si++) {
        ...
        // Only flag when a patch was actually applied — without patch data the
        // return value comparison is unreliable (void methods return garbage RAX).
        // P0-C (false+-fix): additionally require the patched value to be in the
        // sentinel range [0xBEEF0000, 0xBEEFFFFF] — the PatchDataExtractor rewrites
        // Subject_N bodies to "ldc.i4 0xBEEF0000|idx; ret" (see PatchDataExtractor.cs
        // L630-634).  A patched value in this range proves the interpreter executed
        // the replacement body (not the original AOT body).  Without this range check,
        // void methods whose baseline is garbage RAX and keep-native methods whose
        // patched value is also garbage (different from baseline) would produce false
        // semantic change — the "change" is just RAX noise, not a real patch application.
        // The sentinel range [0xBEEF0000, 0xBEEFFFFF] is specific enough that a real
        // return value collision is virtually impossible (the 0xBExxxxxx prefix is
        // reserved for debug sentinels in the test framework).
        //
        // The comparison is done in UNSIGNED space.  PatchDataExtractor writes the
        // sentinel as a signed int32 (e.g. 0xBEEF0000 → -1091633152), and that value
        // survives the interpreter round-trip as a negative int64.  Comparing it
        // against the signed literal 0xBEEF0000LL (a large positive number) would
        // classify every real sentinel as "outside the range" and suppress the count,
        // which is exactly the semanticChangedCount=0 symptom.  Reinterpreting both
        // sides as uint64 compares the bit pattern the sentinel was built from.
        auto _patched_bits = static_cast<CHAOS_IL2CPP_UINT64>(patched_value);
        if (patch_ctx != nullptr && !baseline_caught[si] && !patched_caught &&
            baseline_value[si] != patched_value &&
            (_patched_bits < CHAOS_IL2CPP_UINT64_C(0xBEEF0000) || _patched_bits > CHAOS_IL2CPP_UINT64_C(0xBEEFFFFF))) {
            // Patched value is NOT in the sentinel range — this is a keep-native
            // method or a void method with garbage RAX.  Do NOT count as semantic
            // change (the baseline value is undefined for void methods).
        } else if (patch_ctx != nullptr && !baseline_caught[si] && !patched_caught &&
                   baseline_value[si] != patched_value) {
    ...
    // P2-A: report the actual number of dispatch entries that were patched.
    // This allows the Python gate to cross-check semantic_changed against
    // activePatchCount — if semantic_changed > 0 but activePatchCount == 0,
    // something is wrong (semantic change from a non-patch source).
    int active_patch_count = (patch_ctx != nullptr) ? static_cast<int>(patch_ctx->method_count) : 0;
    printf("\"activePatchCount\":%d,", active_patch_count);
...
#pragma warning(pop)
    ...
    // --benchmark-out <file>: route benchmark protocol JSON to a file instead of
    // stdout (decouples machine data from the console stream). Closes any prior
    // handle if present; on failure falls back to stdout (non-fatal).
    for (int ai = 1; ai < argc; ai++) {
        if (std::strcmp(argv[ai], "--benchmark-out") == 0 && ai + 1 < argc) {
            FILE* f = std::fopen(argv[ai + 1], "w");
            if (f != nullptr) {
                if (g_benchmark_out != stdout) std::fclose(g_benchmark_out);
                g_benchmark_out = f;
            } else {
                std::fprintf(stderr, "WARN: cannot open --benchmark-out %s; using stdout\n", argv[ai + 1]);
            }
            break;
        }
    }
    ...
    // ── Override unresolved external runtime entries ──────────────
    // After module registration, entries from non-whitelisted external
    // assemblies still point to CHAOS_IL2CPP_FAIL stubs that would abort
    // the process.  Replace them with safe return-0 stubs and also patch
    // the corresponding hotpatch entries so the interpreter doesn't route
    // through the fail stubs either.
    chaos_host->OverrideUnresolvedExternalRuntimeEntries();
...
shutdown:
    // ── Interpreter profiling hook (项8 基建) ─────────────────────────────
    // Dump the FastExecute opcode-frequency histogram + register-VM per-method
    // cycles so an entry.exe --benchmark / --benchmark-all run in a profile
    // build (CHAOS_IL2CPP_CONFIG_PROFILE → CHAOS_IL2CPP_PROFILE_ENABLED=1, or
    // the separate CHAOS_IL2CPP_VM_PROFILER_ENABLED=1) exposes the interpreter's
    // per-opcode distribution — the interpreter perf anchor the 2026-08-13
    // review flagged as missing.  Fully compiled out unless a macro is on, so
    // default/optimized builds are unaffected.
    //
    // NOTE: DumpFastExecuteOpcodeHistogram is in the chaos_interpreter lib,
    // which is NOT linked by all profile builds (only families with interpreter-
    // heavy subjects).  The GC profile data (ProfileRecord*, ProfileEmitJson)
    // lives in chaos_runtime_core and is always available.  The histogram dump
    // is disabled here to avoid link errors; it can be re-enabled once the
    // profile-build cmake template guarantees chaos_interpreter linkage.
    //
    // The VM profiler (CHAOS_IL2CPP_VM_PROFILER_ENABLED) is also in
    // chaos_interpreter and gated by its own macro, which is OFF in profile builds.
#if 0
#if CHAOS_IL2CPP_PROFILE_ENABLED
    {
        extern void chaos::il2cpp::runtime_core::DumpFastExecuteOpcodeHistogram() noexcept;
        chaos::il2cpp::runtime_core::DumpFastExecuteOpcodeHistogram();
    }
#endif
#if CHAOS_IL2CPP_VM_PROFILER_ENABLED
    {
        extern void chaos::il2cpp::interpreter::DumpProfilerToFile(const char* path) noexcept;
        extern void chaos::il2cpp::interpreter::ResetProfiler() noexcept;
        chaos::il2cpp::interpreter::DumpProfilerToFile(nullptr);  // nullptr → DumpAll() to stderr
