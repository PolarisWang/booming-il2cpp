...
set SUMMARY=%REPO%\hotpatch_summary.txt

echo Hotpatch Test Summary > %SUMMARY%
echo ========================= >> %SUMMARY%
echo. >> %SUMMARY%
...
for %%f in (
    ...
    if exist "!BUILD_DIR!\_build_hotpatch_%%f.bat" (
        call "!BUILD_DIR!\_build_hotpatch_%%f.bat"
        if !ERRORLEVEL! equ 0 (
            if exist "!BUILD_DIR!\hotpatch_verify_%%f.exe" (
                "!BUILD_DIR!\hotpatch_verify_%%f.exe" > "!BUILD_DIR!\d3_result.json" 2>&1
                set EX=!ERRORLEVEL!
            ) else ( set EX=-2 )
        ) else ( set EX=-1 )
    ) else ( set EX=-3 )
