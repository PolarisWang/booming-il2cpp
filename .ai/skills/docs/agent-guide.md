...
| 路径 | 职责 |
|------|------|
| `.ai/skills/discovery/skill-index.md` | 技能索引；列出所有领域与技能数量 |
| `.ai/skills/discovery/registries/<domain>.md` | 按领域拆分的二级 registry；从 manifest 自动生成 |
| `.ai/skills/discovery/domain-catalog.json` | 领域定义、摘要、读取提示与排序 |
| `.ai/skills/library/skills/` | 正式技能源目录（唯一可信源） |
| `.ai/skills/library/skills/<name>/skill.manifest.json` | 技能的结构化元数据 |
| `.ai/skills/lifecycle/incubator/` | 草稿区；不参与发现层 |
| `.ai/skills/lifecycle/learning/` | 学习信号与 candidate 聚类产物 |
| `.ai/skills/lifecycle/telemetry/` | 使用率数据 |
| `.ai/skills/lifecycle/governance/` | promotion / retire 日志与备份 |
