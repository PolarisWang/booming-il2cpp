...
计划收尾必须进入固定闭环：`结构告警与架构审视 -> 测试通过 -> 归档 completed -> 合并&提交`。

对于来自已批准的 roadmap 的 child task，默认目标是自动继续执行，而不是在每一步都回头询问用户。执行前只做最小 `preflight self-review`；只要结果是 `pass` 或 `warn`，就继续推进。只有命中 `stop` 条件，才停下来确认：
...
- 不要在 `notes/progress-*.md` 当作每一步都要追加的流水账
- 不要为了一次普通推进同时改 `STATUS.md`、`ACTIVE.md`、`notes/progress-*.md`、索引和 wiki，除非这些对象真的都发生了变化
- completed 阶段默认写结构化证据，不写长总结

**目录一致性原则：**

- `docs/dev/in-progress/` 只允许承载 `lifecycle_status: in-progress` 的任务。如果任务进入 `completed`、`hanging` 或 `abandoned`，必须同步更新 `STATUS.md` 前置字段、`current_dir`、索引和物理目录
- 绝不允许只在 `STATUS.md` 正文、计划复选框或父 roadmap 表格里写"已完成/已挂起/已放弃"，但当前任务目录仍停留在 `docs/dev/in-progress/`
...
- 不要只更新父 roadmap 或父任务摘要，而跳过当前 child task 的终态归档。
...
`STATUS.md` 是真相源、`ACTIVE.md` 是活动指针、handoff 是交接语义层——**仅当 task 数较多（≥5）或跨会话时**，在任务目录 `notes/` 下追加一个轻量 ledger 文件 `notes/ledger-<plan-basename>.md`，作为「已完成任务机器清单」，不替代 STATUS：

- 首行锁计划身份：`# ledger — plan: <plan-basename>`
- 每完成一个任务，追加一行：`Task <N>: complete <提交或验证哈希>`
- context 压缩后恢复：读 ledger 首行确认身份 → 第一个没有 `complete` 的 task 即恢复点。**注意 ACTIVE.md 可能指向别处**，以 ledger 锁定的 plan 身份为准，不重派已 complete 任务。
- 目录归属：ledger 只在当前任务目录的 `notes/`，**不建独立工作区、不碰他人任务目录**，与"目录一致性原则"一致。
- 身份锁首行防误读他人 plan 的 ledger（禁止读别人任务目录的 notes）。

## 第一步：加载并审查计划

1. 读取当前任务目录中的计划文件，明确识别计划中的任务总数
2. 提取执行需要的关键引用：
   - 当前 `STATUS.md` 中的 `blocking_questions`
   ...
   - 每个任务的知识沉淀目标
3. 批判性审查计划，识别任何疑虑
   - 如果发现执行前问题尚未清零，或清零状态缺失且无法从已批准输入中确认：停止执行，返回 `brainstorming` 或 `writing-plans`
   - 不要把执行阶段变成补设计拍板或补 open questions 的地方。
4. 如无疑虑：创建或更新
   - 当前任务目录中的 `STATUS.md`
   - `docs/dev/ACTIVE.md`
...
- 开始实现前，必须确认当前任务 `STATUS.md` 至少满足：
  - `blocking_questions: []`
  - `question_clearance: cleared`
  - `clearance_confirmed_by_user: true`
- 如果任务来自 `roadmap-child`、`plan` 或恢复执行，但上述条件不成立：
  - 不得继续实现
  - 不得在执行阶段自行补拍板后假定问题已关闭
  - 返回上游 `brainstorming` 或 `writing-plans` 修正输入
- 执行阶段允许继续提出的问题，只能是：
  - 当前实现遇到的 `blocker`
  - 验证失败或编译崩溃的根因
  - 结构告警与架构审视发现的实现级问题。
- 执行阶段不负责继续确认边界、authority、阶段切分或关键结构方案

## roadmap-child 自动执行守卫

- 如果当前任务带有 `source_relation: roadmap-child*`，或 `clearance_source: parent-roadmap`：
  - 默认按父 roadmap 的 `child_execution_mode` / `auto_continue` / `auto_stop_policy` 执行
  - 若父 roadmap 的 `child_execution_mode = auto`，则不为 child 再次向用户索要启动确认。
- 开始实现前，必须存在一次最小 `preflight self-review`
  - 如果 `preflight_review` 已存在：直接消费
  - 如果缺失：先补做一次最小 `preflight self-review` 并写入 `STATUS.md`
- 最小 `preflight self-review` 检查项：
  - 当前 child 是否仍处于父 roadmap 已批准边界内
  - 是否出现新的边界漂移、authority 变化、职责变化或关键结构改动
  - 是否出现新的执行前问题，无法直接从父 roadmap / plan / design 输入中解决
  - `depends_on`、阶段依赖、`requiredGates` 或 obligation 是否已满足
  - 是否新增 formal object、proof、benchmark、hotupdate 或其它必须先确认的义务
  - 当前 `ACTIVE.md`、目录状态、父子任务指针是否一致
- 结果只允许：
  - `preflight_review: pass`
  - `preflight_review: warn`
  - `preflight_review: stop`
- 对应动作：
  - `pass`：写 `auto_execution_decision: continue`，直接继续。
  - `warn`：把风险写入 `STATUS.md`，写 `auto_execution_decision: continue`，继续。
  - `stop`：写 `auto_execution_decision: stop`，停止自动推进，并向用户说明问题后等待确认。
- 只有以下情况才应判为 `stop`：
  - 新的执行前问题无法用已批准输入解决
  - 子任务范围已超出 roadmap 已批准边界
  - authority / owner / contracts / required gates 发生变化
  - 必须新增 proof / benchmark / formal object
  - 前置依赖未满足，继续推进会造成错误执行
  - 如果 roadmap 本身需要修改，而不是 child 内部实现即可解决
...
1. 在开始实现前，先把当前任务编号或任务名称写入 `STATUS.md` 和 `docs/dev/ACTIVE.md`
2. 严格按照计划步骤执行
3. 按规定运行验证。
4. 完成一次最新的结构告警与架构审视
...
10. 如果当前任务带有 `parent_task_id`：
    ...
    - 如果 roadmap 写明 `child_execution_mode: auto` 或 `auto_continue: true`：
...
- 存在单文件混入多种职责，尤其是策略决策、数据映射、输出拼装耦合在一起。
- 对于 `codegen`，是否应把输出拼装下沉到 `Scriban`；或反过来把复杂语义决策从 `Scriban` 提回宿主代码
- 对于 codegen 改动，是否已运行快照测试确认基线变更符合预期

约束：

- 不使用固定行数作 gate；文件大小只能作为上下文信号
- 不引入独立人工复核流程；`warn` 由当前执行者在推进中处理。
- 最新一轮结构告警必须覆盖当前 diff 或当前批次涉及的文件
- `arch_review_mode = critical` 的任务，除上述检查外，还必须补齐最新 `权责图审核`

`critical` 任务的最新 `权责图审核` 至少包括：
...
如果 authority 要求正式对象的 case，但仓库里还不存在，不得跳过；先补对象或 case，再继续完成链路。

## 第三步：完成闭环

所有任务完成后，必须按固定顺序收尾：`结构告警与架构审视 -> 测试通过 -> 归档 completed -> 合并&提交`。

1. 完成最新一轮结构告警与架构审视
   - 覆盖最新 diff 或最终受影响文件集合
   - 重点确认重复逻辑、结构与抽象、文件职责拆分、`codegen` 与 `Scriban` 分层
...
如果某项不存在，用 `n/a`，不要为了"完整叙述"补长段说明。`summaryPath` / `eventsPath` / `consolePath` 只要仓库提供，就应写入；如本轮要求新的 verification 数据，则 `verification_data.*` 也必须同步写入，保证后续恢复与复查可重新定位正式产物。

## Rulings 裁决默认（运行中不等人类）

运行中的 plan **默认裁决前进**：对「小而散的歧义」——计划里的冲突、轻微缺陷、需要超出的额度、两可解读——**当场裁决，记入 STATUS.md/ledger 的 `Ruling:` 行长，继续执行**，不为这些停下打断。

格式：`Ruling: <决定了什么> — <为什么> — <错了代价多大>`

本小节**只放宽「小而散的歧义」**，不削弱下方「何时停下来寻求帮助」中的硬性停止条件（阻塞、计划关键缺口、不理解指令、验证反复失败、roadmap 边界/authority 需重审等——那些仍必须停）。换句话说：**该停的停（既有「何时停下来」保留），不该停的别因小歧义浪费整场会话**。

必要时必须停下等确认的高风险动作（与既有停止条件互补）：
1. 不可逆或破坏性操作（删数据、破坏性重建）
2. 安全敏感行动
3. 会作用到当前任务目录之外 / 非计划工作区的副作用
4. 需要人类做出不可逆承诺的决策

一条错误裁决的代价是可见、可回滚的返工；把整个会话停在一个小问题上则赔上全天。**spec 是约束权威，plan 是它的论证，裁决定两者都不涉的细微处。**
...
- 你不理解某条指令
- 验证反复失败
- roadmap-child 的 `preflight self-review = stop`
- 如果 roadmap 边界、authority 或执行策略需要重审

在停下来之前，把新的风险、阻塞和下一步记录进 `STATUS.md`。如果本次停下属于阻塞、重要决策或验证失败，再同步写入最新 `notes/progress-*.md`。
