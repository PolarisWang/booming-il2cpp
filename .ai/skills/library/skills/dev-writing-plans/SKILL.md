...
## 任务的接口契约（Interfaces 块）

每个任务在其文件清单后，必须写一个 `Interfaces` 契约块，让实现者只读自己的任务就能拿到相邻任务的精确签名，无需读彼此上下文：

**Interfaces:**
- Consumes: [这个任务用到的、来自更早任务的精确签名 — 函数名 + 参数 + 返回类型]
- Produces: [这个任务产出、要被后续任务依赖的精确签名 — 函数名 + 类型]

写 plan 时，任务 N 的 `Produces` 必须与任务 N+1 对同一符号的 `Consumes` 逐字段一致。这与上文「计划要求」中“先列文件及职责、每个任务独立可验证”配合：**文件结构决定任务切分，接口契约保证跨任务类型一致**。类型/签名不一致（如任务 3 写 `clearLayers()`、任务 7 写 `clearFullLayers()`）是本技能的自审必查项。
