...
> ⚠️ **分层**：
> - 「四层架构红线」与「统一内存分配」两节是 **CLAUDE.md 🔴 红线 3/4 的强制执行明细** —— **强制，不可违规**，违反即阻断/数据损失。它们不是"参考建议"。
> - 底部「编码规范 wiki 链接」节是**纪律级参考**（应当遵守，违反被审查）。
> 本文件由 CLAUDE.md 红线 3/4 引用（`明细见 .ai/references/coding-references.md`），故红线内容以本文件为准。
